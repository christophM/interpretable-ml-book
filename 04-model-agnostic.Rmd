...
##### Example
Let's look at a concrete example. We go back to the bike rental and predict the number of rental bike on a given day. 

```{r lime-tabular-example-train-black-box, cache = TRUE}
ntree = 30

model <- caret::train(cervical[names(cervical) != 'Biopsy'],
               as.factor(cervical$Biopsy),
...
set.seed(44)
explain <- lime::lime(cervical[names(cervical) != 'Biopsy'], model, bin_continuous=FALSE)
...
explanation <- explain(cervical[names(cervical) != 'Biopsy'],
