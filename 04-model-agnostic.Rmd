...
Partial dependence plots show the marginal effect of a variable on the target (regression / classification) [@friedman2001greedy]. A partial dependence plot can show if the relationship between target and feature is linear, monotonic or something else.
...
```{r dpd-data, fig.cap='Artificial dataset with x1, x2 and y as color. Bright color indicate a high predicted value for y.'}
# Already using the LIME data here.
source(sprintf('%s/lime.R', src_dir))
set.seed(42)
rf_numerical = randomForest::randomForest(as.numeric(y_noisy) ~ x1 + x2, data = lime_training_df, ntree=100)
...
```{r dpd, fig.cap = 'The plots show the partial dependence plots of x1 and x2. x1 exhibits the V-shaped structure.'}
set.seed(42)
pds = mmpf::marginalPrediction(data = lime_training_df, vars = c('x1'), n = c(15, 500), model = rf_numerical)

px1 = ggplot(pds) +
  geom_line(aes(x=x1, y=preds)) + my_theme()


pds = mmpf::marginalPrediction(data = lime_training_df, vars = c('x2'), n = c(15, 500), model = rf_numerical)

px2 = ggplot(pds) +
  geom_line(aes(x=x2, y=preds)) + my_theme()


gridExtra::grid.arrange(px1, px2, ncol=2)
```
...
### Individual dependence plots (ICP) or Individual Conditional Expectation (ICE)
