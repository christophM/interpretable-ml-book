...
In predictive modeling, you have to make a trade-off:
...
The necessity for interpretability comes from an incompleteness in the problem formalization [@Doshi-Velez2017], meaning that for certain problems or tasks it is not enough to get the answer (the **what**).
...
To facilitate learning and satisfy curiosity about why certain predictions or behaviors are created by machines, interpretability and explanations are crucial.
...
There is a contraction between the knowledge about the dog's past behavior and the newly made, unpleasant experience of the bite.
...
The more a machine's decision affects a human's life, the more important it will be for the machine to explain its behavior.
...
An explanation might reveal that the most important feature learned is to recognize the two wheels of a bike and this explanation helps you to think about edge cases like bikes with side bags, that partially cover the wheels.
...
In this case, the incompleteness in the problem formulation lies in the fact that you not only want to minimize loan defaults, but you are also required to not discriminate based on certain demographics.
This is an additional constraint, which is part of your problem formulation (Handing out loans in a low-risk and compliant way), which is not captured by the loss function, which the machine learning model optimizes.
...
In a famous experiment, @heider1944experimental showed the participants videos of shapes, where a circle opened a door to enter a "room" (which was simply a rectangle).
...
I would not completely accept my robot vacuum cleaner if it would not explain its behavior to some degree.
...
Interestingly, there can be a misalignment between the goal of the explaining machine, which is generating trust, and the goal of the recipient, which is to understand the prediction or behavior.
...
These reasons (and some more) caused the robot to get stuck, but it only explained that there was something in the way, and this was enough for me to trust its behavior, and to get a shared meaning of that accident, which I can share with my girlfriend.
...
A user of a house prize prediction website might want to have an explanation of a house prize prediction contrastive to her own house or maybe to some other house on the website or maybe to an average house in the neighborhood.
...
"Tsubasa and his team lost the match because of a weak defense: they left their opponents to much free space to play out their strategy.",
...
Ensemble methods and combine multiple models with different features (different explanations) and thrive because averaging over those "stories" makes the predictions more robust and accurate.
...
"The decentralised, distributed blockchain-based ledger that cannot be controlled by a central entity resonates with people's desire to secure their wealth, which explains the high demand and prize.".
...
People focus more on abnormal causes to explain events (@kahneman1981simulation).
...
Even if some attribution method finds out that the three balconies contribute the same prize difference as the above average house size, the good neighborhood and the recent renovation, the abnormal feature "three balconies" might be the best explanation why the house is so expensive.

**Explanations are truthful**.
Good explanations proof to be true in reality (i.e. in other situations).
...
An example would be negative effects of house size for the predicted prize of the house for a few of the houses, which, let's assume, improves accuracy (because of some complex interactions), but strongly contradicts prior beliefs.
