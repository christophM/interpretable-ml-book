...
Making a machine learning interpretable does not necessarily mean providing a (human-style) explanation of a prediction.
...
- Probability. But keep in mind that people tend to wrongly judge probability of joint events. (Joe is a librarian. Is it more likely that he is shy or that he is a shy person that loves reading books?)
...
Knowing that good explanations should be contrastive, selected and social, it is easy to see, that complete causal attribution don't make good explanations.
Causal attributions give each cause a 'weight' instead of attributing everything to one or two causes (not selective), they are often not contrastive to a concrete other scenario and don't adapt to the social situation.
Yet, a complete causal attribution is probably needed, when the reasons for an "explanation" is legally required.
Also if you were to debug a system, you want causal attribution, which gives out the full picture of causes for a prediction, instead of a selective, contrastive explanation.
The human-style explanation is more useful when it has to be fast and easy to explain a prediction to a human, especially lay-humans.
