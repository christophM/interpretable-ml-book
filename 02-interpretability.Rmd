...
Interestingly there can be a disalignment between the goal of the explaining machine, which is generating trust and the goal of the recipients, which is to understand the prediction.
...
To explain why class A was predicted rather than class B, a good explanation consists of the difference between class A and B and not what constituts class A (or class B) in general.
...
For loan default, a predicted default could be compared to typcial examples, where the loan would not default.


Definition of explanations:
Someone explains something to someone.
In machine learning:
Some interpretability method explains a prediction to the end user or the machine learning developer.
Explanations are conversations.
The difference between causal attribution and an explanation is that the explanation has a conversational aspect.
Step 1: The explainer determines the causality
Step 2: The explanation is delivered in a social process.
