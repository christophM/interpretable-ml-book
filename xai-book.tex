...
The i-th observation's outcome is a weighted sum of it's \(p\) features.
...
\item
  ...
  for prices to vary.
\item
  \textbf{Independence}: Each observation is assumed to be independent
  from the next one. If you have repeated measurements, like multiple
...
\(p\) is the number of features and \(n\) the number of observations.
...
\addcontentsline{toc}{subsection}{Interpretation templates}
...
largest contributions are from temperature and the days feature, which
...
feature has goes from zero to large positive contribution, because the
...
the predicted bike count: Temperature (5.20089) and the trend feature
``days\_since\_2011'', because this instance is from late 2011 (value =
698).
...
feature selection and regularization of the selected features. We did
