...
This book is about interpretable machine learning: Making machine
decisions interpretable. Machine learning is being integrated into many
products and processes of our daily lives, yet decisions made by
machines don't automatically come with an explanation. An explanation
increases the trust in the decision and in the machine learning model.
As the programmer of an algorithm you want to know whether you can trust
the learned model. Did it learn generalizable features? Or are there
some odd artifacts in the training data which the algorithm picked up?
This book will give you an overview over techniques that you can use to
make black boxes as transparent as possible and make their predictions
interpretable. The first part of the book introduces algorithms that
produce simple, interpretable models and instructions how to interpret
the output. The later chapters focus on general tools that help
analyzing complex models and making their decisions interpretable. In an
ideal future, machines will be able to explain their decisions and make
a transition into an algorithmic age more human.
...
\subsection{Who should read this book}\label{who-should-read-this-book}
...
\subsection{What is machine learning and why is it
...
\subsection{When is interpretability
...
\subsection{Scope of explainability}\label{scope-of-explainability}
...
\subsubsection{Algorithm transparency}\label{algorithm-transparency}
...
\subsubsection{Global, holistic model
...
\subsubsection{Global model explainability on a modular
...
\subsubsection{Explain the decision for a single
...
\subsubsection{Explain the decisions for a group of
...
\subsection{Evaluating explainability}\label{evaluating-explainability}

There is no real consensus what explainability in machine learning is.
Also it is not clear how to measure it.

\subsubsection{Approaches for evaluation of the explanation
...
\paragraph{Function level evaluation}\label{function-level-evaluation}
...
\subsection{Regression dataset: Bike sharing
...
\subsection{The dataset: speed dating}\label{the-dataset-speed-dating}
...
\subsection{TubeSpam dataset: Spam classification on YouTube
...
\subsection{Risk factors for cervical
...
\section{Simple, interpretable models}\label{simple}
...
\subsection{Linear models}\label{limo}
...
\subsubsection{Interpretation example}\label{interpretation-example}
...
\subsubsection{Interpretation templates}\label{interpretation-templates}
...
\subsubsection{Visual parameter
interpretation}\label{visual-parameter-interpretation}

\paragraph{Weight plot}\label{weight-plot}
...
times on
\includegraphics{xai-book_files/figure-latex/linear_weights-1.pdf} TODO:
Add interpetation

\paragraph{Effect plot}\label{effect-plot}
...
\includegraphics{xai-book_files/figure-latex/linear_effects-1.pdf} The
...
\subsubsection{Explaining single
...
effects.
\includegraphics{xai-book_files/figure-latex/linear_effects_single-1.pdf}
...
\subsubsection{Coding categorical
...
\subsubsection{The disadvantages of linear
...
\subsubsection{Towards complexer relationships within linear model
...
\subsection{Sparse linear models}\label{sparse-linear-models}
...
\subsection{Logistic regression: a linear model for
...
\subsubsection{What's wrong with linear regression for
...
\subsubsection{Interpretation example}\label{interpretation-example-1}
...
\subsection{Other simple, interpretable
...
\subsubsection{RuleFit}\label{rulefit}
...
\subsubsection{And so many more \ldots{}}\label{and-so-many-more}
...
\section{Model-agnostic explanations}\label{model-agnostic-explanations}
...
\subsection{Global: Explain the behaviour of a
model}\label{global-explain-the-behaviour-of-a-model}

\subsubsection{Partial dependence plot}\label{pdp}
...
\subsubsection{Average Marginal
Effects\}}\label{average-marginal-effects}
...
\paragraph{LOCO}\label{loco}

\paragraph{Model dependent feature
...
\subsection{Local: Explain a single
...
\paragraph{Centered ICE plot}\label{centered-ice-plot}
...
\paragraph{LIME for tabular data}\label{lime-for-tabular-data}
...
\paragraph{LIME for images}\label{lime-for-images}
...
\paragraph{LIME for text}\label{lime-for-text}
