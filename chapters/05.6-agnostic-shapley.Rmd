...
The effect $\phi_j$ of feature $x_{ij}$ is on the prediction $\hat{f}(x_{i\dot})$ is:
...
$$ \phi_{ij} (\hat{f}) = \sum_{S \subseteq \{x_{i1}, \ldots, x_{ip}\} \setminus \{x_{ij}\}} \frac{|S|!\left(p-|S| - 1\right)!}{p!} \left(\hat{f}^*\left(S \cup \{x_{ij}\}\right) - \hat{f}^*(S)\right)$$
...
$\hat{f}^*(S)$ is the prediction for feature values in set $S$ (marginalised over features not in $S$):
$$\hat{f}^*(S) = \int_{x_{i\cdot} \notin S} \hat{f}(x_{i\cdot}) $$
The approximate solution for the Shapley value is: 
$$ \hat{\phi}_{ij} =  \frac{1}{m} \sum_{m=1}^M (\hat{f}(x^{*+j}) - \hat{f}(x^{*-j})) $$
...
After the following excursion about the Shapley value itself, the Estimation of the Shapley value will be explained in more detail.
