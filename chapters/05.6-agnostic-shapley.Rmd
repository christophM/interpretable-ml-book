...
For example someone gives you a Random Forest model trained to predict the value of a flat, which predicts 300.000 Euros for a flat with 50 square meters on the 2nd floor, with a park nearby and having cats in the flat is forbidden. 
This instance is visualised in figure \@ref(fig:shapley-instance).
...
In our flat example from Figure \@ref(fig:shapley-instance), the feature values park-allowed, cat-forbidden, area-$50m^2$ and floor-2nd collaborated, resulting in a 'payout' of 300.000 Euro prediction. 
...
That's important to understand, because a feature can also negatively contribute to a prediction, meaning that its presence reduces the predicted value. 
...
One of these possible subsets is displayed in Figure \@ref(fig:shapley-instance-intervened). 
```{r shapley-instance-intervened, fig.cap = "We want to assess the contribution of the cat-prohibited feature value to a coalition of park-nearby, cat-prohibited, size-$50m^2$. By randomly drawing the value for the floor feature, we simulate that only the park, cat and size feature are in a coalition. Then we predict the value of the flat with this combination (301.000 Euros). In a second step we remove the cat-prohibited feature value from the coalition and replace it with a random value from another flat. Here it was cat-allowed, but note that it could have been cat-allowed again. Again we predict the flat value for this coalition of park-nearby and size-$50m^2$ (320.000 Euros). The contribution of the cat-disallowed feature value was 301.000 Euros - 320.000 Euros = -19.000 Euros. A negative effect. Of course this estimation depends on the sample of the non-participating features, so we should repeat this procedure to get a better estimate."}
