...
A linear model for $\hat{f}$ has following form:
$$\hat{f}(x) = \hat{f}(x_1, \ldots, x_p) = \beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p $$
The effect of feature $x_{\cdotj}$ and instance i would be measured by:
$$ \phi_k = \beta_k x_j - \beta_k E(X_j)$$
where $E(X_k)$ is the mean estimate for feature k.
We also want to compute feature effects in single predictions for non-linear functions with feature interaction.

A solution for computing feature effects for single predictions is the Shapley value $\phi_k$ for feature $x_{\cdotk}$. 
...
where $S$ is a subset of features, $\hat{f}(S)$ is the prediction for feature values in set $S$ (marginalised over features not in $S$), $x_{i\cdot}$ all feature values of observation $i$ and $p$ the number of features. 
...
The Shapley value is the only attribution method that satisfies these properties:
1. Eficiency: $\sum_{j=1}^p \phi_j = val(x_i) =  \hat{f}(x_i) - E_x(\hat{f}(x))$. The feature effects have to sum up to the difference in prediction between prediction for $x_i$ and the models mean prediction.
1.   Symmetry: If $val(S \cup \{x_j^{(i)}\}) = val(S \cup \{x_k^{(i)}\})$ for all $S \subseteq \{x^{(i)}_1, \ldots, x^{(i)}_p\} \setminus \{x_j^{(i)},x_k^{(i)}\}$, then $\phi_j = \phi_k$. The contribution for two features should be the same if they contribute the same to all possible coalitions.
1.  Dummy: If $val(S \cup \{x_j^{(j)}\}) = val(S)$ for all $S \subseteq \{x^{(i)}_1, \ldots, x^{(i)}_p\}$, then $\phi_j = 0$. A feature which does not change the predicted value no matter to which coalition of features it is added should get assigned a contribution of 0.
1.  Additivity: For a game with combined values $val + val'$ the respective Shapley values are $\phi_j + \phi_j'$. The additivity axiom has no relevance in the context of feature effects.
...
They propose a kernel function $K_{x'}(x)$, the Shapley kernel: 
$$K_{x'}(x) = \frac{(p-1)}{{p \choose |S|}|S|(p-|S|)}$$ 
where $p$ is the number of features, $|S|$ is the number of features in $x$ coming from the instance of interest $x'$.  
Based on the kernel, following loss function is defined: $$L(f,g,K_{x'})  = \sum_{x \in X}\left[f(x)) - g(x)\right]^2 K_{x'}(x)$$
