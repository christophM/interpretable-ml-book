...
```{r shapley-instance, fig.cap = "The predicted price for our flat is 300.000€. It's a 50 square meter flat on the second floor. There is a park nearby and cats are not allowed. Our goal is to explain how each of these features values contributed towards the predicted price of 300k€."}
...
The Shapley value (@shapley1953value) is a method for assigning payouts to players depending on their contribution towards the total payout.
...
In our flat example from Figure \@ref(fig:shapley-instance), the feature values 'park-allowed', 'cat-forbidden', 'area-50m^2^' and 'floor-2nd' worked together to achieve the prediction of 300.000€.
...
The answer might be:
The great 'park-nearby' contributed 30.000€ higher value, the size of the flat (50 m^2^) contributed 10.000€ and the fact that cat's are not allowed contributed -50.000€.
The contributions add up to the final prediction minus the average of -10.000€ (the payout).
...
```{r shapley-instance-intervened, fig.cap = "We assess the contribution of the 'cat-forbidden' feature value when added to a coalition of 'park-nearby', 'size-50m^2^'. We simulate that only 'park-nearby', 'cat-forbidden' and 'size-50m^2^' are in a coalition by randomly drawing the value for the floor feature. Then we predict the price of the flat with this combination (301.000€). In a second step we remove 'cat-forbidden' from the coalition by replacing it with a random value of the cat allowed/forbidden feature from the randomly drawn flat. In the example it was 'cat-allowed', but it could have been 'cat-forbidden' again. Again we predict the flat value for this coalition of park-nearby and 'size-50m^2^' (320.000€). The contribution of 'cat-forbidden' was 301.000€ - 320.000€ = -19.000€. This estimation depends on the sampled non-participating feature values, so we should repeat this procedure to get a better estimate. This figure shows the computation of the marginal contribution for only one coalition. The Shapley value is the weighted average of marginal contributions over all coalitions."}
...
The interpretation of the Shapley value $\phi_{ij}$ is: the feature value $x_{ij}$ contributed $\phi_{ij}$ towards the prediction for instance $i$ compared to the average prediction for the dataset.
...
Figure \@ref(fig:shapley-bike-plot) shows the explanations created for the prediction of a RandomForest on the bike rental dataset for an instance for the pure Shapley values see Table \@ref(tab:shapley-bike-table)
...
```{r shapley-bike-plot, fig.cap = sprintf("Feature value contributions for instance %i. With a predicted %.2f number of rented bikes, this day is %.2f below the average prediction of %.2f. The feature values that had the most negative effects were the weather situation, humidity and the time trend (years since 2011). The Temperature on that day had a positive effect compared to the average prediction. The feature contributions sum up to the difference of actual and average prediction (%.2f).", instance_indices[2], actual.prediction, diff.prediction, avg.prediction, diff.prediction)}
=======
...
```{r shapley-bike-plot, fig.cap = sprintf("Feature value contributions for instance %i. With a predicted %.2f rented bikes, this day is %.2f below the average prediction of %.2f. The feature values that had the most negative effects were the weather situation, humidity and the time trend (years since 2011). The temperature on that day had a positive effect compared to the average prediction. The feature contributions sum up to the difference of actual and average prediction (%.2f).", instance_indices[2], actual.prediction, diff.prediction, avg.prediction, diff.prediction)}
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
kable(shapley2$data()[c('feature', 'phi')], caption = sprintf("The Shapley values for data point %i from the cervical cancer dataset.", instance_indices[2]))
```
Ways to misinterpret the Shapley value:
=======
table.dat = shapley2$data()[c('feature', 'phi')]
kable(table.dat, digits = 2,
  caption = sprintf("The Shapley values for data point %i from the cervical cancer dataset.", instance_indices[2]))
```
...
This Section goes deeper into the definition and computation of the Shapley value for the curious reader.
Skip to this part straight to `Advantages and Disadvantages` if you are not interested in the technicalities.
...
$$ \phi_{ij}(\hat{f}) = \beta_j x_{ij} - E(\beta_j X_{\cdot j}) = \beta_j x_{ij} - \beta_j E(X_{\cdot j})$$
where $E_X(\beta_jX_{\cdot j})$ is the mean estimate for feature $X_{j}$.
...
The Shapley value of a feature value $x_{ij}$ is it's contribution in the payed outcome, weighted and summed over all possible feature value combinations:
$$ \phi_{ij} (val) = \sum_{S \subseteq \{x_{i1}, \ldots, x_{ip}\} \setminus \{x_{ij}\}} \frac{|S|!\left(p-|S| - 1\right)!}{p!} \left(val\left(S \cup \{x_{ij}\}\right) - val(S)\right)$$
>>>>>>> Stashed changes
...
The features enter a room in random order.
All features in the room participate in the game (= contribute to the prediction).
The Shapley value $\phi_{ij}$ is the average marginal contribution of feature $j$ by joining whatever features already entered the room before, i.e.
$$\phi_{ij} = \sum_{\text{All orderings}} val(\{\text{features before j}\} \cup x_{ij}) -  val(\{\text{features before j}\})$$
...
#### Estimating the Shapley value
All possible coalitions (sets) of features have to be evaluated, with and without the feature of interest for calculating the Shapley value for one feature
For more than a few features the exact solution to this problem becomes intractable, because the number of possible coalitions increases exponentially by adding more features.
\citep{vstrumbelj2011general} suggest an approximation with Monte-Carlo sampling:
$$ \hat{\phi}_{ij} =  \frac{1}{m} \sum_{m=1}^M \left(\hat{f}(x^{*+j}) - \hat{f}(x^{*-j})\right) $$
where $\hat{f}(x^{* + j}_$ is the prediction for $x_{i\cdot}$, but with a random number of features replaced from a random sample $x$ from your data, excluding the feature value $x_{ij}$
...
Algorithm: Estimating the Shapley value: feature value $x_{ij}$'s contribution towards the difference $\hat{f}(x_{i\cdot}) - \mathbb{E}(\hat{f})$ for instance $x_{i\cdot} \in X$.
>>>>>>> Stashed changes
...
=======
	- $\phi_{ij}^{(k)} = \hat{f}(x_{+j}^*) - \hat{f}(x_{-j}^*)$
- $\phi_{ij}(x) = \sum_{k=1}^m\frac{\phi_j^{(k)}}{m}$
...
The first instance $x_{+j}^*$ is the sample where all values in order before and including feature $j$ are replaced by feature values from the instance of interest.
The second instance $x_{-j}^*$ has all the values in order before, but excluding feature $j$, replaced by features from the instance of interest.
The difference in prediction from the black box is computed: $f(x_{+j}^*) - f(x_{-j}^*)$.
All these differences are averaged and result in $\phi_{ij} = \frac{1}{M}\sum_{k=1}^m\left(f(x_{+j}^*) - f(x_{-j}^*)\right)$
