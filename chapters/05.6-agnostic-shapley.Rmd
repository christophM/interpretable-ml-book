...
- $\phi_{ij}(x) = \sum_{k=1}^m\frac{\phi_{ij}^{(k)}}{m}$
...
But that's not the only way to compute the Shapley value:
...
The exponential number of the coalitions is handled by sampling coalitions and fixing the number of samples $m$  \citep{vstrumbelj2011general}.
...
If $\phi_{ij}$ would be equal to the sum of the main feature effect and all possible interactions, then interactions involving feature $j$ would be fully attributed to each $\phi_{ij}$.
For example, the interaction effect between feature $j$ and feature $k$ would be attributed to both $\phi_{ij}$ and $\phi_{ik}$.
This would violate the efficiency axiom - all effects sum up to the total prediction - because interactions would be counted multiple times.
...
Interaction effects contributing to the prediction are split up evenly among the features.
