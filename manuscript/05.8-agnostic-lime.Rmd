...
The interpretable model can basically be anything from the [interpretable models chapter](#simple), for example [Lasso](#lasso) or a [decision tree](#tree).
The learned model should be a good approximation to the machine learning model predictions locally, but it does not have to be a good global approximation.
...
The lower the K, the easier it is to interpret the model.
...
This varies depending on the type of data, which can be either text, image or tabular data. 
...
LIME samples are not taken around the instance of interest, but from the training data's mass centre, which is problematic.
...
```{r lime-fitting, fig.cap='LIME algorithm for tabular data. A) Random forest predictions given features x1 and x2. Predicted classes: 1 (dark color) or 0 (light color). B) Instance of interest (yellow dot) and data sampled from a normal distribution (black dots). C) Assign higher weight to points near the instance of interest. D) Colors and signs of the grid show the classifications of the locally learned model from the weighted samples. The white line marks the decision boundary (P(class=1) = 0.5).', fig.height=9, fig.width=9}
## Creating dataset ###########################################################
library("dplyr")
library("ggplot2")
...
The black box model is a deep decision tree on the document word matrix.
...
Let us look at the two comments of this dataset and the corresponding classes:
...
For the following image the top 3 predictions were *strawberry*; *candle, taper, wax light*; and *Granny Smith*. 
...
For the second prediction, it is very interesting to see which part of the image contributed to this class. 
We could see that there were objects labeled as *candle, taper, wax light* in the training set that looked shiny like the tomato.
...
IMHO it is the biggest problem with LIME and the reason why I would recommend to use LIME only with great care.
