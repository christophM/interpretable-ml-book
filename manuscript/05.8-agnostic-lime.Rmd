...
It is best to visually explain how the sampling and local model fitting works:
...
The big problem is that we do not have a good way to find the best kernel or the optimal width.
...
```
Let us look at two comments of this dataset and the corresponding classes:
...
That is why I see LIME more in applications where the recipient of the explanation is a lay person or someone with very little time.
It is not sufficient for complete causal attributions, so I do not see LIME in compliance scenarios where you might be legally required to fully explain a prediction.
...
That is just a small complaint, because in the end I see no way around the user having to define the tradeoff between fidelity and sparsity.
...
Instability means that it is hard to trust the explanations, and you should be very critical.
