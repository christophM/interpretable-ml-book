...
Neural feature visualization creates data inputs that maximally active different different units of the neural network.
By that, feature visualization tries to show which (abstract) features the model learned.
...
The feature learning part is especially remarkable for image and text data.
Let's say you want to build an image classifier without relying on deep learning, but using a support vector machine.
The raw pixels are not the best input to your e.g. SVM, so you create new features based on color features, frequency domain features, edge detectors and so on and feed this to your machine learning training.
Before deep learning, you would have extracted features like edges or shapes with specifically designed extractor functions.
With convolutional neural networks, the image is passed into the network in its raw formi (pixels), transformed multiple times through the convolutional layers and the fully connected layers and the network spits out the predictions.
During training, the convolutional neural network learns new features in its layers.
What happens in the hidden layers, when we train a neural network to e.g. classify images?

```{r fig.cap = "Architecture of Inception V1 neural network. Each enumerated unit (3a to 5b) represents a layer with differently sized convolutions and pooling. Figure from Olah, et al. 2019 (CC-BY 4.0) https://distill.pub/2017/activation-atlas/.", out.width = 800}
knitr::include_graphics("images/inceptionv1.svg")
```


- In the first convolutional layer(s) the CNN learns low-level features like edges and simple textures.
- In the later convolutional layers the CNN learns mid-level features like different textures and patterns.
- In the higher up convolutional layers the CNN learns high-level features like objects or parts of objects.
- In the fully connected layers the CNN learns to connect the activations from the high-level features to the individual classes to be predicted.

This looks like the following picture that you might already have seen when learning about neural networks:
...
TODO: Draw image of neuron in feature map, channel in feature map, feature map, class prob.
...
You are looking for a new image $img^*$ that maximize the (mean) activation of a unit, here a single neuron:


$$img^*=\arg\max_{img}h_{n,x,y,z}(img)$$

The function $h$ is the activation of a neuron, x the input to the network (e.g. image), x and y describe the spatial position of the neuron, n enumerates the layers z is the channel index.
There are usually constraints on img, e.g. that only small changes are allowed, or that it comes from the training data distribution.
...
Alternatively, it would also work to maximize random directions, which means that the neurons would be differently weighted.

What the iterative optimization 
