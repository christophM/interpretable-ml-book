...
### LIME for images
LIME for images works different than for tabular data and text.
Variations of the data are created by performing superpixel segmentation and switching superpixels off. A superpixel can be turned OFF by replacing each pixel by the average of the pixels or by a user provided value (e.g., a reasonable value would by gray). 

TODO: check if average is also possible in R package (it's default in python lime)
