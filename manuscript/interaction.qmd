...
Overall, the interaction effects between the features are weak (below 12% of variance explained per feature).

```{r}
#| label: fig-interaction-bike
#| fig-cap: The interaction strength (H-statistic) for each feature with all other features for a support vector machine predicting bike rentals.
...
#| eval: true 
#| fig-cap: The interaction strength (H-statistic) for each feature with all other features for a random forest predicting P(female). 
...
Body mass interacts mostly with bill depth and species.
