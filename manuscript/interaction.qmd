...
$$H^2_{j} = \frac{\sum_{i=1}^n\left[\hat{f}(\mathbf{x}^{(i)}) - PD_j(x^{(i)}_j) - PD_{-j}(\mathbf{x}_{-j}^{(i)})\right]^2}{\sum_{i=1}^n \hat{f}^2(\mathbf{x}^{(i)})}$$
...
@fig-interaction-bike shows the feature interaction H-statistic.
Overall, the interaction effects between the features are very weak (below 10% of variance explained per feature).

```{r}
#| label: fig-interaction-bike
#| fig-cap: The interaction strength (H-statistic) for each feature with all other features for a support vector machine predicting bicycle rentals.
...
With the H-statistic, it's also possible to analyze arbitrary **higher interactions** such as the interaction strength between 3 or more features.
