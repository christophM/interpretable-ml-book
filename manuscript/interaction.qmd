...
pred.bike = Predictor$new(bike_svm, data = bike_test)
...
We analyze the interactions between features in a random forest trained to predict [penguin sex](#penguins), given body measurements, see @fig-interaction-penguins.


```{r}
#| label: fig-interaction-penguins
#| eval: true 
#| fig-cap: The interaction strength (H-statistic) for each feature with all other features for a random forest by species predicting P(female). The body mass has the highest interaction strength.
...
We are interested in how this behaves by species, which I visualize in @fig-interaction-penguins-by-species.

```{r}
#| label: fig-interaction-penguins-by-species
#| eval: true 
#| fig-cap: The interaction strength (H-statistic) for each feature with all other features for a random forest by species predicting P(female). Especially for body mass, the interaction strengths differ between species.
...
Body mass has the strongest interaction, so let's take a deeper look in @fig-interaction2-penguins.

```{r}
#| label: fig-interaction2-penguins
#| eval: true 
#| fig-cap: The 2-way interaction strengths (H-statistic) between body mass and all other features. 

# Due to long running time and timeouts on TravisCI, this has to be run locally. 
# And image has to be added to git repo manually.
ia2 = Interaction$new(pred.penguins, grid.size = 50, feature = "body_mass_g")
plot(ia2)
```
...
So the technique is not useful for image classifiers.
