...
```{r}
penguins$is_adelie = penguins$species == "Adelie"
penguins = penguins[c("is_adelie", "body_mass_g", "bill_length_mm", "bill_depth_mm", "flipper_length_mm", "sex")]
...
![A learner learns a model from training data (features plus target). The model makes predictions for new data.](./images/learner.png){width: 50%}
...
**Cook's distance** (Cook, 1977 [@cook1977detection]) measures the effect of deleting an instance on model predictions.
...
where $\beta$ is the weight vector when the model is trained on all data instances, and $\beta^{(-i)}$ is the weight vector when the model is trained without instance i.
...
In the following example, we train a support vector machine to predict whether a penguin is an [Adelie penguin](#penguins) given body measurements and measure which training instances were most influential overall and for a particular prediction.
Since this is a classification problem, we measure the influence as the difference in predicted probability for penguin species.
...
Measuring influence for all `r nrow(penguins)` training instances requires training the model once on all data and retraining it `r nrow(penguins)` times (= size of training data) with one of the instances removed each time.
...
influence.matrix = matrix(NA, ncol = nrow(penguins), nrow = nrow(penguins))

lrn = makeLearner("classif.svm", predict.type = "prob")
tsk = makeClassifTask(data = penguins, target = "is_adelie")
mod = train(lrn, tsk)
predicted.orig = getPredictionProbabilities(predict(mod, newdata = penguins))
cs = lapply(1:nrow(penguins), function(to.remove.index) {
  mod.2 = train(lrn, tsk, subset = setdiff(1:nrow(penguins), to.remove.index))
  predict.removed = getPredictionProbabilities(predict(mod.2, newdata = penguins))
...
df = data.frame(influence = colMeans(abs(influence.df), na.rm = TRUE), id = 1:nrow(penguins))
...
This does not sound like much, but it's an average over the entire data coming from just removing 1 data point.
...
For this example, I chose a decision tree (following figure) that shows that data from penguins with short bills and short flippers were the most influential for the support vector machine.
Penguins with long bills and a low body mass were the least influential.

```{r}
#| label: cooks-analyzed
#| fig.height: 8 
#| fig.width: 12
#| fig.show: hide
#| eval: false
...
df.penguins= cbind(df, penguins[,setdiff(colnames(penguins), "is_adelie")])
...
![A decision tree that models the relationship between the influence of the instances and their features. The maximum depth of the tree is set to 2.](./images/cooks-analyzed-1.jpeg)
...
I chose to explain the prediction for instance `r i` because for this penguin there were some highly influential other penguins.
...
![Decision tree explaining which instances were most influential in predicting the %i-th instance. Data from penguins with flippers shorter than 179 mm had a large influence on the prediction of the %i-th instance, with an average change in absolute prediction of 1 percentage point of P(Adelie) and one of the penguins even 6 percentage points.](./images/influence-single-1.jpeg)

Penguins with short flippers have a high influence on the prediction for the `r i`-th penguin.
Penguins No. `r i` has a flipper length of `r penguins$flipper_length_mm[i]`, which is just on the edge of 179.
And there are only 7 penguins with a flipper length shorter than 179 mm, which likely strongly influences the shape of the prediction function at the tail.
...
Koh and Liang (2017) [@koh2017understanding] suggested using influence functions, a method of robust statistics, to measure how an instance influences model parameters or predictions.
...
## Strengths of Identifying Influential Instances
...
## Limitations of Identifying Influential Instances
