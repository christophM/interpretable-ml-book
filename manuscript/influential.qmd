...
A learner, that is, the algorithm that generates the machine learning model, is a function that takes training data consisting of features $\mathbf{X}$ and target $\mathbf{y}$ and generates a machine learning model, see also @fig-learner.
...
For this example, I chose a decision tree (@fig-cooks) that shows that data from penguins with short bills and long flippers were the most influential for the support vector machine.
...
How much will the loss change if we upweight a particular instance $\mathbf{z}^{(i)}$ from the training data by a little ($\epsilon$) and downweight the other data instances accordingly?
...
The last expression $\nabla_{\theta}L(\mathbf{z}, \hat{\theta})$ is the loss gradient with respect to the parameters for the upweighted training instance.
...
$$H_{\theta} = \frac{1}{n}\sum_{i=1}^n \nabla^2_{\hat{\theta}}L(\mathbf{z}^{(i)}, \hat{\theta})$$
...
The x-axis shows the value of the $\theta$ parameter and the y-axis the corresponding value of the loss with upweighted instance $\mathbf{z}$.
The model parameter here is 1-dimensional for demonstration purposes, but in reality, it is usually high-dimensional.
We move only $\frac{1}{n}$ into the direction of improvement of the loss for instance $\mathbf{z}$.
We do not know how the loss would really change when we delete $\mathbf{z}$, but with the first and second derivative of the loss, we create this quadratic approximation around our current model parameter and pretend that this is how the real loss would behave.
```{r}
