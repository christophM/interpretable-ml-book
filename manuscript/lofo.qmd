...
We predict bike rentals based on weather and calendar information using a random forest trained on $\frac{2}{3}$ of the data.
...
Because of how LOFO works algorithmically, we now know that removing the holiday feature increases the model performance.
...
LOFO also tells us that we could safely remove the feature `species` and `flipper_length`.
...
However, due to retraining the model, the interpretation shifts from only interpreting that one single model, and more to interpreting the learner and how model training reacts to changes in the features.
...
But LFO and PFI differ in other aspects.
...
LOFO **is useful for feature selection**: A feature with a LOFO importance below zero can be removed to improve model performance; a feature with an importance of zero can also be safely be removed without affecting the performance.
However, be aware that LOFO is subject to randomness of the training process and also depends on the data sample.
Just make sure that you can only remove one feature at a time based on LOFO importance results.
If you want to remove two features, you have to do it sequentially, meaning compute LOFO importance, remove unimportant feature, compute LOFO again, and remove again the least important.
...
Meaning the interpretation is not only based on the model under inspection, but also on other models produced by the machine learning algorithm so that results are to be interpreted in terms of the machine learning algorithm.
...
[^loco]: The first formal description of LOFO I'm aware of is by @lei2018distributionfree. They call it Leave One Covariate Out (LOCO), and the paper is mostly about distribution-free testing. However, due to the approach's simplicity and use in feature selection, it doesn't make sense to attribute it to a single publication.
...
[^not-same-lofo]: The features `temp` and `temp_copy` have similiar but not equal LOFO importances which is due to randomness in the model training.
