...
The LOFO importances of both temperature features are now slightly negative.
...
With that knowledge, let's try LOFO for a random forest predicting penguins species from body measurements.
...
# Compute LOFO feature importance
feature_importances <- data.frame(feature = setdiff(names(penguins), 'species'), importance = 0)
...
However, from the [data chapter](#penguins), we know that flipper length and body mass are highly correlated, so it might be that they actually were important.
If we compare it to their [marginal permutation feature importance](#pfi), we could see that flipper_length_mm is a little bit important to the model.
Again, be aware that these are different ideas of "importance".


```{r}
#| label: fig-lofo-penguins-sex
#| fig-cap: "LOFO importances for the penguin data."
#| eval: false
penguins_rf_sex = randomForest(sex ~ ., data = penguins_train)
test_prediction <- predict(penguins_rf_sex, newdata = penguins_test)
test_accuracy <- mean(penguins_test$sex == test_prediction)

print(test_accuracy)

test_rpart = rpart(sex ~ ., data = penguins_train)
print(mean(penguins_test$sex == predict(test_rpart, penguins_test, type="class")))
