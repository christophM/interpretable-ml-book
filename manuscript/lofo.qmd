...
Because of how LOFO works algorithmically, we now know that removing the feature increases the model performance.
...
However, we want to have it in there, because the model has to be able to distinguish between the species.
...
This distinguishes LOFO from *marginal* feature importance.
...
LOFO **is useful for features selection**: Features with LOFO Importance below zero can be removed to improve model performance.
Just make sure that you can only remove one feature based on LOFO importance results.
If you want to remove two features, you either have to do it sequentially, meaning compute LOFO importance, remove unimportant feature, compute LOFO again, and remove again the least important.
