...
This measurement is called H-statistic, introduced by Friedman and Popescu (2008) [@friedman2008predictive].
...
```{r}
#| label: interaction-bike
#| fig-cap: The interaction strength (H-statistic) for each feature with all other features for a support vector machine predicting bicycle rentals. Overall, the interaction effects between the features are very weak (below 10\\% of variance explained per feature).
data(bike)
library("mlr")
library("iml")
library("ggplot2")
...
We analyze the interactions between features in a random forest trained to predict [penguin species](#penguins), given body measurements.

```{r}
#| label: interaction-penguins-prep
...
ia1 = Interaction$new(pred.penguins, grid.size = 100) 
plot(ia1) +
 scale_y_discrete("")
```

```{r}
#| label: interaction-penguins-include
#| fig-cap: "'The interaction strength (H-statistic) for each feature with all other features for a random forest predicting the probability of penguin species. The feature with the overall the strongest interactions is the bill length.'"
knitr::include_graphics("images/interaction-penguins-1.jpeg")
```
...
Since the bill length has the strongest interactions with other features, we look more deeply into it.


```{r}
#| label: interaction2-penguins-bill
#| eval: false
#| fig-show: "hide"
# Due to long running time and timeouts on TravisCI, this has to be run locally. 
# And image has to be added to git repo manually.
ia2 = Interaction$new(pred.penguins, grid.size = 100, feature = "bill_length_mm") 
plot(ia2)
```

```{r interaction2-penguins-bill-include, fig.cap = 'The 2-way interaction strengths (H-statistic) between bill length and each other feature. There is a strong interaction with body mass, but also with flipper length and bill depth.'}
knitr::include_graphics("images/interaction2-penguins-bill-1.jpeg")
```
...
Variable Interaction Networks (VIN) by Hooker (2004) [@hooker2004discovering] is an approach that decomposes the prediction function into main effects and feature interactions.
The interactions between features are then visualized as a network. 
Unfortunately no software is available yet.

Partial dependence based feature interaction by Greenwell et al. (2018) [@greenwell2018simple] measures the interaction between two features.
