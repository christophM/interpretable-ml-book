...
![Positive (left) and negative (right) activation of Inception V1 neuron 484 from layer mixed4d pre relu. ](./images/a484.png){#fig-activation-maximization}
...
![Iterative optimization from random image to maximizing activation. Olah, et al. 2017 (CC-BY 4.0), https://distill.pub/2017/feature-visualization/.](./images/activation-optim.png){#fig-iterative-maximization}

If you want to dive a lot deeper into feature visualization, take a look at the distill.pub online journal, especially the feature visualization post by Olah et al. (2017) [@olah2017feature], from which I used many of the images.
...
Karpathy et al. (2015) [@karpathy2015visualizing] showed that recurrent neural networks indeed have neurons that learn interpretable features.
...
The Network Dissection approach by Bau & Zhou et al. (2017) [@bau2017network] quantifies the interpretability of a unit of a convolutional neural network.
...
- For each convolutional channel k:
    ...
    - For each image x in the Broden dataset:
        ...
        - Binarize the activation map: A pixel is either on or off, depending on whether or not it exceeds the activation threshold $T_k$. The new mask is $M_k(x)=S_k(x)\geq{}T_k$.
...
@fig-iou-dogo illustrates the intersection and union of the activation mask and concept mask for a single image:
...
@fig-many-dogos shows a unit that detects dogs:

![Activation mask for inception\\_4e channel 750 which detects dogs with $IoU=0.203$. Figure originally from http://netdissect.csail.mit.edu/](./images/dissection-dogs.jpeg){#fig-many-dogos}
...
The combination of both methods allows us to explain an individual classification along with local visualization of the learned features that were involved in the classification.
