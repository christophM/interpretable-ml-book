...
It is unclear to me which of the two results are more desirable.
...
The partial dependence plot shows how the model output changes based on changes of the feature inputs and does not rely on the generalization error.
...
This means that the feature importances do not add up to the total drop in performance, would we shuffle all the features, but they are greater than that. 
Only when there is no interaction between the features, like in a linear model, the importances would roughly add up.
- Permutation feature importance does not require retraining the model like.
...
Let us say you train a sparse linear model (with Lasso) with a fixed amount of features with a non-zero weight.
...
### Disadvantages
- It is very unclear whether you should use training or test data for computing the feature importance.
...
In this case, you would not be interested in how much the model performance drops given the permutation of a feature, but rather how much of the model's output variance is explained by each feature. 
...
If someone only gives you the model and unlabeled data - but not the actual target - you cannot compute the permutation feature importance.
...
The temperature at 9:00 AM does not give me much additional information, when I already know the temperature at 8:00 AM.
...
Even though the importance measure might make sense on the model behavior level, it is damn confusing if you have correlated features.
