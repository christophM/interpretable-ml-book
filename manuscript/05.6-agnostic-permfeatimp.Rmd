...
2. For each feature j = 1,...,p do:
    - Generate feature matrix X~perm~ by permuting feature x~j~ in the data X. This breaks the association between feature x~j~ and true outcome y.
...
Fisher, Rudin, and Dominici (2018) suggest in their paper to split the dataset in half and swap the values of feature j of the two halves instead of permuting feature x~j~. 
This is exactly the same as permuting feature x~j~, if you think about it. 
If you want a more accurate estimate, you can estimate the error of permuting feature x~j~ by pairing each instance with the value of feature x~j~ of each other instance (except with itself). 
...
The mean absolute error (mae)for the training data is `r round(perf1,2)` and for the test data `r round(perf2,2)`, which is also the error of the best possible model that always predicts the mean outcome of 0 (mae of  `r round(mean(abs(y2)),2)`).
...
most_imp = importance$feature[importance$importance == max(importance$importance)]
```

```{r importance-cervical, fig.cap = sprintf("The importance of each of the features for predicting cervical cancer with a random forest. The most important feature was %s. Permuting %s resulted in an increase in 1-AUC by a factor of %.2f", most_imp, most_imp, max(importance$importance))}
plot(importance) +
