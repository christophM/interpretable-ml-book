...
We fit a random forest model to predict [penguin species](#penguins), using 2/3 of the data for training, and 1/3 for computing the importance.
We measure the error as classification error.
Features associated with a model error increase by a factor of 1 (= no change) were not important for predicting penguin species.

```{r importance-penguins-prep}
library('mlr')
library('iml')
...
split = sample(c(TRUE, FALSE), nrow(penguins), replace=TRUE, prob=c(2/3, 1/3))
penguins_train = penguins[split, ]
penguins_test = penguins[!split, ]

# Create task and learner
task = makeClassifTask(data = penguins_train, target = "species")
learner = makeLearner('classif.randomForest', predict.type = 'prob')

# Train model
mod = mlr::train(learner, task)

# Create predictor using the trained model and test data
predictor = Predictor$new(mod, data = penguins_test)
...
```{r importance-penguins, fig.cap = sprintf("The importance of each of the features for predicting penguin species with a random forest. The most important feature was %s. Permuting %s resulted in an increase in classification error by a factor of %.2f", most_imp, most_imp, max(imp.dat$importance))}
plot(importance) +
  scale_x_continuous("Feature importance (classification error)", limits=c(0.5, NA)) +
   scale_y_discrete("")
```

The feature with the highest importance was `r imp.dat[1, 'feature']` associated with an error increase of `r round(imp.dat[1,'importance'], 2)` after permutation from the original error of `r importance$original.error`.
