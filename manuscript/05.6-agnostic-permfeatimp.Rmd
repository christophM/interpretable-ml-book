...
A feature's importance is the increase in the model's prediction error after we permuted the feature's values, which breaks the relationship between the feature and the true outcome. 
...
1. Estimate the original model error e_orig = L(y, f(X))  (e.g. mean squared error)
2. For each feature j = 1,...,p do:
    - Generate feature matrix X_permj by permuting feature j in the data X. This breaks the association between feature j and true outcome y.
    - Estimate error e_perm = L(Y,f(X_permj)) based on the predictions of the permuted data.
    - Calculate permutation feature importance FI_j= e_permj/e_orig. Alternatively, the difference can be used: FI_j = e_permj - e_orig
...
Permutation feature importance **does not require retraining of the model**.
...
Another example:
The model is a decision tree and we analyze the importance of the feature that was chose as the first split.
You remove the feature and retrain the model. 
...
In this case, you would not be interested in how much the model performance decreases when a feature is permutated, but how much of the model's output variance is explained by each feature. 
