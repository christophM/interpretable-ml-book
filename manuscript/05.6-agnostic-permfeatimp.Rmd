...
### Should I compute feature importance on the training or test data? {#feature-importance-data}
...
tl;dr: I don't have a definite answer, both ways might be correct.  
Answering the question of training or test data touches on the fundamental question what feature importance means.
The best way to understand the difference between feature importance based on training and based on test data is an "extreme" example.
I trained a support vector machine to predict a continuous, random target outcome given 50 random features (200 observations).
With "random" I mean that the target outcome is independent of the 50 features.
This is like predicting tomorrows temperature given the latest lottery numbers.
If the model "learns" any relationships, then it overfits.
And, indeed, it did overfit the training data.
The mean absolute error for the training data is `r round(perf1,2)` and for the test data `r round(perf2,2)`, which is en par with the best possible model, which always predicts the mean outcome of 0 (mae  `r round(mean(abs(y2)),2)`).
In other words, the SVM model is garbage.
What feature importance values would you expect for the 50 features of this overfitted SVM?
Zero, because none of the features contributes to an improved performance on unseen test data?
Or should the importances reflect how much the model relies on each of the features, regardless whether the learned relationships generalize to unseen data?
Let's have a look at how the distributions of feature importances differ for training and test data.
...
To me it's unclear which of the both results are more desirable.
So I will try to make the case for both versions and let you decide what you think.

**The case for test data**

That's an easy case to make: Model error estimates based on training data are garbage -> feature importance relies on model error estimates -> feature importance based on training data is garbage.  
Really, it's one of the first things you learn in machine learning:
When you measure the model error (or performance) on the same data the model was trained on, the measure will usually be over-optimistic, meaning that the model seems to work much better than it actually would on unseen test data.
And since the permutation feature importance requires to measure the model error, we should use unseen test data.
The feature importance based on training data falsely makes us believe that feature are important for the predictions, when in reality the model was just overfitting and the features weren't important at all.
<!-- The "original" feature importance was introduced for random forests. -->
...
The arguments for using training data are a bit more difficult to make, but are IMHO as compelling as the arguments for using test data.
We look again at our garbage SVM.
Based on the training data, the feature `r max.imp$feature` was the most important one.
Let's look at a partial dependence plot of feature `r max.imp$feature`.
The partial dependence plot shows how the model output changes based on the feature inputs and doesn't rely on the generalization error.
It doesn't matter if the PDP is computed with training or test data.

```{r garbage-svm-pdp, fig.cap=sprintf("The partial dependence of feature %s, which is the most important feature according to the feature importance based on the training data. The plot shows that the SVM relies on this feature for making predictions", max.imp$feature)}
pdp = FeatureEffect$new(pred2, max.imp$feature, method = "pdp")
pdp$plot()
```

The plot shows clearly that the SVM has learned to rely on feature `r max.imp$feature` for its predictions, yet the feature importance based on the test data is `r round(imp2$results[imp2$results$feature == max.imp$feature, "importance"], 2)`.
Based on the training data, the feature importance is `r round(max.imp$importance, 2)`, reflecting that the model learned to use this feature.
Feature importance based on the training data tells us which features are important for the model in the sense that it relies on them for making predictions.

As part of the case for using training data, I want to introduce an argument against test data.
In practice, you want to use all of your data to train your model to get the best possible model in the end.
...
If you would use (nested) cross-validation for the feature importance, you would have the problem that the feature importance is not computed on the final model with all the data, but on a model with a subset of the data, which might behave differently.

In the end, you must decide whether you want to know how much the model relies on each feature for making predictions (-> training data) or whether you want to know how much the feature contributes to the models performance on unseen data (-> test data).
Research addressing the training vs. test data question is non-existent to my best knowledge.
It will need more rigorous examination than my single "garbage-SVM" example.
...
I based the importance computation on the training data.
