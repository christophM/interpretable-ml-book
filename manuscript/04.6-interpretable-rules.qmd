...
rule = OneR::OneR(species ~ ., data = penguins)
...
The flipper length feature (`flipper_lengh_mm`) was chosen by OneR as the best predictive feature.
To get a better feel for how this one rule performs, let's look into the cross table between the (discretized) flipper length feature and the penguin species:
...
tt <- penguins %>%
  mutate(flipper_length_bin = bin(penguins$flipper_length_mm)) %>%
  group_by(flipper_length_bin, species) %>%
  summarise(count = n()) %>%
  group_by(flipper_length_bin) %>%
  mutate(percentage = count / sum(count) * 100) %>%
  ungroup() %>%
  mutate(display = sprintf("%.1f%% (%d)", percentage, count)) %>%
  pivot_wider(id_cols = flipper_length_bin, 
              names_from = species, 
              values_from = display, 
              values_fill = "0% (0)")

# Rename the first column
colnames(tt)[1] <- "flipper_length_mm"
...
We can see that this feature is particularly useful for classifying penguins with very short and very long flippers.
With a flipper length above 219 we can be sure it is a Gentoo and between 207 and 219 it is at least likely. Below 184 mm, it's likely (90%) to be the Adelie species.
However, in between these extremes, relying on flipper length alone doesn't give us as much clarity about the species.
...
```
First we extract the rules for the [penguin species classification task](#penguins).
Here we get these 5 rules plus the default rule.

```{r}
#| label: jrip-penguins
#| include: true
#| eval: true
rule = JRip(species ~ . - island, data = penguins)
...
penguins2$island = NULL
penguins2$label = ifelse(penguins$species == "Adelie", "Adelie", "Gentoo/Chinstrap")
penguins2$species= NULL
rules = sbrl(penguins2, pos_sign = "Adelie", neg_sign = "Gentoo/Chinstrap", rule_maxlen = 2)
rn = rules$rulenames
