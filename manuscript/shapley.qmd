...
![The predicted price for a 50 $m^2$ 2nd floor apartment with a nearby park and cat ban is €300,000. Our goal is to explain how each of these feature values contributed to the prediction.](./images/shapley-instance.png){#shapley-instance}
...
```{r}
#| label: shapley-instance-intervened
...
![All 8 coalitions needed for computing the exact Shapley value of the `cat-banned` feature value](./images/shapley-coalitions.jpg){#fig-shapley-coalitions width=80%}
...
#| fig-cap: !expr sprintf("Shapley values for penguin male vs female classification. This penguin's predicted P(female)=%.2f is %.2f below the average probability for P(female) of %.2f. The bill length contributed most to the probability of being female, but most factors were (correctly) pointing to male. The sum of the contributions yields the difference between actual and average prediction. (%.2f).", actual.prediction,diff.prediction, avg.prediction, avg.prediction)
...
Consider using a smaller sample size $M$ of data points when estimating Shapley values to reduce computation time but beware of increased variance in the estimate. 
...
Pair Shapley values with ceteris paribus or ICE plots to get a sense of local sensitiviy to feature changes. 
