...
```{r}
#| label: shapley-penguins-plot
#| fig-cap: !expr sprintf("Shapley values for penguin male vs female classification. This penguin's predicted P(female)=%.2f is %.2f above the average probability for P(female) of %.2f. The bill length contributed most to the probability of being female, but most factors where (correctly) pointing to male. The sum of the contributions yields the difference between actual and average prediction. (%.2f).", actual.prediction,diff.prediction, avg.prediction, avg.prediction)
...
Shapley values **are not local in the sense of gradients.** [@bilodeau2024impossibility]
...
The resulting values are no longer the Shapley values to our game, since they violate the symmetry axiom, as found out by Sundararajan et al. (2019) [@sundararajan2020many] and further discussed by Janzing et al. (2020) [@janzing2020feature].
