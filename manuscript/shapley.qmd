...
![The predicted price for a 50 $m^2$ 2nd floor apartment with a nearby park and cat ban is €300,000. ](./images/shapley-instance.png){#fig-shapley-instance}
...
The Shapley value, coined by Shapley (1953) [@shapley1953value], is a method for assigning payouts to players depending on their contribution to the total payout.
...
We simulate that only `park-nearby`, `cat-banned` and `area-50` are in a coalition by randomly drawing another apartment from the data and using its value for the floor feature, see @fig-shapley-instance-intervened.
...
![One sample repetition to estimate the contribution of `cat-banned` to the prediction when added to the coalition of `park-nearby` and `area-50`.](./images/shapley-instance-intervention.png){#fig-shapley-instance-intervened}
...
See @fig-shapley-coalitions for all possible coalitions `cat-banned` can be added to.
...
Figure  @fig-shapley-penguins-plot shows the Shapley values for a male penguin.
...
#| fig-cap: Shapley values for penguin male vs female classification.
...
When comparing our penguin with other `r x.interest$species` penguins, then the reason why it has a low probability of being female is its weight.
...
Shapley value interpretation is always in reference to the dataset that was used for replacing missing values.
...
#| fig-cap: sprintf("Shapley values for day %i of the bike data.", i)
...
The contribution $\phi_j$ of the $j$-th feature on the prediction $\hat{f}(\mathbf{x})$ is:

$$\phi_j(\hat{f})=\beta_{j}x_j-\mathbb{E}[\beta_{j}X_{j}]=\beta_{j}x_j-\beta_{j}\mathbb{E}[X_{j}]$$
...
$$val_{\mathbf{x}}(S)=\int\hat{f}(x_{1},\ldots,x_{p})d\mathbb{P}_{X_C}-\mathbb{E}[ \hat{f}(X)]$$
...
$$val_{\mathbf{x}}(S)=val_{\mathbf{x}}(\{1,3\})=\int_{\mathbb{R}}\int_{\mathbb{R}}\hat{f}(x_{1},X_{2},x_{3},X_{4})d\mathbb{P}_{X_2X_4}-\mathbb{E}[ \hat{f}(X)]$$
...
$$\sum\nolimits_{j=1}^p\phi_j=\hat{f}(\mathbf{x})-\mathbb{E}[ \hat{f}(X)]$$
...
Strumbelj et al. (2014) [@strumbelj2014explaining] propose an approximation with Monte-Carlo sampling:
...
First, select an instance of interest $\mathbf{x}$, a feature $j$ and the number of iterations $M$.
For each iteration, a random instance $\mathbf{z}$ is selected from the data and a random order of the features is generated.
Two new instances are created by combining values from the instance of interest $\mathbf{x}$ and the sample $\mathbf{z}$.
The instance $\mathbf{x}_{+j}$ is the instance of interest, but all values in the order after feature $j$ are replaced by feature values from the sample $\mathbf{z}$.
The instance $\mathbf{x}_{-j}$ is the same as $\mathbf{x}_{+j}$, but in addition has feature $j$ replaced by the value for feature $j$ from the sample $\mathbf{z}$.
The difference in the prediction from the black box is computed:

$$\phi_j^{(m)}=\hat{f}(\mathbf{x}^{(m)}_{+j})-\hat{f}(\mathbf{x}^{(m)}_{-j})$$
...
In situations where the law requires explainability -- like the EU's "right to explanations" -- the Shapley value might be the only legally compliant method, because it is based on a solid theory and distributes the effects fairly.
I'm not a lawyer, so this reflects only my intuition about the requirements.
...
That's why I would also recommend pairing Shapley values with, for example, [ceteris paribus plots](#ceteris) or [ICE plots](#ice), so you get the full picture.
...
Another solution is [SHAP](https://github.com/slundberg/shap) introduced by Lundberg and Lee (2016)[@lundberg2017unified], which is based on the Shapley value, but can also provide explanations with few features.
...
It's not sufficient to access the prediction function because you need the data to replace parts of the instance of interest with values from randomly drawn instances of the data.
