...
$$\text{explanation}(\mathbf{x}) = \arg\min_{g \in G} L(f,g,\pi_{\mathbf{x}}) + \Omega(g)$$
...
#| tbl-cap: Sample of comments from the YouTube Spam dataset.
library(kableExtra)
load("../data/ycomments.RData")
tab = kableExtra::kbl(ycomments[1:10, c('CONTENT', 'CLASS')], booktabs = TRUE)
tab %>% column_spec(1, width = "24em") %>% column_spec(2, width="3em")
```
