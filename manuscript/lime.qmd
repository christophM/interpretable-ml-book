...
It's best to visually explain how sampling and local model training works:
...
#| fig-cap: LIME explanations for two instances of the bike rental dataset. Warmer temperature and good weather situation have a positive effect on the prediction. The x-axis shows the feature effect, which is the weight times the actual feature value.
...
Let's look at the two comments of this dataset and the corresponding classes (1 for spam, 0 for normal comment):

```{r}
#| label: load-text-classification-lime
data("ycomments")
...
```{r}
#| label: show--data-TubeSpam
kableExtra::kbl(ycomments[example_indices, c('CONTENT', 'CLASS')], booktabs = TRUE)  %>% kable_styling(position = "center")
...
```{r}
#| label: lime-text-variations-output
#| results: asis
kableExtra::kbl(variations, row.names = FALSE, booktabs = TRUE)  %>% kable_styling(position = "center")
...
kableExtra::kbl(explanations[c("case", "label_prob", "feature", "feature_weight")], booktabs = TRUE)  %>% kable_styling(position = "center")
...
The image used shows some bread I baked that is in a bowl.
