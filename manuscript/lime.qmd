...
Local interpretable model-agnostic explanations (LIME) [@ribeiro2016why] is a paper in which the authors propose a concrete implementation of local surrogate models.
...
```{r}
#| label: lime-tabular-example-train-black-box
data("bike")
ntree = 100
bike.train.resid = factor(resid(lm(cnt ~ cnt_2d_bfr, data = bike)) > 0, levels = c(FALSE, TRUE), labels = c('below', 'above'))
...
The results of the sparse local linear models trained for two instances with different predicted classes is shown in @fig-lime-tabular-example-explain-plot-1.
...
Let's look at the two comments of this dataset and the corresponding classes (1 for spam, 0 for normal comment) in @tbl-show-data-TubeSpam.
...
cap = "Two examples of comments."
kableExtra::kbl(ycomments[example_indices, c('CONTENT', 'CLASS')], booktabs = TRUE, caption=cap)  %>% kable_styling(position = "center")
```
...
#| results: asis
cap = "Variations of one of the youtube comments along with their weights (based on distance) and predicted probability of being spam."
kableExtra::kbl(variations, row.names = FALSE, booktabs = TRUE, caption=cap)  %>% kable_styling(position = "center")
```
...
cap = "LIME explanations for text classification."
kableExtra::kbl(explanations[c("case", "label_prob", "feature", "feature_weight")], booktabs = TRUE, caption=cap)  %>% kable_styling(position = "center")
```
...
![Left: Image of a bowl of bread. Middle and right: LIME explanations for the top 2 classes (bagel, strawberry) for image classification made by Google's Inception V3 neural network.](images/lime-images-package-example-1.png){#fig-lime-images-package-example-include}
...
It's not sufficient for complete attributions, so I do not see LIME in compliance scenarios where you might be legally required to fully explain a prediction.
...
In my opinion, it is the biggest problem with LIME and the reason why I would recommend using LIME only with great care.
