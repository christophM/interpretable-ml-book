...
Let us map out the territory a bit:
...
The methods have in common that the output that has the same size as the input image (or can at least be projected meaningfully onto it) and give each pixel a value which somehow can be interpreted as the relevance of the pixel for the prediction or classification of that image.

Another useful category for pixel attribution methods is the question of baseline:

1. Gradient only: The gradient just tells us whether a change of this pixel (increasing color) would change the prediction. Examples are Vanilla Gradient and Grad-CAM.
...
1. Path attribution: The path attribution compares the current image with a reference image (which can be some artificial "Null" image) and distributes the difference of the prediction for both among the pixels. This category contains both model-specific, gradient-based methods like Deep Taylor and Integrated Gradients, but also model-agnostic methods like LIME and SHAP.
Some path attribution methods are "complete", meaning that the sum of the relevance values for all input features sum up to the difference between the prediction of the image/data point minus the prediction of a reference point (e.g. all grey image).
Examples are SHAP and Integrated Gradients.
For the path-attribution, the interpretation is always with respect to the baseline:
...
Add this point, I would usually give some intuition about how these methods work, but I think it is best if we just start with the Vanilla Gradient method, because it shows quite clearly the general recipe that many other methods follow.
...
1. Visualize the gradients. Either absolute values or by highlighting negative / positive contributions.
...
But the motivation is that we can approximate that score by applying a first-order Taylor expansion
...
Here, $\mathbf{I}$ is the element-wise indicator function which is 0 where the activation at the lower layer was negative or zero, and 1 where it is positive.
Or, in words, we take the gradient that we have back-propagated so far until the layer n+1, then simply set the gradients to zero where the activation at the layer below is negative.
...
Vanilla Gradient has Saturation Problem (explained in Avanti et. al, 2017 [^deeplift]):
...
For ReLU ($X_{n+1}=max(X_n,0)$) the vanilla gradient computes: $\delta{}f/\delta{}X_n=\delta{}f/\delta{}X_{n+1}\mathbf{I}(X_n>0)$, where $\mathbf{I}$ is an (element-wise) indicator function.
For DeconvNet ReLU, this is instead: $R_n=R_{n+1}\mathbb{I}(R_{n+1}>0)$, where $R_n$ and $R_{n+1}$ are the layer reconstructions.
...
In DeConvNet, activation with negative value in layer x are set to zero in layer n-1.
Here layers are just the activations before and after ReLU.
Guided Backpropagation sets both to 0 in layer n-1: neurons that in the forward pass were 0 in layer x and neurons that are negative in the backward passe in layer n.
...
As a reminder, the first convolutional layer of a CNN takes as input the images and outputs feature maps, which encode learned features (see [Learned Features](#cnn-features)).
...
Since we are only interest in our chosen class, the Grad-CAM authors suggest to look only at the positive gradients.
...
Other attribution techniques backpropagate the entire way back to the image and each of the pixels.
...
There are approaches to investigate how robust and correct the explanations are.
They give us some hints that the explanations sometimes are even independent of network and data, which is really bad.
