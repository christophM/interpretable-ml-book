...
The SVM predicts that the person has a good credit risk with a probability of 24.2\%.
...
Counterfactuals are **not so useful for model and data insights**, since insights from counterfactual are specific to one instance and one counterfactual prediction.
