...
$$o_1(\hat{f}(\mathbf{x'}),y')=\begin{cases}0&\text{if $\hat{f}(\mathbf{x}^{\prime) \in y^{\prime}$} \\ \inf\limits_{y^{\prime} \in y^{\prime}}|\hat{f}(\mathbf{x}^{\prime})-y^{\prime}| & \text{else} \end{cases}$$
