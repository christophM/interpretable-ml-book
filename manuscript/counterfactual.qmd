...
Counterfactuals (the IML method) alone don't support any **causal** claims about the real world.
...
For instance, continuing our loan example, one counterfactual explanation might suggest only to double the income to get a loan, while another counterfactual might suggest shifting to a nearby city and increasing the income by a small amount to get a loan.
...
In the following, we focus on two of them: first, the one by Wachter et al. (2017) [@wachter2018counterfactual], who introduced counterfactual explanation as an interpretation method and, second, the one by Dandl et al. (2020) [@dandl2020multiobjective] that takes into account all four criteria mentioned above.
...
Let's now have a look on another approach overcoming these issues.
...
$$o_1(\hat{f}(\mathbf{x'}),y')=\begin{cases}0&\text{if $\hat{f}(x')\in{}y'$}\\\inf\limits_{y'\in y'}|\hat{f}(x')-y'|&\text{else}\end{cases}$$
...
The fourth objective $o_4$ reflects that our counterfactuals should have likely feature values/combinations. We can infer how "likely" a data point is using the training data or another dataset. We denote this dataset as $\mathbf{X}^{\text{obs}}$. As an approximation for the likelihood, $o_4$ measures the average Gower distance between $\mathbf{x'}$ and the nearest observed data point $x^{[1]}\in \mathbf{X}^{\text{obs}}$:
...
Sticking with above's credit example, one counterfactual could suggest increasing the income by €30,000 while another one proposes to have no default in the last five years and a reduction in age by ten.
All other feature values are equal to the values of x. Each candidate is then evaluated using the four objective functions of above.
...
|age|sex|job |amount|dur.|$o_2$|$o_3$|$o_4$|$\hat{f}(\mathbf{x}^{\prime})$|
|---|---|----|-------|---|-----|-|-----|----|
...
Karimi et al. (2020) [@karimi2020modelagnostic] also provided a Python implementation of their algorithm MACE in a [GitHub repository](https://github.com/amirhk/mace).
...
Mothilal et al. (2020) [@mothilal2020explaining] developed [DiCE (Diverse Counterfactual Explanation)](https://github.com/interpretml/DiCE) to generate a diverse set of counterfactual explanations based on determinantal point processes.
DiCE implements both a model-agnostic and a gradient-based method.

Another way to search counterfactuals is the Growing Spheres algorithm by Laugel et al. (2017) [@laugel2017inverse].
...
Anchors by Ribeiro et al. (2018) [@ribeiro2018anchors] are the opposite of counterfactuals; see the chapter about [Scoped Rules (Anchors)](#anchors).
