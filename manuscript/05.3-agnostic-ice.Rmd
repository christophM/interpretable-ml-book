...
Let's go back to the [penguin classification task](#penguins) and see how the prediction of each instance is associated with the feature "bill_length_mm".
We will analyze a random forest that predicts the probability for different species given bodily measurements.
In [partial dependence plot](#pdp) we have seen that a larger bill length yields a lower P(Adelie).
But that was an average effect, now we can use ICE to get a look at individual instances.

```{r ice-penguins, fig.cap="ICE plot of penguins species probability by bill length. Each line represents one penguin. We can almost see two types of penguins. Some start out with a high probability of being Adelie at a low bill length, and another segments starting out with an around 25% probability."}
library("mlr")
library("ggplot2")
...
The plot shows that the effect is not the same for each penguin.
...
For example, take the Adelie penguin ICE plot for body mass and center the lines on the lightest penguin:
...
```{r ice-bike-centered, fig.cap='Centered ICE plots of predicted number of bikes by weather condition. The lines show the difference in prediction compared to the prediction with the respective feature value at its observed minimum.', fig.height=3.3}
data(bike)
set.seed(43)
