...
A model is created by a learner, which learns the mdoel from the training data.
...
A learner, which is the algorithm that produces the machine learning model is - in its essence - a function that takes in training data consisting of features X and the target y and spits out a model, which can be described by its parameters.
...
When facing a problem, we should always look if other people have already solved the same kind of problem. 
In this case, statistician have already worked with deletion diagnostics.
Identfiying influential instances is a procedure used for researching the robustness of a model and also to maybe remove these instance from the training data.
...
save(influence.df, file = influence.matrix.filename)
```
...
rpart(influence ~ . -id, data = df.cervical)
ct = ctree(influence ~ . -id, data = df.cervical)
...
ct = ctree(influence ~ ., cervical.200)
plot(ct, inner_panel = node_inner(ct, pval = FALSE), type='simple')
