...
In the following example, we train a support vector machine to predict whether a penguin is an [Adelie penguin](#penguins) given bodily measurements and measure which training instances were most influential overall and for a particular prediction.
...
The measurement of the influence for all `r nrow(penguins)` training instances requires to train the model once on all data and retrain it `r nrow(penguins)` times (= size of training data) with one of the instances removed each time.
...
An influence of `r sprintf('%.2f', abs(df[1,"influence"]))` means that if we remove the `r df$id[1]`-th instance, the predicted probability changes by `r sprintf('%.1f', 100 * df[1,"influence"])` percentage point on average.
This is does not sound like much, but it's an average over the entire data coming from just removing 1 data point.
From that perspective, I think it is quite substantial.
...
```{r cooks-analyzed, eval=FALSE, fig.height=8, fig.width=12, fig.show='hide'}
...
```{r cooks-analyzed-include, fig.cap = "A decision tree that models the relationship between the influence of the instances and their features. The maximum depth of the tree is set to 2."}
knitr::include_graphics("images/cooks-analyzed-1.jpeg")
```
...
```{r influence-single, eval = FALSE, fig.height=8, fig.width=12, fig.show='hide'}
...
```{r influence-single-include, fig.cap = sprintf("Decision tree that explains which instances were most influential for predicting the %i-th instance. Data from penguins with fippers shorter than 179 mm had a large influence on the prediction of the %i-th instance, with an average change in absolute prediction by 1 percentage points of P(Adelie) and one of the penguins even 6 percentage points.", i, i)}
knitr::include_graphics("images/influence-single-1.jpeg")
```

Penguins with short flippers have a high influence on the prediction for the `r i`-th penguin.
Penguins No. `r i` has a flipper length of `r penguins$flipper_length_mm[i]`, which is right on the edge of 179.
And there are only 7 penguins with a flipper length shorter than 179 mm, which likely influences strongly how the prediction function is shaped at the tail.
