...
### Identifying Influential Instances with Cook's Distance and DFBETA  
...
This requires to refit the random forest `r nrow(bike)` times, because that's the size of the training data.
...
#'@param n.features The number of features
#'@param errors The vector of model errors
...
lrn = makeLearner("classif.ranger", predict.type = "prob", num.trees  = 10)
...
cs = lapply(1:nrow(cervical), function(to.remove.index) {
  if(to.remove.index %% 50 == 0) print(to.remove.index)
  ...
  error = (cervical$Biopsy == "Cancer") - predicted.orig
  cooks(predicted.orig, predict.removed, n.features = ncol(cervical) - 1, errors = error)
})

df = data.frame(cooks = unlist(cs), id = 1:nrow(cervical))

df = df[order(df$cooks, decreasing = TRUE),]
kable(cervical[df$id[1],])
```

The following table shows the instance with the highest Cook's Distance of `r df[1,"cooks"]`. 
The mean of Cook's Distances is `r mean(df$cooks)`.
...
rpart(cooks ~ . -id, data = df.cervical, control = rpart.control(maxdepth = 2))
```
