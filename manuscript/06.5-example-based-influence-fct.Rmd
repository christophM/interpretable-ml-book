...
By identifying influential training instances, we can 'debug' machine learning models and better explain their behavior and predictions.
...
Robust statistics also provides methods to measure how robust estimates are from data (such as a mean estimate or the weights of a prediction model).
...
With the deletion approach, we would recalculate the mean value ten times omitting one of the income statements each time and measuring how much the mean estimate changes. 
A big change means that an instance was very influential.
The second approach upweights one of the persons by a small weight, which corresponds to the calculation of the first derivative of a statistic or model.
...
A more robust choice is the median (the value at which one half of people earn more and the other half less), because even if the person owith the heighest income in your sample would earn ten times more, the resulting median would not change.

Deletions diagnostics and influence functions can also be applied to the parameters or predictions of machine learning models to understand their behavior better or to explain individiual predictions.
Before we look atthese two approaches for finding influential instances, we will examine the difference between an outlier and an influential instance.

**Outlier**

An outlier is an instance that is far away from the other instances in the dataset.
"Far away" means that the distance, for example the Euclidean distance, to all the other instances is very great.
In a dataset of newborn body measurements, a newborn weighing 6 kg would be considered an outlier.
In a dataset of bank accounts where most are checking accounts, a dedicated loan account (large negative balance, few transactions) would be considered an outlier.
...
Outliers can be interesting data points (like [criticisms or prototypes](#proto)).
Outliers can influence the model training, but don't have to. 
...
The more the model parameters or predictions of a model change when the model is retrained with an instance removed from the training data, the more influential that instance is. Whether an instance is influential for a trained model also depends on its value for the target y.
...
The key idea behind influential instances for interpretability is to trace predictions back to where it all began: the training data.
...
With influential instances, we don't treat the model as fixed, but as a function of the training features and the target
...
Cook's distance tells us how much the predicted output of a linear model changes when we remove the i-the instance in training.
...
This expression is basically the numerator of Cook's distance, with the difference that the absolute difference is added up instead of the differences.
This was a choice I made, because it makes sense for the examples later.
The general form of deletion diagnostic measures consists of choosing a measure (such as the sum of the absolute differences) and calculating the difference of the measure for the model trained on all instances and when the influential instance is deleted.
...
In the following example, we train a support vector machine to predict [cervical cancer](#cervical) given risk factors and measure which training instances were most influential overall and for a given prediction.
...
The measurement of the influence for all `r nrow(cervical)` training instances of cervical cancer training data requires to train the model once on all training data and retrain it `r nrow(cervical)` times (= size of training data) with one of the instances removed each time.
...
An influence of `r sprintf('%.2f', abs(df[1,"influence"]))` means that if we remove the `r df$id[1]`-th instance, the predicted probability changes by `r sprintf('%.1f', 100 * df[1,"influence"])` percentage points on average.
...
All methods that return instances as output only make sense if we have a good way of representing an instance.
But we can better understand what kinds of instances are influential when we ask:
What distinguishes an influential instance from non-influential instances?
...
This means that errors in the data of these instances can have a strong effect on the model.
...
Data instance of women smoked or have been smoking for 19 years or longer have a high influence on the prediction of the `r i`-th instance.
...
The most extreme change in the prediction happens when we remove the `r worst.case.index`-th instance.
...
The predicted probability for the `r i`-th instance changes from `r sprintf('%.2f', 100 * predicted.orig[i])` percentage points to `r sprintf('%.2f', 100 * (predicted.orig[i]  - cervical.200$influence[worst.case.index]))` percentage points if we remove the `r worst.case.index`-th instance!
...
The whole retraining can be quete slow, because if you have thousands of training samples, you will have to retrain your model thousands of times.
Assuming the model takes one day to train and you have 1000 training examples, then the computation of influential instances - without parallelization - will take almost 3 years.
...
*You*: I want to know influence a training instance has on a particular prediction.  
...
Koh and Liang (2017)[^koh] suggested using influence functions, a method of robust statistics, to measure how an instance influences model parameter or a prediction.
...
However, instead of deleting training instances, the method approximates how much the model changes when the instance is upweighted.
...
How much will the loss change if we upweight a particular instance $z_i$ from the training data by a little ($\epsilon$) and downweight the other data instance accordingly?
...
We approximate the function locally by using information about the steepness (= gradient) and the curviness (= Hessian matrix) at the current model parameter setting.
...
The approximate parameter vector is basically the original parameter minus the gradient of the loss of z (because we want to decrease the loss) scaled by the curvature (= multiplied by the inverese Hessian matrix) and scaled by 1 over n, because that's the weight of a single training instance.
...
The formula for $I_{\text{up,loss}}$ states that the influence function of the training instance z on the prediction of an instance $z_{test}$ is "how strongly the instance reacts to a change of the model parameter" times "how much the parameters change when we upweight the instance z".
...
Influence functions have many applications, some of which have already presented in this chapter.
...
For the neural network, even one image of a dog image was among the most influential images, showing that it learned the concepts and not the Euclidean distance in color space.
...
Handling domain mismatch is closely related to a better understanding the model behavior.
...
You take one of the new patient, for whom the model has made a false prediction and find and analyse the most influential instances.
...
The conclusion would be that the model neeeds to be trained on more patients who are older in order to work well in the second hospital.
...
But checking patient information and correcting it can be very time-consuming, because once you have reported which patients you need to check, the hospital actually needs to send someone to look at the patients records more closely, which can be handwritten and checking data for a patient can take an hour or more.
...
TODO: CONTINUE HERE WITH DEEPL


### Advantages of Identifying Influential Instances

- The deletion diagnostics and influence functions approaches are very different from the mostly feature-perturbation based approaches presented in the [model-agnostic chapter](#agnostic). 
Looking at influential instances emphasizes the role the training data plays in the learning process.
This makes influence function and deletion diagnostics **one of the best machine learning debugging tools**.
...
But, really, **the approach is generalizable** to 'what happens to ... when we delete/upweight instance z?', where you can fill '...' with any function of your model of your desire. 
...
When your model takes an hour to train and you have 10,000 instances, then it will take longer than a year to sequentially compute the influence of all instance by leaving them out one after another from the training data.
...
You can train models with thousands of training instances and hundreds of parameters within seconds/minutes on a laptop. 
So it's not a big leap to speculate that deletion diagnostics will even work with big neural networks easily in 10 years.
...
But for the latter problem, there is a trick:
use a differentiable loss as replacement for the calculation of the influence, for example when the underlying model uses Hinge loss instead of some differentiable loss.
The loss is replaced with a smoothed version of the problematic loss for the influence functions, but the model can still be trained with the non-smooth loss.
...
But there is no guarantee that the approximation is always so close.
- A minor issue, but there is **no clear cutoff at which we call an influence of an instance influential or non-influential**.
It's still useful to rank the instances by influence, but it would be great to have the means to not only rank the instances, but to actually distinguish between influential and non-influential.
For example, when you identify the top 10 most influential training instance for a test instance some of them might not be influential, because for example only the top 3 were really influential.
- The influence measures **only consider deletions of single instances** and not deletion of multiple instances at once.
...
And you certainly don't want to start going any further, because there are just too many combinations.
...
The code is focused on the Tensorflow library, so you can't directly use it for black box models using other frameworks, like sci-kit learn.
- Here is a [great blog post for influence functions](http://mlexplained.com/2018/06/01/paper-dissected-understanding-black-box-predictions-via-influence-functions/#more-641), which helped me understand the paper from Koh and Liang better. 
