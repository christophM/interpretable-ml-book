...
This makes influence function and deletion diagnostics **one of the best debugging tools for machine learning models**.
Of the techniques presented in this book, it's the only one that directly helps to identify the instances which should be checked for errors.
...
If your model needs an hour to train and you have 10,000 instances, it takes longer than a year to sequentially calculate the influence of all instance by omitting them one after the other from the training data.
...
It is therefore not a big leap to assume that deletion diagnostics will work without time problems even with large neural networks in 10 years.
...
Koh and Liang (2017) showed for some examples that the influence calculated by the influence function was close to the influence measure obtained when the model was actually recomputed after the instance was deleted.
...
- The influence measures **only takes into account the deletion of individual instances** and not the deletion of several instances at once.
...
Take a a look at [the code](https://github.com/christophM/interpretable-ml-book/blob/master/manuscript/06.5-example-based-influence-fct.Rmd) I wrote for the examples in this chapter.
