...
I recommend the book 'The Elements of Statistical Learning' (Friedman, Hastie, and Tibshirani 2009)[@hastie2009elements] for a more detailed introduction to CART.
...
$I_{\{ \mathbf{x}^{(i)} \in R_m \}}$ is the identity function that returns 1 if $\mathbf{x}^{(i)}$ is in the subset $R_m$ and 0 otherwise.
If an instance falls into a leaf node $R_l$, the predicted outcome is $\hat{y}^{(i)}=c_l$, where $c_l$ is the average of all training instances in leaf node $R_l$.
...
#| fig.cap: Regression tree fitted on the bike rental data. The boxplots show the distribution of bicycle counts in the terminal node.
#| fig-width: 7
#| fig-height: 5
...
Some of them interact with the data size, so make sure to pick hyperparameters that control number of nodes and tree depth directly.
...
If recently not so many bikes were rented, then the temperature decides whether to predict more or less rentals.
...
@fig-tree-importance shows the feature importance.
...
CART and other tree algorithms are invariant to the scale of the feature.
For example, changing a feature from kilograms to grams (multiply by 1000), will change the split value, but the overall tree structure will be unchanged.
