...
Besides these approaches ones, there are many more approaches to interpretable models, ranging from extensions of these basic approaches to very specialized approaches.
To include all of them would be impossible, so I have focused on the most basic ones.
Here are a some examples of other interpretable-by-design approaches:
...
- **The model is entirely interpretable.** Example: small decision tree can be visualized realistically as an entire model. Or linear regression model with not too many coefficients.
...
Some approaches allow to interpret individual predictions.
...
Or for decision trees, a prediction is explained by returning the decision list that lead to the prediction.

The scope of interpretability is another tool you can use to understand a new interpretability approach you are learning about.
Ask yourself: At which level(s) of interpretability does it operate?
...
But it gets tricker when it comes to data insights, because of the need for a theoretical link between model structure and data.
To interpret the model in place of the data, you have to assume that the model structure reflects the world -- something statisticians work very hard on and needs a lot of assumptions.
...
- Model-specific: We analyze parts of the model them to better understand it. This can be  analyzing which types of images a neuron responds the most in a neural network, or the Gini importance in random forests.
...
Looking at this from a more higher level, the modeling process gains another layer: It starts with the world which, we capture in the form of data, from which we learn a model.
...
* [SHAP](#shap) is a computation method for Shapley values, but also suggest global interpretation methods based on combinations of Shapley values across the data.
...
* [Functional decomposition](#decomposition) is a central idea of interpretability and a technique for decomposing prediction function into smaller parts.
...
Two broad categories within global model-agnostic method are **feature effects** and **feature importance**.
...
In particular, LOFO is related to feature selection methods and is particular useful for model improvement.
...
For example, while decision rule lists make it easy to justify individual predictions, you may also want to justify the model itself by showing which features were important over all.
...
As the name implies, post-hoc model-specific methods applied after model training but only work for specific machine learning models. 
...
Neural networks are an interesting target for interpretation interpretation target because neural networks learn features and concepts in their hidden layers.
...
In general, the biggest strength of model-specific methods, in general, is the ability to discover learn about the models themselves.
...
- Shapley values to explain individual predictions are solidly classified as model-agnostic. To explain a linear regression prediction, you can multiply each feature value with it's coefficient. These are called effects then. In addition, you can subtract from each effect the average effect from the data. If you do these things, you have computed Shapley values.
