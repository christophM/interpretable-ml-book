...
![Short taxonomy of interpretability methods which also reflects the structure of the book.](./images/taxonomy.png){#fig-taxonomy width=70%}
...
If you want an machine learning algorithm that produces interpretable models, the algorithm has to constrain the search of models to those that are interpretable.
...
Models that are interpretable by design are also called **intrinsically** or **inherently** interpretable models, see also @fig-inherently.

![Interpretability by design means using machine learning algorithms that produce "inherently interpretable" models.](./images/interpretable-box.png){#fig-inherently width=80% fig-align="center"}
...
Including all of them would be impossible, so I have focused on the most basic ones.
...
- @yang2024inherently proposed inherently interpretable tree ensembles which are boosted trees (e.g., with XGBoost) with adjusted hyperparameters, such as low max depth, a different representation where feature effects are sorted into main effects and interactions, and pruning of effects. This approach mixes both interpretability by-design and post-hoc interpretability.
- Model-based boosting, is an additive modeling framework. The trained model is a weighted sum of linear effects, splines, tree stumps, ... [@buhlmann2007boosting].
...
But how easy they make your job of improving the model varies.
...
![Model-agnostic interpretation methods work with inputs and outputs and ignore model internals.](./images/agnostic-black-box.png){#fig-black-box width=80% fig-align="center"}
...
![The big picture of (model-agnostic) interpretable machine learning. The real world goes through many layers before it reaches the human in the form of explanations.](./images/big-picture.png){#fig-big-picture}
...
![Model-specific methods make complex models more interpretable by analzying the models.](./images/specific-black-box.png){#fig-model-specific width=80% fig-align="center"}
...
The neural network part cover the following techniques that answer different questions:
...
- [Concepts](#concepts): What more abstract concepts did the neural network learn?
...
- Shapley values for explaining individual predictions are seen as model-agnostic method. To explain a linear regression prediction, you can multiply each feature value with its coefficient. These are then called effects. In addition, you can subtract from each effect the average effect from the data. If you do these things, you have computed Shapley values.
