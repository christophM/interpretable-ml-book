...
This chapter provides a map of interpretability approaches so that when diving into the individual models and methods, you still realize you are in a forest when looking at an individual trees. 
...
These would easily break the scope of this book, so I'll just give you a few examples:
...
-   TODO: Add a few examples here
-  TODO: Maybe move  @yang2024inherently  as example of mixing stuff up

But really, the approaches are so varied here.
Some interpretability-by-design methods make it easy to write down the entire logic of the model into a little table, like you can do for linear regression models.
While other claim interpretability, but it's only for individual predictions.
An example would be a decision rule lists with hundreds of rules, which is difficult to grasp as a whole, but for a single prediction, we might have a simple rule we can interpret.

<!-- Strengths and Limitations -->
To argue whether or not the by-design approach furthers your specific interpretability goals therefore depends on the actual ML approach.

Interpretable models are much easier to debug and improve, since the decision-making mechanisms are laid bare.
You can rather easily see when a coefficient is zero, or when a rule doesn't make any sense.
...
In many fields there are established interpretable models.
For example, medical research is big in e.g. logistic regression and other regression methods.
Depending on the research journal / community, using a decision rule list may be more difficult to justify.
The justification is so very strong that they are seen as given.

Justification really depends on whether you can make the relevant stakeholders understand the prediction mechanism of the model.

When it comes to discovering insights, interpretable models are a bit of a mixed bag.
They often make it easy to read out information:
For a linear regression model, we can easily say how a feature affects the prediction.
But to make conclusions about the data, we need more:
Does this feature REALLY influnce the outcome linearly?
Because we kind of forced the feature to do so by using linear regression.
...
This is aggravated for cases where we might find much better performing "black box" models:
What if the proto-type based image classifier has much less performance than the convolutional neural network?
How far does it help us understanding the data?

There is another price to be paid: By reducing the model search to a subset interpretable models, you exclude many other especially flexible models.
And searching only a subset of the models may cost you a lot in performance.
...
We take the data (in this case we sample all), intervene on the data by permuting it, get the model predictions, and compute again the model error and compare it with the original loss (aggregation).


![](./images/agnostic-black-box.png)


Model-agnostic interpretation separates the model interpretation / explanation from the model training. 
...
Some methods even allow you flexibility for the feature representation used for creating the explanations: For example you may use image patches instead of pixels.
...
* [Ceteris paribus](#ceteris-paribus) describe how changing a feature changes a data points prediction.
...
Other methods, like ceteris paribus and ICE focus on individual features.
Methods like counterfactual and Anchors find themselves somewhere in the middle of explaining the predictions with a subset of the features. 

<!-- Strengths and Limitations -->
For model debugging, local methods provide rather "zoomed in" views, which can be useful for understanding edge cases or studying unusual predictions.
...
When it comes to using local interpretation methods for justifying individual predictions, the usefulness is rather mixed:
Methods like ceteris paribus and counterfactual explanations can be very useful for justifying model predictions, since faithfully reflect the raw model predictions.
Attribution methods, however, like SHAP or LIME don't are kind of "models" (or at least more complex estimations) on top of the model and therefore might not be as useful for some high-stakes justification purposes [@rudin2019stop].

Local methods shine for data insights.
...
LOFO is related to feature selection methods and particularly suitable for model improvements. 

For justifying the models to stakeholders, global interpretation methods can deliver some of the broad strokes, in terms of which features were relevant and how, on average, features affect the predictions.
You can also use them in combination with inherently interpretable models.
Because while decision rule lists may make it easy to justify individual predictions, you may want to also justify the model itself, by showing which features were important and so on.
...
As the name says, these are methods applied after model training which only work for specific types of models.
...
Since they are gradient-based we can leverage many 
...
Model-specific methods can be helpful for debugging and improving the models.
When it comes to insights, model-specific draw the interpretation a lot to the internals of the models, and we again have to assume somehow that these internal mechanisms reflect reality for making such a leap.
