...
We want as little of the features to change. 
The prediction should  come as close as possible to the target we set (e.g. flipping or crossing threshold).

The resulting instances should have realistic value: So at least it should be bounded, so that the feature don't  cross some minimal and maximal interval boundaries.
Best, it is likely in the data distribution.
...
Other approach: Sample plausible x values and check if the prediction changes in a relevant way.
