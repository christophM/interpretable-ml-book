...
- The counterfactual method does not need any access to the data itself nor does it need to know anything about the underlying model.
...
Counterfactuals over this balance between transparency and privacy.
...
- The counterfactuals are nowhere implemented as an easy to use software package.
- A reality, which is often not desirable. 
There is not one correct counterfactual, but many (the Rashomon effect).
This is inconvenient - people love single, simple explanations for things and not being offered multiple different explanations.
...
- The proposed solution and distance function has no good solution for categorical features, which may have many different levels. 
...
- Finding Counterfactuals through growing spheres around the point and searching within that. [^spheres]
- Anchors: This is the opposite of counterfactuals. They ask, what should not change to keep the prediction the same. TODO CITE
