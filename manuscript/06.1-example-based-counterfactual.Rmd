...
Ideally, If the number of square meters is increased, an increase in the number of rooms should be also proposed. 
...
In this section, we focus on two of them: first, the one by Wachter et. al (2017)[^wachter] who introduced counterfactual explanation as an interpretation method and, second, the one by Dandl et al. (2020)[^dandl] that takes into account all four criteria mentioned above. 
...
The Gower distance can handle both numeric and discrete features but does not count how many features were changed. 
...
The fourth objective reflects that our counterfactuals should have likely feature values/combinations. 
...
Fitter counterfactuals have lower objective values. 

The algorithm consists of four steps that are repeated until a stopping criterion is met, e.g. a maximum number of iterations/generations.
[Figure 6.2](#fig:nsgaII-cf) visualizes the four steps of one generation. 
...
They are pairwise recombined to produce children that are pretty similar to them, e.g. by averaging their numerical feature values or by crossing over their categorical features. 
...
Given the ranking, we select the most promising and/or most diverse half of the candidates. 
...
The used German Credit Risk dataset is from a [kaggle challenge](https://www.kaggle.com/uciml/german-credit). 
...
The counterfactuals should answer how the input features need to be changed, to get a predicted probability larger 50 \%?
...
All counterfactuals suggest a reduction of the duration from 48 months to minimum 23 months, some of them propose that the woman should become skilled (job = 2) instead of unskilled (job = 1). 
...
The multi-objective counterfactual explanation method by Dandl et al. is implemented in a [Github repository](https://github.com/susanne-207/moc/tree/master/counterfactuals) but might become an R package soon.
...
<!-- Another model-agnostic approach was proposed by Martens et. al (2014) for explaining document classifications. -->
