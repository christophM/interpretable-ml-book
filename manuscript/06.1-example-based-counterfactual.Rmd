...
A counterfactual can explain a causal situation in the form "If X had not occured, Y would not have occured".
...
Analysing these What-If-scenarios requires imagining a hypothetical reality that is contraire to the observed facts (e.g. a world in which I didn't drink the hot coffee), hence the name "counterfactual".
...
The "event" is the predicted outcome of an instance, the "causes" are the particular feature values of that instance that were input to the model.
We can easily simulate What-If-scenarios for machine learning model predictions:
We simply change the feature values of an instance before making the predictions and analyse how the prediction changes.
We are interested in scenarios, where the prediction changes in some relevant way, like a flip in predicted class (e.g. loan application accepted vs. rejected) or the prediction passes some threshold (e.g. probability for cancer reaches 10%).
**A counterfactual explanation of a prediction of an instance describes the smallest change to the feature values that would change the prediction to some predefined output.**


Counterfactual explanations are model-agnostic method, since they work only with the model input and output.
Arguably, this method would also feel at home in the [model-agnostic chapter](#agnostic), since the output can be expressed as a summary for a feature ("change feature A and B to get prediction change").
But a counterfactual explanation is always a new instance itself, that's why it lives in this chapter ("starting from instance X, change A and B to get instance X'").
In contrast to [prototypes](#proto), counterfactuals don't have to be actual instances from the data, but can be an unseen combination of feature values.
...
All are valid counterfactuals, but each tells a different 'story' how to come to that outcome.
One counterfactual might say to change feature A, the other counterfactuals says to leave A the same, which is a contradiction.
...
The users define a relevant change in the prediction of an instance (= the alternative reality), so an obvious first requirement is that **a counterfactual instance produces the predefined prediction as closely as possible**.
...
Searching for changes of the features of an instance, in order to switch the label from the common to the rare class might be impossible.
So we might want to relax the requirement a bit that the predicted output of the counterfactual matches exactly the defined outcome.
In the classification example we could look at the predicted probabilities and search for counterfactual where the probability of the rare class is increased, e.g. 10% instead of the 2% predicted probability an instance has.
...
Further, **a good counterfactual is as similar as possible to the instance regarding feature values**.
This requires a measure of distance between the instance to be explained and other distances, which we want to minimize.
...
This can be factored into the distance measure (for example by using the Manhatten norm).
The last requirement is that **a counterfactual instance should have feature values that are likely**. 
It wouldn't be useful to generate a counterfactual explanation for the rent example where an apartments size is negative or the number of rooms is set to 200.
...
The first natural instinct for finding counterfactuals is trial and error (a search method).
...
Like in the example about Anna trying to find a version of her apartment for which she can charge more rent.
But there are better approaches than that.
First, we define a loss function that takes an instance and a counterfactual and measures how far away the predicted outcome of the counterfactual is to the predefined outcome and how close the counterfactual is to the instance of interest.
With this loss function, we can either try to optimize it directly with an optimization algorithm or by searching / sampling (like the method 'Growing Spheres', see [Software and Alternatives](#example-software)).

In this section I will present the approach suggested by Wachter et. al 2017[^wachter]
They suggest a loss function that finds good counterfactuals by finding the ones that minimize the loss.


$$L(x, x', \lambda) = \lambda\cdot(\hat{f}(x')-y')^2+d(x,x')$$
...
The $\lambda$ balances the distance in prediction (first term) against the distance of the points (second term).
The loss is solved for a given $\lambda$ and returns a counterfactual x'.
A higher value means that we favor counterfactuals that are close to the desired outcome y', a lower value means that we favor counterfactuals that are very similar to x in the feature values.
If $\lambda$ is large, but there is no counterfactual with a prediction that comes close to y', then the one will be chosen that is closest, regardless how far it is away from x.
...
The distance is the sum over all features of the absolute distance between the counterfactual and the instance of interest.
It is scaled by the inverse of the median absolute deviation of feature j over the dataset, which is defined as:


$$MAD_j = median_{i\in{}\{1,\ldots,n\}}(|x_{i,j}-median_{l\in{}\{1,\ldots,n\}}(x_{l,j})|)$$
The MAD is the equivalent to the variance of a variable, but instead of using the mean as a center and summing over the quadratic distance, we use the median as the center and sum over the absolute distance.
The used distance function has the advantage over e.g. Euclidean distance of introducing sparsity to the explanations that are generated.
...
1. Optimize the counterfactual.
1. Return the counterfactual that minimizes the loss function.
1. Repeat steps 2-4 and return the list of counterfactuals or the one that minimizes the los.

You might need to go back to step 1 and ajdust $\lambda$ when the tradeoff between matching the desired outcome and closeness of instance and counterfactual is not as desired.

### Example

The examples are both taken from the paper by Wachter et. al (2017) [^wachter].

The first example works with data on average Law School Admission Tests (LSAT).
...
The goal is to find counterfactuals for each case which answers the following question:
...
The counterfactuals for the first two rows say what would have to change to decrease the score and for the other three cases what would have to change to increase the score to average.
...
The classifier is again a three-layer fully-connected neural network and is trained to predict the probability of diabetes given age, BMI, number of pregnancies and so on for women of Pima heritage.
The counterfactuals answer the question "What feature values would have to be different to increase or decrease the probability of diabetes to 50%".
...
**The interpretation of counterfactuals explanations is very clear**.
...
There are no assumptions involved or some magic happening in the background. 
...
This is attractive for companies, who are audited by a third party or for creating explanations for customers, without revealing the model or the data. 
...
We can create counterfactuals for any system that receives inputs and creates outputs.
The system that predicts apartment rents could also consist of hand-written rules and counterfactuals would still make sense.
...
Let's say we have generated 23 counterfactuals explanations for an instance. 
...
It can also be an advantage to have multiple counterfactual explanations, because then humans can pick the ones they like best, which matches prior knowledge.

It is **unclear how to optimize for $\lambda$** in practice and hard to give general advice or automate it.
The choice of the parameter is mostly up to the user and the specific needs in the project.
...
Maybe it's okay to create a counterfactual that changes the the predicted apartment rent from 900 Euros to 990 (10 Euros away from the desired 1000) by only changing one feature. 
By increasing lambda, maybe we find a counterfactual apartment with a predicted rent of 1001, which is closer to the desired outcome, but 4 features had to change dramatically for it. 
...
The authors of the method suggested to run the method for each unique level combination of the categorical features, but this will result in a combinatorial explosions when you have multiple categorical features  with many values (6 categorical features with 10 unique levels would mean 1 million runs).
...
The method first draws a sphere around the point of interest, samples point within that sphere, checks if one of the sampled points has the desired outcome and either contracts or expands the sphere and finally returns a spares counterfactual.
They don't use the word counterfactual in their paper, but the method is rather similar.
They also define a loss function that favors counterfactuals with as few changes in the features as possible.
...
Anchors answer to the question: 
Which features are sufficient to anchor a prediction, i.e. changing the other features can't change the prediction?
Once we have found features acting as anchors of the prediction, we won't find any counterfactual instances by changing the features not used as anchor.
