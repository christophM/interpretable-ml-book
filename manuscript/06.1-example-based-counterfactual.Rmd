...
Thinking in counterfactuals requires imagining a hypothetical reality that is contradicts the facts observed (e.g. a world in which I have not drunk the hot coffee), hence the name "counterfactual".
...
We can easily simulate counterfactuals for predictions of machine learning models:
We simply change the feature values of an instance before making the predictions and we analyse how the prediction changes.
We are interested in scenarios in which the prediction changes in a relevant way, like a flip in predicted class (e.g. credit application accepted vs. rejected) or in which the prediction reaches a certain threshold (e.g. the probability for cancer reaches 10%).
**A counterfactual explanation of an instance prediction describes the smallest change to the feature values that would change the prediction to a predefined output.**
...
But a counterfactual explanation is itself a new instance, so it lives in this chapter ("starting from instance X, change A and B to get a counterfactual").
Unlike [prototypes](#proto), counterfactuals don't have to be actual instances from the training data, but can be an new combination of feature values.

Before discussing how to create counterfactuals, I would like to discuss some example applications for counterfactuals and how a good counterfactual explanation looks like.

In this first example, Peter applies for a loan and gets rejected by the (machine learning powered) banking software.
Since he wanted the loan, he wonders why his application was rejected.
The question of "why" can be formulated as counterfactual:
...
In this second example, we look at a model that predicts a continuous outcome in which we want counterfactuals for a predefined prediction.
Anna wants to rent out her apartment, but she is not sure how much to charge for it, so she decides to train a machine learning model to predict rents (of course, since Anna is a data scientist, that's how she solves her problems).
...
She expected 1000 Euro or more, but she trusts her model and decides to play with the feature values of the appartment to see how she can improve the value of the apartment.
She finds out that the apartment could be rented out for over 1000 Euro, if it were 15 square meters larger.
Interesting, but non-actionable knowledge, because she can't enlarge her apartment.
Finally, by tweaking only the things under her control (built-in kitchen yes/no, pets allowed yes/no, type of floor, etc.), she find out that if she allows pets and installs windows with better insulation, she can charge 1000 Euro.
Anna had intuitively worked with counterfactuals to change the outcome.

Counterfactuals are [human-friendly explanations](#good-explanation), because they are contrastive to the current situation and selective, because they usually focus on a small number of feature changes.
...
The same can also happen with counterfactuals, since there are usually multiple different counterfactual explanation.
Each counterfactual tells a different 'story' how to reach a certain outcome.
...
The user of a counterfactual explanation defines a relevant change in the prediction of an instance (= the alternative reality), so that an obvious first requirement is that **a counterfactual instance produces the predefined prediction as closely as possible**.
...
Searching for changes to the feature values that would switch the label from the common class to the rare class might be impossible.
...
In the classification example we could look at the predicted probability and look for a counterfactual where, for example, the probability of the rare class is increased to 10% instead of the current 2%.
...
Another quality criterium is that **a counterfactual should be as similar as possible to the instance regarding feature values**.
This requires a distance measure between the instance to be explained and other instances.
The counterfactual should not only be close to the original instance, but should also **change as few features as possible**.
This can be included in the distance measure, for example by relying the Manhatten norm.
The last requirement is that **a counterfactual instance should have feature values that are likely**. 
It wouldn't make sense to generate a counterfactual explanation for the rent example if the size of an apartment is negative or the number of rooms is set to 200.
It is even better when the counterfactuals is likely according to the joint distribution of the data, e.g. an apartment with 10 rooms and 20 square meters should not be regarded as counterfactual.
...
If $\lambda$ is large, then the closest instance will be selected, regardless how far it is away from x.
...
The instance (x) to be explained, the desired output (y') and the control parameter ($\lambda$) must be set in advance.
The loss function is minimized for x' and the (locally) optimal counterfactual x' returned.

$$\arg \min_{x'}L(x,x',y',\lambda)$$

The distance function d for measuring the distance between instance x and counterfactual x' is the L1-norm of the Manhatten distance weighted with the inverse median absolute deviation (MAD).
...
Scaling with the MAD is necessary to bring all the features to the same scale - it shouldn't matter whether you measure the size of an apparment in square meters or square feet.

The recipe for producing the counterfactuals is simple:

1. Select an instance (x) to be explained, the desired outcome y', and $\lambda$.
...
When the tradeoff between the distance to the desired outcome y' and the distance to the instance x is not as desired, you may need to return to step 1 and adjust $\lambda$.

### Examples

Both examples are from the work of Wachter et. al (2017) [^wachter].

In the first example, the authors train a three-layer fully-connected neural network to predict a student's average grade of the first year at law school, based on the grade point average (GPA) prior to the law school, the race and the law school entrance exam scores.
The goal is to find counterfactuals for each student that answer the following question:
How would the input feature need to be changed, to get a predicted score of 0?
Since the scores were have been normalised before, a student with a score of 0 is as good as the average of the students.
A negative score means a below-average result, a positive score an above-average result.

The following table shows the counterfactuals learned:
...
The counterfactuals always change the race from black (coded with 1) to white (coded with 0) which shows a racial bias of the model.
...
The classifier is again a three-layer fully-connected neural network and is trained to predict the risk for diabetes depending on age, BMI, number of pregnancies and so on for women with Pima heritage.
...
This also means it is not as dangerous as [local models like LIME](#lime), where it is unclear how close a data point must be to extrapolate the prediction.

The counterfactual method creates a new instance, but we can also summarize a counterfactual by specifying which feature values have changed.
...
This is attractive for companies that are audited by a third parties or which are creating explanations for customers, without disclosing the model or data. 
A company has an interest in protecting the model and data because for business secrets or data protection reasons.
Counterfactuals offer a balance between explanation of model predictions and protection of the interests of the model owner.
...
It is **unclear how to choose $\lambda$** in practice and it is difficult to give general advice or automate it.
The choice of the parameter is mostly left to the user and the specific requirements of a project.
...
Maybe it's okay to create a counterfactual that changes the predicted apartment rent from 900 Euro to 990 (10 Euro away from the desired 1000) by changing only one feature. 
By increasing $\lambda$ we can find a counterfactual apartment with a predicted rent of 1001, which is closer to the desired outcome, but 4 features had to change dramatically. 
Which is the better counterfactual?
...
The authors of the method suggested running the method for each level combination of the categorical features, but this will lead to a combinatorial explosion if you have multiple categorical features with many values (6 categorical features with 10 unique levels would mean 1 million runs).
...
The method first draws a sphere around the point of interest, samples point within that sphere, checks whether one of the sampled points has the desired outcome, contracts or expands the sphere and finally returns a sparse counterfactual.
