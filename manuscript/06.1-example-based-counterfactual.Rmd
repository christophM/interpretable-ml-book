...
It wouldn't make sense to generate a counterfactual explanation for the rent example where the size of an apartment is negative or the number of rooms is set to 200.
...
You may need to return to step 1 and adjust $\lambda$, when the trade-off between distance to  y' and distance to x is not as desired.
...
```{r counterfactuals-pima, fig.cap="Counterfactual explanations for predicted risk of diabetes. Example by Wachter et. al (2017)."}
...
By increasing $\lambda$ we can find a counterfactual apartment with a predicted rent of 1001 Euro, which is closer to the desired outcome, but 4 features had to change dramatically. 
Which one is the better counterfactual?
This question must be answered on a case-by-case basis.

The proposed method **can't handle categorical features** with many different levels well. 
The authors of the method suggested running the method seperately for each combination of feature values of the categorical features, but this will lead to a combinatorial explosion if you have multiple categorical features with many values.
