...
A counterfactual describes a causal situation in the form "If X had not occurred, Y would not have occurred".
...
This can be included in the distance measure, for example by relying the Manhattan norm.
...
The distance function d for measuring the distance between instance x and counterfactual x' is the L1-norm of the Manhattan distance weighted with the inverse median absolute deviation (MAD).
...
When the trade-off between the distance to the desired outcome y' and the distance to the instance x is not as desired, you may need to return to step 1 and adjust $\lambda$.
