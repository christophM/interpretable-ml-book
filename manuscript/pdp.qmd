...
Note that here the $x^{(k)}_S$ are the $K$ unique values of feature $X_S$.
For categorical features we have:

$$I(X_S) = \frac{max_k(\hat{f}_S(x^{(k)}_S)) - min_k(\hat{f}_S(x^{(k)}_S))}{4}$$
...
We see a similar pattern for bill depth, but the probability doesn't go to zero for deep bills.
