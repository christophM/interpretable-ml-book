...
Be cautious when interpreting PDPs for correlated features.
...
In this case, we have fitted a random forest to predict the number of bicycles and used the partial dependence plot to visualize the relationships the model has learned.
...
```{r}
#| label: pdp-bike-cat
#| fig.cap: PDPs for the bike count prediction model and the season. All seasons show similar effect on the model predictions, only for winter the model predicts fewer bicycle rentals, and slightly fewer for spring.
#| fig.height: 4
...
Both predict P(female) based on body measurements.
We compute and visualize the partial dependence of the probability for being female based on body mass and bill depth for the random forest:
...
For this we only have to compute the individual PDPs by subsetting the data and plot the curves in the same plot, as I did in @fig-pdp-penguin-by-species.


```{r}
#| label: fig-pdp-penguin-by-species
#| fig.cap: PDP split by penguin species, comparing logistic regression and random forest. A more nuanced interpretation emerges. For Adelie, more weight means less likely to be female. For Chinstrap the weight doesn't affect the probability very much. Gentoo penguins are in general heavier, but also show the "males are more heavy" pattern. Also logistic regression is, suprise, surprise, more smooth, and the random forest probabilities are drawn to the mean.
#| fig.height: 4
...
#| fig-cap: PDP for P(female) and the interaction between bill depth and bill length. The dots are black for female penguins, gray for male. There's an interaction between the two, because at small bill depths, bill length doesn't make a difference, but for longer bills, P(female) becomes smaller.
...
Means to do this is reducing tree depth when using tree-based methods or adding monotonicity constraints.
