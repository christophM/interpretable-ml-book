...
#| fig-cap: PDP for P(female) and the interaction between bill depth and bill length. The dots are black for female penguins, gray for male. There is an interaction between the two, because at small bill depths, bill length doesn't make a difference, but for longer bills, P(female) becomes smaller.
