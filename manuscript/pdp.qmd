...
In this case, we have fitted a random forest to predict the number of bicycles and used the partial dependence plot to visualize the relationships the model has learned, see @fig-pdp-bike.
...
```{r}
#| label: pdp-bike,
#| fig.cap: 'PDPs for the bicycle count prediction model and temperature, humidity and wind speed. Marks on the x-axis indicate the data distribution.'
#| fig.height: 3
...
All seasons show similar effect on the model predictions, only for winter the model predicts fewer bicycle rentals, and slightly fewer for spring.
...
The heavier the penguin, the less likely it is female. We see a similar pattern for bill depth, but the probability doesn't go to zero for deep bills.
...
A more nuanced interpretation emerges. For Adelie, more weight means less likely to be female. For Chinstrap the weight doesn't affect the probability very much.
Gentoo penguins are in general heavier, but also show the "males are more heavy" pattern.
Also logistic regression is, surprise, surprise, more smooth, and the random forest probabilities are drawn to the mean.

```{r}
#| label: fig-pdp-penguin-by-species
#| fig.cap: PDP split by penguin species, comparing logistic regression and random forest. #| fig.height: 4
...
#| fig-cap: PDP for P(female) and the interaction between bill depth and bill length. The dots are black for female penguins, gray for male. pred.adelie = Predictor$new(pengu_rf, data = penguins, class="female")
