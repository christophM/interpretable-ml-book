...
```{r , }
#| label: correlation-pdp-ale-plot
...
We train a random forest to predict the [penguin species](#penguins) based on body measurements.
We visualize the accumulated local effects for two of the features:

```{r}
#| label: ale-penguins-1D
#| fig.cap: "ALE plots for the effect of bill length and body mass on predicted probability for each species. Short bills are associated with a higher probability for Adelie and a lower probability for Chinstrap. This relationship is reversed for bill lengths greater than 45 mm. The probability of Gentoo doesn't change much with bill length. Increasing body mass slightly increases P(Adelie) and P(Gentoo), but decreases P(Chinstrap)."
#| fig.height: 4
...
ale1 = FeatureEffect$new(pred.penguins, "bill_length_mm", method = "ale")$plot() +
  scale_x_continuous("Bill length in millimeters") + 
  scale_y_continuous("", limits=c(-0.2, 0.25))
ale2 = FeatureEffect$new(pred.penguins, "body_mass_g", method = "ale")$plot() +
  scale_x_continuous("Body mass in grams") +
  scale_y_continuous("", limits=c(-0.2, 0.25))
gridExtra::grid.arrange(ale1, ale2, ncol = 1)
```

The effects are quite similar to the PDPs in [the PDP chapter](#pdp).
Only for Adelie the effect of body mass is different, since we can clearly see an increase in P(Adelie) with higher body mass.
Next, we look at the interaction between body mass and bill length, but only for P(Adelie).

```{r , }
#| label: ale-penguins-2d
#| fig.cap: 'ALE plot of the 2nd order effect of body mass and bill length. Both features interact with each other. For example, a bill length between 41 and 46 combined with a body mass below 4000g decreases the predicted P(Adelie) by almost 0.2. However, the same bill length but for heavier penguins has increased the predicted P(Adelie) by about 0.15. For other bill lengths, the body mass does not have the same effect, making it an interaction.'
pred.penguins = Predictor$new(mod, data = penguins, class="Adelie")
FeatureEffect$new(pred.penguins, c("bill_length_mm", "body_mass_g"), grid.size = 30)$plot(show.data = TRUE) + 
scale_fill_gradient("ALE", low = "red", high = "yellow") + 
  scale_x_continuous("Bill length inn mm")  + 
  scale_y_continuous("Body mass in g") +
  scale_fill_viridis(option = "D")

```

The interaction plot shows really interesting effects that we couldn't see in the PDP, which is a nice property of the 2D-ALE, since it shows the pure interaction without the main effects.
...
Grömping (2020) [@gromping2020model] showed that in a linear model with two correlated features and an additional interaction term ($\hat{f}(x) = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_1 x_2$), the first-order ALE plots do not show a straight line.
