...
To get a better feel for how this one rule performs, let's look into the cross table:
...
We can see that especially for very short and very long flippers the feature is useful for classification.
With a flipper length above 219, we can be sure it's a Gentoo penguin and between 207 and 2019, it's at least likely. Below 184 mm, it's likely (90%) to be the Adelie species.
However, inbetween these extremes, relying on the flipper length alone doesn't give us as much clarity on the species.
...
extract.rules.jrip = function (rule) {
  ...
  # removes text
    ...
    if (nrow(rules) == 0) {
      ...
      kableExtra::kbl(rules,  booktabs = TRUE) %>% kable_styling(position = "center")
    }
}

```

```{jrip-penguins, include=FALSE, eval=FALSE}
rule = JRip(species ~ ., data = penguins)
...
We use the SBRL algorithm to predict the [penguins species](#penguins).
Since there are 3 possible species in the palmers penguin data, but SBRL needs binary labels, I simplified the task to classifying Adelie versus other (Gentoo/Chinstrap). 
Also, I first had to discretize all input features for the SBRL algorithm to work. 
