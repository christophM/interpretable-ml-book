...
kable(df,  booktabs = TRUE) %>% kable_styling(position = "center")
```
...
```{r OneR-freq-table2}
kable(table(paste0("location=", df[,"location"]), value.f), booktabs = TRUE) %>% kable_styling(position = "center")
kable(table(paste0("size=", df[,"size"]), value.f),  booktabs = TRUE) %>% kable_styling(position = "center")
kable(table(paste0("pets=", df[,"pets"]), value.f),  booktabs = TRUE) %>% kable_styling(position = "center")
```
...
kable(rule.to.table(rule), row.names = FALSE, booktabs = TRUE) %>% kable_styling(position = "center")
```
...
kable(tt, booktabs = TRUE, align = "c") %>%
...
extract.rules.jrip = function (rule) {
  ...
  # removes text
    ...
    if (nrow(rules) == 0) {
      ...
      kable(rules,  booktabs = TRUE) %>% kable_styling(position = "center")
    }
}
...
tab = kable(rules$rules, booktabs = TRUE) %>% kable_styling(position = "center")
if (is.pdf) tab = tab %>%  column_spec(1, "10cm")
tab
```
...
conditions = gsub(",", ", ", conditions)
tab = kable(conditions, col.names = "pre-mined conditions",  booktabs = TRUE) %>% kable_styling(position = "center")
if (is.pdf) tab = tab %>%  column_spec(1, "10cm")
tab
```
...
tab = kable(rules, booktabs = TRUE) %>% kable_styling(position = "center")
