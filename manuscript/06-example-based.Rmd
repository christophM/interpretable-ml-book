...
Let's start with a few real world examples [^cbr]:
...
A decision tree predicts a new data points by finding the data points that are similar (= in the same terminal node) and by returning the average of the outcomes of those points as the prediction.
...
- [k-nearest neighbours model](#other-interpretable): An (interpretable) machine learning model that is based on examples.
...
A **prototype** is a data instance that is representative for all the data.
...
They are only sufficient when the data distribution within the classes is very homogeneous.
A **criticism** is a data instance that is not represented well by the set of prototypes.
...
Prototypes and criticisms can be used independently from machine learning to just describe the data, but it can also be used to build an interpretable model or to make a black box model interpretable.

Throughout this chapter I will also be using the word '(data) point' to refer to a single instance, to emphasize the interpretation that an instance is also a point in a coordinate system where each feature is a dimension.
The following plots shows an artificial data distribution, with some of the instances chosen as prototypes and some as criticisms.
The small dots are the data, the big dots the prototypes and the big triangles the criticisms.
The prototypes are (manually) chosen to cover the centers of the data distribution and the criticisms are points in a cluster without a prototype.
...
Actually any clustering algorithm which returns actual data points as cluster centers would qualify for selecting prototypes.
...
Data points in areas of high data density are good prototypes, especially when points from different 'data clusters' are selected.
...
Points are chose as criticisms when the distribution of prototypes and of the data differ the most.
...
A kernal is a function that weights two data points according to their distance.
Based on density estimates, we need a measure that tells us how different two distributions are, so that we can tell if the distribution of prototypes we have chosen is close to the data distribution.
...
With the witness function we can choose criticisms of the data points, which are points where the distribution of prototypes and data diverges and the witness function takes on large absolute values.
...
We want to test whether the distributions are different, based on their empiricial distributions estimated by kernel densities.
...
All clear?
I personally understand those concepts much better once I see the formulas for how something is calculated with data.
...
The closer MMD2 is to zero, the better the prototype distribution matches the data.

The following graphic illustrates the MMD2 measure. 
The first plot shows the data points with two features. where the data density estimate is indicated with a shaded background.
Each of the other plots show different selections of prototypes, together with the MMD2 measure in the plot titles.
The prototypes are the big red dots and their distribution is plotted as contour lines.
The prototypes selection that covers the data the best in these scenarios (bottom left plot) has the lowest discrepancy value.
...
The value of the kernel decreases with the distance of the two points and ranges between zero, when the two points are infinitely far apart, and one, when the two points are equal.
...
- While the number of prototypes is below the chosen number m:
    - Check for each point in the dataset how much it decreases the MMD2 of the prototypes, when added to the list. Add the data point that minimizes the MMD2 to the list.
...
For two datasets (with the same features), the witness function gives you the means to check in which empirical distribution the point x is more likely to fit in.
For finding criticisms we look for extreme values of the witness function in both negative and positive direction.
...
A positive witness function at point x means that the prototype distribution overestimates the data distribution (for example when we select a prototype with few data points nearby);
a negative witness function at point x means that the prototype distribution underestimates the data distribution (for example when there are many data points, but we don't select prototype nearby).

To give you more intuition, let's re-use the prototypes from the plot before with the lowest MMD2 and display the witness function for a few manually selected points.
The labels in the following plot show the value of the witness function for different points marked as triangles
...
The witness function allows us to explicitly search for data instances that are not represented well by the prototypes.
Criticisms are points with large absolute value in the witness function.
The procedure for finding criticisms is the same greedy search as for finding the prototypes. 
But instead of finding the point that decreases the MMD2, we search points that maximizes a cost function that involves the witness function and a regularizer term.
The additional term in the optimization function enforces diversity in the points, which is needed so that the points are not all from the same cluster, but different types of criticisms.

This second step is completely agnostic of how the prototypes are found.
I could have also hand-picked some prototypes and used the procedure described here to learn criticisms.
Or the prototypes could have come from any clustering procedure, like k-mediods.
