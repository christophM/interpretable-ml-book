...
Each point is multi-dimensional, i.e. it can have many features.
...
p = ggplot(dt, aes(x = x1, y = x2)) + stat_density_2d(geom = "tile", aes(fill = ..density..), contour = FALSE, alpha = 0.5) + geom_point() + 
...
The remaining ingredient we need for finding criticisms is the witness function, which can be estimated using:
...
The witness function is positive when x fits better into the distribution of the x, and it is negative when the x fits better with the distribution of the z's.
...
```{r, prototypes-and-criticisms2, fig.cap = "Prototypes and criticisms for a digit recognition dataset. Interestingly the number of prototypes differ per class. Criticisms include digits drawn with unusually thick or thin lines amongst others. Or also unrecognizable digits."}
