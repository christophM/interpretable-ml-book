...
1) By helping to better understand the data distribution; 2) By building an interpretable model; 3) By making a black box model interpretable.

If you apply MMD-critic to your data to find prototypes and criticisms, it will improve understanding of the data, especially if you have a complex data distribution with some edge cases.
If I would include all methods into this book that helps to understand the data better,  I would have to cover boxplots, summary statistics, scatter plots and all the other tools for descriptive statistics tools in this book (which I don't).
But with MMD-critic you can achieve more!

For example, you can create an interpretable prediction model from the prototypes.
...
which means that we select the prototype i from the set of prototypes S that is closest to the new data point, in the sense that yields the highest value of the kernel function.
...
1. Predict outcomes for the prototypes and criticisms with the ML model.
...
Perhaps they should have used the procedure described here before deploying their image recognition algorithm.
It is noth enough just to check the performance of the model, because if it were 99% correct, this issue could still be in the 1%.
...
But the selection of - sazy a few thousand - prototypes and criticisms is feasible and could have revealed a problem with the data:
...
Both applications are based on image dataset.
Each image was represented by image embeddings with 2048 dimensions. 
An image embedding is an abstract representation of an image by a vector that can be extracted from the weights of a neural network trained to solve an image recognition task, in this case on the ImageNet classification dataset.
...
On the right side the prototypes contain images of dogs outdoors. 
...
Each image shows a handwritten digit and I would expect the prototypes to be images of the most common ways of writing a digit and the criticisms some less common ways.

Looking at the actual prototypes and criticisms images you might notice that the number of images per digit is different. 
...
```{r, prototypes-and-criticisms2, fig.cap = "Prototypes and criticisms for a handwritten digits dataset."}
...
It can itself be an interpretable machine learning model; 
...
The number of prototypes and criticisms has to be determined by the user and there are no instructions how to choose a good number.
Only when using MMD-critic to build a classifier do we have a way to optimize it directly.
One solution could be a screeplot showing on the x-axis the number of prototypes and the MMD2 measure on the y-axis. 
...
(Maybe I am a bit harsh here, since all unsupervised problems have this problem.)
