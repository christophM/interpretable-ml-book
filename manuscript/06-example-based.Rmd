...
Now for some heavy theory:

The MMD function is defined is:

$$MMD(F,P,Q)=sup_{f\in{}F}\left(E_{X\sim{}P}[f(X)]-E_{Y\sim{}}Q[f(Y)]\right)$$
F is a reproducing kernel Hilbert space (RKHS) with kernel function . 

$$f(x)=E_{X'\sim{}P}[k(x,X')]-E_{X'\sim{}Q}[k(x,X')]$$
f(x) is also known as the witness function.
