...
We train a random forest to predict the [penguins species](#penguins) based on bodily measurements.
We visualize the accumulated local effects for two of the features:

```{r ale-penguins-1D, fig.cap = "ALE plots for the effect of bill length and body mass on predicted probability of Adelie. Short bills are associated with a higher probability for Adelie and a lower probability for Chinstrap. This relation inverses for bill lengths over 45 mm. The probability of Gentoo doesn't change much with bill length. An increasing body mass increases P(Adelie) and P(Gentoo), but decreases P(Chinstrap).", fig.height = 4}
...
```
The effects are quite simiilar to the PDPs in [the PDP chapter](#pdp).
...
```{r ale-penguins-2d, fig.cap = 'ALE plot of the 2nd-order effect of body mass and bill length. Both features interact with each other. For example, a bill length between 41 and 46 combined with a body mass below 4000g lowers predicted P(Adelie) by almost 0.2. However, the same bill length but for heavier penguins has increased predicted P(Adelie) by around 0.15. For other bill lengths, the body mass does not have the same effect, that is what makes it an interaction.'}
