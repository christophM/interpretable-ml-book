...
Gkolemis et al. (2023) [^rhale] proposed a method to estimate the optimal number and size of intervals.
The central idea is to grow the intervals as long as the model predictions inside them do not change rapidly.
This approach automates bin splitting and helps balance smoothness and accuracy.
...
ALE is also implemented the Python packages: [`effector`](https://github.com/givasile/effector), [ALEPython](https://github.com/blent-ai/ALEPython) and [Alibi](https://docs.seldon.io/projects/alibi/en/stable/index.html).
...
[^rhale]: Gkolemis, V., Dalamagas, T., Ntoutsi, E., & Diou, C. (2023). RHALE: robust and heterogeneity-aware accumulated local effects. In ECAI 2023 (pp. 859-866). IOS Press.