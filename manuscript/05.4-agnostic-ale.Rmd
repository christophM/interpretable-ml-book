...
- **The second-order ALE estimates have a varying stability over the feature space , which is not in any way visualized.** 
The reason is that each estimation of a local effect in a cell uses a different number of data instances.
...
The number of instances is the same in all intervals, thanks to the use of quantiles as grid, but in some areas there will be many short intervals and the ALE curve will consist of much more estimates.
But for long intervals, which will be responsible for a big part of the entire curve, there are comparably less instances.
This for example happened in the [cervical cancer prediction ALE plot](# ale-cervical-1D) for high ages and for years on hormonal contraceptives.
...
The list of disadvantages has become quite long, but don't be fooled by the number of words I am using:
