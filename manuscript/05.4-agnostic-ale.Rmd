...
Accumulated local effects [^ALE] describe how features affect the predicted outcome of a machine learning model on average.
ALE plots are a faster and unbiased alternative the [partial dependence plots](#pdp) (PDPs).

*Keywords: ALE plots, partial dependence plots, marginal means, predictive margins, marginal effects*

I recommend reading the chapter on [partial dependence plots](#pdp) first, because the goal of both methods is the same:
Describe how a feature on average influences the prediction.
In the following section, I will convince you that partial dependence plots have a severe problem when the features are correlated.


### Motivation and Intuition

When features of a machine learning model are correlated, the partial dependence plot cannot be trusted.
Calculating a partial dependence plot for a feature that is strongly correlated with other features, involves creating and averaging over data instances that are unlikely in reality.
This can bias the estimated feature effect heavily.
Imagine we compute partial dependence plots for a  machine learning model that predicts the value of a house depending on the number of rooms and the size of the living area.
We care interested in the effect of the living area on the predicted value.
...
For the first grid value - let's say a living area of 30 square meters - we replace the living area for all instances with 30, even for houses with 10 rooms.
Sounds like a very unlikely house to me.
The partial dependence plots includes these unrealistic houses into it's calculation and pretends everything is fine.
The following plot illustrates two correlated features and why the partial dependence plot method averages over unlikely data areas.

```{r aleplot-motivation1, fig.cap = "Two strongly correlated features x1 and x2. To compute the partial dependence at x1 = 0.75, the PDP replaces x1 with 0.75 for all distances, over the whole range of x2. This creates combinations of x1 and x2 that are unlikely, given their strong correlation. PDPs average over the marginal distribution of features, which assumes that features are independent."}
...
The feature-wise distance compares either the cummulative distribution in both categories, also called Kolmogorov-Smirnov distance (for numerical features) or compares the relative frequency tables (for categorical features).
