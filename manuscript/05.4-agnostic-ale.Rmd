...
What can we do to get a better estimate of the effect of a feature on the prediction?

We could average over the conditional distribution of the feature, meaning at a grid value, we average the predictions of close instances with a similar feature value.
The solution for computing feature effects using the conditional distribution is called Marginal Plots, or M-Plots (confusing name, since they are based on the conditional, not the marginal distribution).
...
Suppose that the living area has no effect on the predicted value of a house, only the numbers of rooms has.
The M-Plot would still show that the size of the living area increases the predicted value, since we use the conditional distribution and the number of rooms increases with the living area.
The following plot shows, for two correlated features, how M-Plots work.

```{r aleplot-motivation2, fig.cap = "Two highly correlated features x1 and x2. M-Plots average over the conditional distribution. Here the conditional distribution of x2 at x1 = 0.75."}
...
M-Plots solve the problem of averaging predictions of unlikely data instances, but the estimation mixes the effects of two features.
ALE plots solve this problem by computing - also based on the conditional distribution of the features -  local effects in the form of differences in prediction instead of averages.
This means that for the effect of the living area at 30 square meters, we first condition on 30 m2 houses and instead of averaging their local predictions (M-Plots) we calculate the difference in prediction if these houses had a living area of for example 31 m2 minus if they had one of 29 m2.
This gives us the pure effect of the living area without mixing the effect with the effects of any correlated features.
...
The effects I show you could be due to feature xj, but also to some correlated features.  
...
All three methods reduce the function by averaging the effects of the other features, but they differ in whether averages of predictions or averages of *differences in prediction* are calculated and whether averaging is done over the marginal or conditional distribution.
...
This is the value of the prediction function f, at feature values $x_S$, averaged over all features in $x_C$.
Averaging means computing the marginal expectation E over the features in xc, which is the integral over the prediction weighted with the probability distribution.
Sounds fancy, but to calculate the expected value over the marginal distribution, we simply take all our data instances, force them to have a certain grid value for the features in xs, and average over the predictions for this manipulated dataset.
...
The only thing that changes compared to PDPs is that we take the average the prediction conditional on each grid value of the feature of interest.
In practice, this means that we have to define a neighbourhood and average over it, for example for the calculation of the effect of 30 square meters on the predicted house value, we could average the predictions of all houses between 28 and 32 square meters.

ALE plots average the change in the predicted outcome and accumulate it over the grid (more on the calculation later).
...
First, we average the change of predictions, not the prediction itself.
The change is defined as the gradient, but later, for the actual computation, replaced by the difference in the prediction over an interval.
...
Because it block the effects of correlated features.
The derivative is calculated at different grid values and averages the effects locally, thus avoiding the error that partial dependence plots make by averaging over the marginal distribution, which is so problematic when features are correlated.
It also avoids the mistakes M-Plots make in mixing the effect estimate with the effects of correlated features.
...
First I will describe how ALE plots are estimated for a single numerical feature and later for two numerical features and for a single categorical feature.
To estimate local effects, we divide the feature into many intervals and compute the differences in the prediction, as visualized in the [figure before with the intervals](#aleplot-computation).
...
At it's core, the ALE method calculates the differences in predictions, whereby we replace the feature of interested with grid values z.
The difference in prediction is the *Effect* (in ALE) the feature has for an individual instance in a certain interval.
The sum on the right sums over all instances within an interval, specified as neighbourhood N(k).
...
The left sum symbol means that we accumulate the average prediction differences across all intervals.
...
The value of the ALE can be interpreted as the effect of the feature at a certain  value compared to the average prediction of the data. 
For example, an ALE estimate of -2 at $x_j=3$ is, it means that when $x_j$ has value 2, then the prediction is lower by 2 compared to the average prediction.
...
Quantiles have the disadnvantage that the intervals can have very different lengths.
This can leat to some weird ALE plots if the feature of interest is very skewed, for example many low values and only a few very high values.
...
The prediction model does something weird at a combination of the two features for which we have never observed instances.
The following figure shows what this scenario looks like.


```{r correlation-problem, fig.cap = "Two features and the predicted outcome. The model simply predicts the sum of the two features (shaded background), with the exception that if x1 is greater than 0.7 and x2 less than 0.3, the model always predicts 2. This area is far from the distribution of data and doesn't affect the performance of the model and also shouldn't affect its interpretation."}
...
But if you suspect that your test distribution might be slightly different and some instances are actually in that range, then it would be interesting to include this aree in the calculation of feature effects. 
...
ALE plot can also represent the interaction of two features.
...
In the example in the previous figure, many cells are empty due to the correlation.
...
PDPs always show the total effects, ALE plots show the first- or second-order effect.
These are design decisions of the ones who implement the methods, and does not depend on the underlying math or so
...
The accumulated local effects could also be calculated for arbitrarily higher orders (combinations of more than two features), but as argued in the [PDP chapter](#pdp), only up to 2D makes sense, because higher interactions can't be visualized meaningfully.
...
One solution is to order the categories according to the similarity of the categories when looking at the other features.
...
We train a regression tree to predict the number of rentals on a given day and use ALE plots to analyze how temperature, relative humidity and wind speed influence the predictions.
...
Instead, I train a linear model where, for example, temperature is the outcome to predict and one of the features is the input feature.
...
But this model-based approach of 'variance-explained' (also called ANOVA, which stands for ANalysis Of VAriance) works even if the second feature is categorical. 
...
We calculate the explained variance of temperature, humidity and wind speed with all the other features to see how how big of a problem we might have when using partial dependence plots. 
...
```{r ale-bike-cor, fig.cap = "The strength of the correlation between temperature, humidity and wind speed with all the features, measured as the amount of variance explained, when we train a linear model with, for example, temperature as outcome to predict and season as feature. For the temperature we observe - not surprisingly - a high correlation with the season and the month. The humidity correlates with the weather situation."}
...
When we compute a partial dependence plot for the temperature, we estimate the model's average prediction for any temperature grid point (e.g. -5 degree Celsius) across all instances.
This means that we also use instances with season 'Summer' and month 'August', replace their temperature with -5 degrees, get the model prediction, use it for calculating the mean and pretend everything is legit.
However, -5 degrees in August is highly unlikely (at least in the Northern Hemisphere) and might bias the estimate of the effect of temperature.
Well, let's see how partial dependence plots estimates the effects.

```{r pdp-bike-compare, fig.cap = "Partial dependence plots for temperature, humidity and wind speed. Compared to the ALE plots, the PDPs show a smaller decrease in predicted number of bikes for high temperature or high humidity. The PDP uses all data instances to calculate the effect of high temperatures, even if they are, for example, instances with the season 'winter'. The ALE plots are more reliable."}
...
This means that, for example, you won't see the main effect that high humidity leads to a lower number of predicted bikes on average in the ALE 2D plot.
...
```{r pdp-bike-vs-ale-2D, fig.cap = "Partial dependence plot of the total effect of temperature and humidity on the predicted number of bikes. The plot combines the main effect of each of the features and their interaction effect, as opposed to the 2D-ALE plot which only shows the interaction."}
...
If you are only interested in the interaction, you should look at the second-order effects, because the total effects mix the main effects into the plot.
But if you want to know the combined effect of the features, you should look the total effects (like the PDP shows).
For example, if you want to know the expected number of bike rents at 30 degrees Celsius and 80 percent humidity, you can read it directly from the 2D PDP. 
If you want to know the same about ALE plots, you need to look at three plots: 
...
In a scenario where two features have no interaction, the 2D total effects plot could be misleading because it represents a complex landscape, but it is simply the product of the to main effects.
...
The ALE plot shows similar interactions between age and number of pregnancies as the [partial dependence plots](#pdp).
Age and number of pregnancies are naturally correlated, with a correlation of `r sprintf("%.2f", cor(cervical$Age, cervical$Num.of.pregnancies))`.
This means that I would trust the ALE plot more than the partial dependence plot.
...
For 20 grid points, PDPs require 20 times more predictions than the worst case ALE plot where each interval has only one instance.
- The **interpretation of ALE plots is clear**: Conditional on a given x value, the effect of the feature on the prediction can be read from the ALE plot. 
...
If the number is too small, the ALE plots are not be very accurate. 
...
This happened in the [cervical cancer prediction ALE plot](# ale-cervical-1D) for high ages and for years with hormonal contraceptives.
...
Another alternative is the Marginal plot (M-Plot), which is interesting for historical reasons.
