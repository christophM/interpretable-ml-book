...
\begin{align*}
\hat{f}_{S,PDP}(\mathbf{x}_S)&=\mathbb{E}_{X_C}\left[\hat{f}(\mathbf{x}_S,X_C)\right] \\
...
For the weather situation (right), we see that especially bad weather has a strong negative effect on number of rented bikes.
