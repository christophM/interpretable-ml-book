...
We train a regression tree to predict the number of rented bicycles on a given day and use ALE plots to analyze how wind speed and weather influence the predictions.

@fig-ale-bike (left) shows that an increasing wind speed has a negative effect on bike rentals.
For the weather situation, we see that especially bad weather has a strong negative effect on number of rented bikes.
...
ale3 = FeatureEffect$new(pred.bike, "windspeed", method = "ale")$plot() +
  ...
  scale_y_continuous("ALE of predicted bike rentals", limits=limits)
...
Next, we consider effects of humidity and temperature and their interaction on the predicted number of bikes.
...
The plot reveals an interaction between temperature and humidity: Cold and humid weather increases the prediction.
...
ale_temp = FeatureEffect$new(pred.bike, feature = "temp", method = "ale")$plot() +   
  scale_x_continuous("Temperature")
ale_hum = FeatureEffect$new(pred.bike, feature = "hum", method = "ale")$plot() +   
...
(ale_hum | plot_spacer()) /
(ale_interact + ale_temp) +
plot_layout(guides = "collect") # Ensures consistent legends
```
