...
&=\int_{X_C}\hat{f}(x_S, X_C)d\mathbb{P}(X_C|X_S = x_S)
\end{align*}
...
= & \int_{z_{0,S}}^{x_S}(\int_{x_C}\hat{f}^S(z_s,X_c)d\mathbb{P}(X_C|X_S = z_S))dz_S-\text{constant}
\end{align*}
...
If there is a meaningful order to the categories of a categorical feartures, use this order for ALE.

:::

## ALE where PDP disappoints
...
```{r}
#| label: ale-bike-cat
#| fig.cap: ALE plot for the categorical feature weather situation. The weather situations are ordered by their similarity to each other, based on the distributions of the other features by weather situation. We observe that "GOOD" has a positive effect, while "MISTY" and especially "RAIN/SNOW/STORM" have negative effects.
...
We train a random forest to predict the [penguin sex](#penguins) based on body measurements.
We visualize the accumulated local effects for two of the features:

```{r}
#| label: fig-ale-penguin
#| fig.cap: The heavier the penguin, the less likely it is female. We see a similar pattern for bill depth, but the probability doesn't go to zero for deep bills.
#| fig.height: 4
pred.penguins = Predictor$new(pengu_rf, data = penguins, class="female")
ale = FeatureEffect$new(pred.penguins, "bill_depth_mm", method = "ale", grid.size = 20)

p1 = ale$plot() +
  scale_x_continuous("Bill depth in millimeters") +
  scale_y_continuous('P(female)', limits = c(-0.45, 0.45))
ale$set.feature("body_mass_g")
p2 = ale$plot() +
...
```{r}
#| label: fig-ale-penguin-by-species
#| fig.cap: ALE by penguin species. A more nuanced interpretation emerges. For Adelie, more weight means less likely to be female. For Chinstrap the weight doesn't affect P(female). Gentoo penguins are in general heavier, but also show the "males are more heavy" pattern.
#| fig.height: 4
...
ggplot(ale_data) +
  ...
  scale_x_continuous("Body mass in grams") +
  scale_y_continuous("ALE for P(female)")
```



Next, we look at the interaction between body mass and bill length.

```{r}
#| label: fig-ale-penguins-2d
...
p = FeatureEffect$new(pred.adelie, c("bill_depth_mm", "bill_length_mm"), method = "ale")
p$plot() + 
  geom_point(data=penguins[penguins$sex =="female",]) +
  geom_point(data=penguins[penguins$sex =="male",], color="grey") +
  scale_fill_viridis("ALE of P(female)", option = "D") +
  scale_x_continuous("Bill depth in millimeters") +
