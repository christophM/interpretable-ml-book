...
We could average over the conditional distribution of the feature, meaning at a grid value of $X_1$, we average the predictions of instances with a similar $x^{(i)}_1$ value.
...
@fig-aleplot-computation provides intuition how ALE plots are calculated.
...
=& \int_{\mathbf{z}_{0,S}}^{x_S}\left(\int_{x_C}\hat{f}^S(z_S,X_C)d\mathbb{P}(X_C|X_S = z_S)\right) d \mathbf{z}_S-\text{constant}
\end{align*}
...
$$\hat{\tilde{f}}_{j,ALE}(x) = \sum_{k=1}^{k_j(x)} \frac{1}{n_j(k)} \sum_{i: x_j^{(i)} \in N_j(k)} \left[\hat{f}(z_{k,j}, \mathbf{x}^{(i)}_{-j}) - \hat{f}(z_{k-1,j}, \mathbf{x}^{(i)}_{-j})\right]$$
...
If you are interested in the calculation, I refer you to the paper [@apley2020visualizingeffects], formulas (13) -- (16).
...
If there is a meaningful order to the categories of a categorical features, use this order for ALE.
