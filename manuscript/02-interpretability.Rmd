...
Let's dig deeper and discover what we humans accept as "good" explanations and what the implications are for interpretable machine learning.
...
When you build the explanation system to get ALL factors for a particular prediction or behavior, you do not want an especially human-friendly explanation, but a complete causal attribution.
...
In all other cases, where lay people or people with little time are the recipients of the explanation, follow the advice here.
...
- Why didn't the treatment work on the patient?
- Why was my loan rejected?
- Why haven't we been contacted by alien life yet?

The first two questions can be answered with an "everyday"-explanation, while the third comes from the category "More general scientific phenomena and philosophical questions".
We focus on the "everyday"-type explanation, as it is relevant to interpretable machine learning.
Questions that start with "how" can usually be rephrased into "why" questions:
"How was my loan rejected?" can be turned into "Why was my loan rejected".


In the following, the term "explanation" refers to the social and cognitive process of explaining, but is also the product of these processes.
...
This chapter further condenses Miller's summary on "good" explanations and adds concrete implications for machine learning applications.
...
For a house price prediction, the house owner might be interested in why the predicted price was high compared to the lower price she had expected.
If my loan application is rejected, I don't care to hear all the factors that generally speak for or against a rejection.
...
A physician might ask: "Why did the drug not work for my patient?".
And she might want an explanation that contrasts her patient with a patient where the drug worked and who is similar to the non-responding patient.
Contrastive explanations are easier to understand than complete explanations.
A complete explanation of the physician's question why the drug doesn't work might include:
...
Humans don't want a complete explanation for a prediction, but want to compare what the differences were to another instance's prediction (could be an artificial one).
...
A user of a house price prediction website might want to have an explanation of a house price prediction contrastive to her own house or maybe to another house on the website or maybe to an average house in the neighbourhood.
...
"The decline in stock prices is blamed on a growing backlash against the companies product due to problems with the latest software update."   
...
Students who can't answer these question will fail the course.
Students can have different levels of preparation, which translates into different probabilities for correctly answering the teacher's question (if she decides to test the student).
...
A student who did not study (10% chance to pass the question part) was not one the lucky ones and gets additional questions that he fails to answer correctly.
...
I would argue that now, the better explanation is that the teacher tested the student because it was unlikely that the teacher would test.
The teacher behaved abnormally.
**What it means for interpretable machine learning**:
...
Even if some attribution method finds that the three balconies contribute as much to the price difference as the above average house size, the good neighbourhood or the recent renovation, the abnormal feature "two balconies" might be the best explanation for why the house is so expensive.
...
An explanation that selects only one or two possible causes can never cover the entire list of relevant causes.
...
So if we say that a second balcony increases the price of a house, then that also apply to other houses (or at least to similar houses).
...
This is difficult to integrate into machine learning and would probably drastically compromise predictive accuracy.
Our prior belief for the effect of house size on predicted price is that the larger the house, the higher the price.
Let's assume that a model also shows negative effect of house size on the predicted price for a few houses.
The model has learned this because it improves predictive performance (due to some complex interactions), but this behavior strongly contradicts prior beliefs.
One thing you can do is enforce monotonicity constraints (a feature can only affect the prediction in one direction) or use something like a linear model that has this property.
...
(Joe is a librarian. Is he more likely to be shy or to be a shy person who likes to read books?).
