...
This works best when the class of models used was already evaluated by someone else in a human level evaluation. 
...
The next chapter focuses on function level evaluation of explanations for individual predictions.
...
To achieve this, we rely on an explanation method that is the algorithm that generates explanations.
...
For example, we could predict cancer risk using a support vector machine and explain predictions using the [local surrogate model method](#lime), which generates decision trees as explanations.
Or we could use a linear regression model instead of a support vector machine.
The linear regression model is already equipped with an explanation method (interpretation of the parameters in the weighted sum).
...
- **Expressive Power** is the "language" or structure of the explanations the method generates.
The language could be IF-THEN rules, decision trees, a weighted sum, natural language, etc..
...
- **Portability** describes the range of machine learning models which which the explanation method can be used.
Methods with a low translucency have a higher portability because they treat the machine learning model like a black box.
[Local-surrogate models](#lime) might be the explanation method with the hightes portability.
Methods that only work for e.g. Recurrent Neural Networks have low portability.
- **Algorithmic Complexity** describes the computational complexity of the algorithm that generates the explanation.
This property is important to consider when computating time is an important factor in generating explanations.
...
High fidelity is one of the most important properties of an explanation, because with low fidelity the explanation is useless to explain the machine learning model.
...
If the explanations are very similar, the explanation method has high consistency.
...
A lack of stability can also be caused by non-deterministic components of the explanation method, such as a data sampling step, like  the [local surrogate models method](#lime) has.
High stability is always desirable.
- **Comprehensibility**: How well do humans understand the explanations?
This look just like one more property among many, but it's the elephant in the room. 
...
Ideas for measuring comprehensibility include measuring the size of the explanation (number of features with a non-zero weights in a linear model, number of decision rules, ...) or testing how well people can predict the behaviour of the black box model from the explanations.
...
Many machine learning models only give predictions without a statement about the model's confidence that the prediction is correct.
...
Explanations can cover the entier model (e.g. interpretation of weights in a linear regression model) or represent only an individual predictions (e.g. with the [Shapley value, a game theoretical attribution method](#shapley)).
