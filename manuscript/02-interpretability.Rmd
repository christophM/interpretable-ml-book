...
If a machine learning model performs well, **why don't we just trust the model** and ignore **why** it made a certain decision?
...
- Fairness: Ensuring that predictions are unbiased and don't implicitly or explicitly discriminate against protected groups.
...
The system can only be gamed if the inputs are proxies for a causal feature, but don't actually cause the outcome.
...
Model-agnostic tools can be used on any machine learning model and are applied after training of the model (post hoc).
...
Because it would mean drawing the estimated hyperplane mentally in a 5-dimensional space.
