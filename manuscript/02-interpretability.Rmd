...
A (non-mathematical) definition of interpretability that I like is given by Miller (2017)[^Miller2017]:
...
A model is more interpretable than another model if its decisions are easier for a human to comprehend.
I will use the terms interpretable and explainable interchangeably.
...
Some models may not require explanations  because they are used in a low-risk environment, meaning a mistake will not have serious consequences, (e.g. a movie recommender system). It could also be that the method has already been extensively studied and evaluated (e.g. optical character recognition).
...
For instance, most people don't need to understand how a computer works.
However, unexpected events make us curious.
...
"The dog was stressed and hence it lashed out by biting you."
...
Advertising follows me on the Internet because I recently bought a washing machine, and I know that for the next few days I will be followed by advertisements for washing machines.
It makes sense to suggest gloves if I already have a winter hat in my shopping cart.
The algorithm recommends a particular movie because users who liked other movies I liked also enjoyed the recommended movie.
...
A good example is a machine learning model for optical character recognition that processes envelop images and extracts addresses.
These systems have been used for several years and it is clear that they work. 
...
Ideally, models would only use causal features because these are not gameable.
...
Which features are important and what kind of interactions take place between them?
...
If you look at an individual prediction, the behavior of an otherwise complex model might make more sense.
...
But if you are looking at only one particular house of 100 square meters, there is a possibility that for that subset pf data, your model prediction depends linearly on the size. 
...
What are the relevant properties of explanations that we should consider for evaluation?
...
"How could the bank reject my loan?" can be turned into "Why was my loan rejected?".
...
For house price prediction, the house owner might be interested in why the predicted price was high compared to the lower price they had expected.
