...
Another take is: **Interpretability is the degree to which a human can consistently predict the model's result** [^kim].
...
```{r doge-stuck, fig.cap="Doge, my vacuum cleaner got stuck. As an explanation for the accident, Doge told me that it needs to be on a flat surface.", out.width=800}
...
Many interpretability methods provide a kind of summary statistic of how each feature affects the model predictions. 
These can be feature importance measures or statistics about the interaction strength between features.
- **Feature summary visualization**: 
Most feature summary statistics can be visualized. 
However, some feature summaries can only be visualized and not meaningfully be placed in a table. 
The partial dependence of a feature is such a case. 
For non-linear relationships, partial dependence plots are arbitrary curves showing a feature and the average predicted outcome.
- **Model internals (e.g. learned weights)**: 
The interpretation of intrinsically interpretable models falls under this category. 
Examples are the weights in linear models or the learned tree structure (which features and feature values are used for the splits?) of decision trees. 
The lines are blurred between model internals and feature summary statistic in, for example, linear models, because the weights are both model internals and at the same time summary statistics for the features. 
Another method that outputs model internals is the visualization of feature detectors that are learned in convolutional neural networks. 
Interpretability methods that output model internals are model-specific by definition (see next point). 
- **Data point**: 
This category includes all methods that return data points (can be existing or newly created) to make a model interpretable. 
Counterfactuals, for example: To explain the prediction of a data point, find a similar data point by changing some of the features for which the predicted outcome changes in a relevant way (like a flip in the predicted class). 
Another example is the identification of prototypes of predicted classes. 
Interpretability methods that output new data points require that the data points themselves can be interpreted. 
This works well for images and text, but is less useful for tabular data with hundreds of features.
- **Intrinsically interpretable model**: 
This is a little circular, but one solution to interpreting black box models is to approximate them (either globally or locally) with an interpretable model. 
The interpretable model themselves are interpreted by internal model parameter or feature summary statistics.
...
An algorithm trains a model, which produces the predictions. 
...
But if you only look at a specific apartment of 100 square meters and check how the price changes by going up and down by 10 square meters, there is a chance that this subregion in your data space is linear. 
Local explanations can be more accurate compared to global explanations because of this.
...
## More on Function Level Evaluation: Properties of explanations

In this chapter we take a closer look at properties of explanations for individual predictions and properties of the methods that produce them (Robnik-Sikonja, Marko, and Marko Bohanec, 2018 [^human-ml]).

**Properties of explanation methods (methods that produce explanations)**

- **Expressive Power** is the "language" or structure of the explanations the method produces.
This can be IF-THEN rules, decision trees, a weighted sum (e.g. linear regression model), natural language, etc.
- **Translucency** describes how much the explanation method relies on looking inside the machine learning model.
Model-agnostic have a low translucency, model-specific methods have a very high translucency.
Depending on the scenario, different translucency might be desirable.
The advantage of high translucency is that the method has more information to produce explanations.
The advantage of low translucency is that the explanation is more portable.
- **Portability** describes how big the range of models is that the method covers.
As such it is related to translucency. 
Methods with a low translucency (i.e. they treat the model as black box) have a higher portability.
For example [local-surrogate models](#lime) have one of the highest portability.
Methods that only work for e.g. Recurrent Neural Networks have a low portability.
- **Algorithmic Complexity** describes the computational complexity of the algorithm that produces the explanation.
This property is important to look at when the computation time for producing explanations is an important factor.

**Properties of explanations (of individual predictions)**

- **Accuracy**: How well does the explanations predict unseen data?
High accuracy is especially important when the explanation might be used for making predictions in place of the black box model. 
A low accuracy can be ok when the accuracy of the black box model is low as well, and when the goal is to explain what the black box model does.
In that case, fidelity matters more.
- **Fidelity**: How well does the explanation approximate the predictions of the black box model?
High fidelity is one of the most important properties of an explanation, because with a low fidelity, the explanation is useless in explaining the machine learning model.
Accuracy and fidelity are closely related.
When the black box model has a high accuracy and the explanation a high fidelity, the explanation will also have high accuracy.
Some explanations only offer local fidelity, meaning that the explanation only approximates the model prediction well for a subset of the data (e.g. [local surrogate models](#lime)) or even for only a data instance (e.g. [Shapley Values](#shapley)).
- **Consistency**: How much do explanations vary across models trained on the same task with similar predictions?
For example, I train an SVM and a decision tree for predicting the same task which produce very similar predictions.
I compute explanations and analyse how different they are.
If the explanations are very similar, the method for creating explanations has a high consistency.
On that account, model-agnostic methods should, in theory, have a higher consistency then model-specific methods (= with higher translucency).
I find this property a bit tricky, because the two models might e.g. use different features, but get similar predictions (also called ["Rashomon Effect"](https://en.wikipedia.org/wiki/Rashomon_effect)). 
In this case, a high consistency is not desirable because IMHO the explanations have to be very different.
High consistency is desirable when the models truly rely on similar relationships.
- **Stability**: How similar are explanations for similar instances?
While consistency compares explanations across models, stability compares explanations for only one model.
High stability means that slight variations of the features of an instance doesn't change the explanation much (unless these slight variations also change the prediction a lot).
A lack of stability can be the result of high variance of the explanation method, i.e. the explanation method is strongly affected by slight changes of the predictions or the features of the instance to be explained.
A lack of stability can also come from random elements of the explanation method, like a data sampling step, like [local surrogate models](#lime) have.
High stability is always desirable.
- **Comprehensibility**: How well do humans understand the explanations?
This look just like one more property among many, but it's one of the most important and certainly the most undefined.
And it certainly is difficult to measure.
Many people agree that comprehensibility depends on the audience.
Ideas to measure comprehensibility include measuring the size of the explanation (number of features with non-zero weights in a linear model, number of decision rules, ...) or testing how well humans can predict the behaviour of the black box model based on the explanations.
Also the comprehensibility of the features used in the explanation should be considered, e.g. complex transformation of features might be less comprehensible than the original features.
- **Certainty**: Are the explanations reflecting the certainty of the machine learning model?
Most prediction models only output predictions without a statement of confidence or certainty.
When the model predicts a 40% chance of cancer for one patient, is it with the same confidence as the 40% another patient got?
An explanation that reports that certainty is highly useful.
- **Degree of Importance**: How well does the explanation reflect the importance for features or parts of the explanation?
For example, when a decision rule is produced as an explanation for an individual prediction, is it clear which of the conditions of the rule was the most important one?
- **Novelty**: Does the explanation reflect whether a data instance to be explained comes from a "new" region, one that is far away from the distribution of the training data?
For such instances, the model might be inaccurate and, as a consequence, the explanation would be useless.
The concept of novelty is related to the concept of certainty.
The higher the novelty, the more likely it is that the model has a low certainty, because of a lack of data.
- **Representativeness**: How representative is an explanation of the model?
Explanations might cover the whole model (e.g. interpretation of weights in a linear regression model) or the might only represent an individual predictions (e.g. with the [Shapley value, an attribution method based on game theory](#shapley)).
...
[^kim]: Kim, B., Khanna, R., & Koyejo, O. (2016). Examples are not Enough, Learn to Criticize! Criticism for Interpretability.
