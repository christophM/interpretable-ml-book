...
**Properties of explanations (expressions that explain individual predictions)**

- **Accuracy**: How well does the explanations generalise to unseen data?
This is especially interesting for interpretable models, because there is usually a tradeoff between accuracy and interpretability.
High accuracy is important especially when the explanation might be used for predictions instead of the black box model. 
But it might be ok if the accuracy is low, when the goal is to explain the black box model, which might have a low accuracy. 
Because then fidelity matters more.
- **Fidelity**: How well does the explanation approximate the predictions of the black box model?
There is a relation between accuracy and fidelity: When the black box model is highly accurate, and the explanation has high fidelity, then the explanation will also have high accuracy.
Often local fidelity is enough for an explanation of an individual explanation, i.e. it doesn't have to cover the entire model.
- **Consistency**: How much does the explanation vary across models trained on the same task with similar predictions?
I am not sure if I understand this one correctly. 
Different models might focus on different features, but get similar predictions (the [Rashomon Effect](https://en.wikipedia.org/wiki/Rashomon_effect)). 
Then the explanations should IMHO also be very different, hence having a low "consistency".
High consistency is desirable when the models really find similar relationships.
- **Stability**: How similar are explanations for predictions of similar instances from the same model?
...
This look just like one more property, but it's actually THE 
This highly depends on the audience. 
Could be measured bt the size of the explanation (number of features with non-zero weights in a linear model, number of decision rules, ...).
Another dimension of comprehensibility can be monotonicity: 
Is the effect of a feature on the prediction always in the same direction?
...
But that's in general a term not agreed upon and matter of ongoing research.
- **Certainty**: Are the explanations reflecting the certainty of the machine learning model?
Most models only output their predictions without a confidence or certainty measure.
...
For example, when a decision rule is produced as an explanation for an individual prediction, is it clear which one of the conditions  of the rule was the most important?
- **Novelty**: Does the explanation reflect if a data instance to be explained comes from a new region, which is different from the training data?
Because for such instances the model might be inaccurate and also the explanation not useful.
The concept of novelty is related to the concept of certainty.
The higher the novelty, the more likely it is that the model has a low certainty.
- **Representativeness**: How representative is the explanation of the model?
For linear regression models, when we use the weighted sum as explanation, it covers the whole model.
For more complex models, explanations might only cover one instance or a small subset of data, and not be valid for the whole model.

I would add one more to the list:
- **Feature Complexity**: What features were used for the explanation? 
PCA components are harder to understand than word occurrences, for example.
