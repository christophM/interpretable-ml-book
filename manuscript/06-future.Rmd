...
Solutions already exist and I argue that it will be sufficicient for many applications to use these automated machine learning services. 
...
The raw data itself is always useless (I exagerate on purpose). 
...
So why switch from analyzing assumption-based, transparent models to assumption-free black box models?
...
Assumption-based models typically have worse predictive performance with untouched test data than black box machine learning models.
The approach of machine learning is more attractive: 
We don't make assumptions, we just let it approximate reality as closely as possible (while avoiding overfitting on the training) and BAM! we have our model of reality.
...
A credit default program that explains to a bank employee why a credit application was rejected ("Applicant has too many credit cards and is employed in and unstable job.");
A robot arm that explains why it moved the cassis from the conveyor belt into the trash bin ("The item has a craze at the bottom.").
