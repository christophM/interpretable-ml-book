...
For a temperature of `r round(as.numeric(as.character(df["temp", "value"])), 1)`C, the effect is `r round(as.numeric(as.character(df["temp", "value"])) * lm_summary_print[as.character(df["temp", "feature"]), "Estimate"], 1)`.
...
$$min_{\boldsymbol{\beta}}\left(\frac{1}{n}\sum_{i=1}^n(y^{(i)}-x^{(i)}^T\boldsymbol{\beta})^2\right)$$

Lasso adds a term to this optimization problem.

$$min_{\boldsymbol{\beta}}\left(\frac{1}{n}\sum_{i=1}^n(y^{(i)}-xîs{(i)}^T\boldsymbol{\beta})^2+\lambda||\boldsymbol{\beta}||_1\right)$$
...
The big drawback is that it cannot be automated and need be an expert.
...
If you take both features into a linear model, it might happen, that the size of the house is the better predictor and get's a large positive weight.
