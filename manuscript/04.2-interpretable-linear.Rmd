...
One of the values counts as the reference level (in some programming languages encoded with 0), such as "No garden".
Changing the feature $x_{j}$ from the reference level to the other level changes the estimated outcome by $\beta_{j}$.
- Categorical feature with multiple levels:
A feature with a fixed number of possible values.
An example is the feature "Floor type", with possible levels "carpet", "laminate" and "parquet".
A solution to deal with many levels is the one-hot-encoding, meaning that each level has its own binary column.
For a categorical feature with l levels, you only need l-1 columns, because the l-th column would have redundant information (e.g. when columns 1 to l-1 all have value 0 for an instance, we know that the instance has category l).
The interpretation for each level is then the same as the interpretation for binary features.
...
For an instance with all numerical feature values at zero and the categorical feature values at the reference level, the estimated outcome is the intercept weight $\beta_{0}$.
...
Changing feature $x_{k}$ from the reference level to the other category increases the expectation for y by $\beta_{k}$, given all other features remain fixed.
...
The categorical feature effects can be summarized in a single boxplot, compared to the weight plot, where each level has its own row.
...
The more nonlinearities and interactions exist, the less accurate the linear model becomes and the less truthful explanations it will produce.
