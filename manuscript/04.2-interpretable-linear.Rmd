...
To get the feature effects of this instance, we have to multiply its features values with the respective weights from the linear regression model.
...
We add those individual effects as points in the effect plot, which shows us the distribution of effects in the data.
...
When we average the predictions for the training data instances, we get `r round(predictions_mean, 0)` (). 
Compared to that, the prediction of the `r i`-th instance is small with only `r round(pred_i, 0)` predicted bikes.
The effect plot reveals the reason.
The boxplots show the distributions of the effects for all instances from the dataset, the red marks the effects for the `r i`-th instance.
The `r i`-th instance has a low temperature effect, because on this day the temperature was `r round(X[i, 'temp'],0)` degrees, which is low compared to most other days (and remember, the weight of the temperature feature is positive).
