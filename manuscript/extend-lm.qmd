...
$$y^{(i)} = \beta_{0} + \beta_{1} x^{(i)}_{1} + \ldots + \beta_{p} x^{(i)}_{p} + \epsilon^{(i)}$$

The linear regression model assumes that the outcome $y^{(i)}$ of an instance can be expressed by a weighted sum of its $p$ features with an individual error $\epsilon^{(i)}$ that follows a Gaussian distribution.
...
$$g(\mathbb{E}[Y|\mathbf{x}])=\beta_0+\beta_1 x_{1}+\ldots+\beta_p x_{p}$$
...
$$\ln(\mathbb{E}[Y|X = \mathbf{x}])=\mathbf{x}^{T}\boldsymbol{\beta}$$
...
$$\mathbf{x}^{T}\boldsymbol{\beta}=\ln\left(\frac{\mathbb{E}[Y|X = \mathbf{x}]}{1-\mathbb{E}[Y|X = \mathbf{x}]}\right)=\ln\left(\frac{\mathbb{P}(Y=1|X = \mathbf{x})}{1-\mathbb{P}(Y=1|X = \mathbf{x})}\right)$$
...
#| tbl-cap: "Results of the Poisson model: Weights, exp(Weights), and 95% confidence intervals."
...
interactions::interact_plot(mod, pred = "temp", modx = "workday") + theme(legend.position="bottom")
```
...
kableExtra::kbl(head(model.matrix(mod.gam)), digits = 2, booktabs = TRUE)
```
