...
```{r}
#| label: data-frame-lm-no-interaction
mod = lm(1:4 ~ ., data = x)
...
If we want the linear model to consider the interaction between temperature and the workday feature, we have to add a column for the interaction:
...
This new feature column is zero for an instance if the work feature is at the reference category ("N" for no working day), otherwise it assumes the values of the instance's temperature feature.
...
Here's some artificial data containing working day (work) and a categorical weather feature (wthr):

```{r}
#| label: data-frame-lm-cat
...
The temperature slope for days on which people do not have to work ('NO Y') can be read directly from the table (`r round(lm_summary_print['temp','Estimate'], 1)`).
...
The intercept of the 'NO Y'-line at temperature = 0 is determined by the intercept term of the linear model (`r round(lm_summary_print['(Intercept)','Estimate'], 1)`).
The intercept of the 'Y'-line at temperature = 0 is determined by the intercept term + the effect of working day (`r round(lm_summary_print['(Intercept)','Estimate'], 1)` + `r round(lm_summary_print['workdayY','Estimate'], 1)` = `r round(lm_summary_print['(Intercept)','Estimate'], 1) + round(lm_summary_print['workdayY','Estimate'], 1)`).
