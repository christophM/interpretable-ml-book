...
There's a solution to all these problems:
...
*"Hey, let us model this data with a linear model ...
Oh it does not work ...
...
@fig-poisson-model-params lists the estimated weights and exp(weights) together with the 95% confidence interval:

```{r}
#| label: fig-poisson-model-params
...
cap = "Results of the Poisson model: Weights, exp(Weights), and 95% confidence intervals."
kableExtra::kbl(cc[c("beta", "ci")], col.names = c("weight", "exp(weight) [2.5%, 97.5%]"), digits = 2, booktabs = TRUE, caption = cap) %>%
...
To predict the [number of bicycles](#bike-data) rented, there may be an interaction between temperature and whether it is a workday or not.
...
When it comes to rental bicycles, you might expect an interaction between temperature and work day.
...
#| tbl-cap: Work day and temperature features for 4 days. 
...
I'm shorting the weather categories here to narrow the table a bit: G=GOOD, B=BAD, and M=misty.

```{r}
#| label: tbl-data-frame-lm-cat
cap = "Data example for bike data with features workday and weather for 4 days."
x = data.frame(work = c("Y", "N", "N", "Y"), wthr = c("B", "G", "M", "B"))
kableExtra::kbl(x, booktabs = TRUE, caption = cap) %>% kable_styling(position = "center")
```
...
#| fig-cap: Corresponding model matrix for @tbl-data-frame-lm-cat.
...
This results in tbl-example-lm-interaction estimated weights and confidence intervals.
...
cap = "Linear regression estimates for bike data including interaction between workday and temperature."
tab = kableExtra::kbl(cbind(lm_summary_print[,c('Estimate', 'Std. Error')], confint(mod)), digits = 1, col.names = c('Weight', 'Std. Error', "2.5%","97.5%"), booktabs = TRUE, caption=cap) %>% kable_styling(position = "center")
tab
```
...
By the way, the data it's debatable whether the data is IID, because days that are close to each other are not independent of each other.
...
Does the temperature have a negative effect given it is a work day?
...
"While leaving all other feature values unchanged, increasing the interaction effect of temperature for work day decreases the predicted number of bikes."
But the interaction effect only adds to the main effect of the temperature.
Suppose it is a work day and we want to know what would happen if the temperature were 1 degree warmer today.
...
interactions::interact_plot(mod, pred = "temp", modx = "workday")
```
...
@fig-nonlinear-effects shows the estimated slope with: the standard linear model, a linear model with transformed temperature (logarithm), a linear model with temperature treated as categorical feature and using regression splines (GAM).
The linear model (top left) does not fit the data well. One solution is to transform the feature with e.g. the logarithm (top right), categorize it (bottom left), which is usually a bad decision, or use Generalized Additive Models that can automatically fit a smooth curve for temperature (bottom right).
...
The problem with this approach is that it needs more data, it is more likely to overfit, and it is unclear how to discretize the feature meaningfully (equidistant intervals or quantiles?
how many intervals?).
...
In the example where we are predicting the number of bicycles with a GAM using only the temperature, the model feature matrix looks like in @tbl-splines.
...
cap = "Feature matrix when using splines."
kableExtra::kbl(head(model.matrix(mod.gam)), digits = 2, booktabs = TRUE, caption=cap) %>%
  kable_styling(position = "center")
```

@fig-splines shows how these spline basis functions look like.

```{r}
#| label: fig-splines
#| fig.cap: "4 spline functions for temperature. Each temperature value is mapped to (here) 4 spline basis values. If an instance has a temperature of 30 °C, the value for the first spline basis feature is -1, for the second 0.7, for the third -0.8 and for the 4th 1.7."
...
cap = "Weights estimated for the spline features when training the model."
kableExtra::kbl(coef(mod.gam), digits = 2, col.names = "weight", booktabs = TRUE) %>%
  kable_styling(position = "center")
```
...
As advantage I've said that linear models live in their own universe.
