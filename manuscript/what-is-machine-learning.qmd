...
For a credit default predictor, you need past data repayed and defaulted loans, and data that will help you make predictions, such as income, past credit defaults, and so on.
...
Step 3: Use the model with new data.
...
Once implemented, a machine learning model can complete a task much faster than humans, deliver consistent results, and can be copied infinitely.
Replicating a machine learning model on another machine is fast and cheap.
Training a human for a task could take decades (especially when they are young) and could be very costly.
A major disadvantage of using machine learning is that insights about the data and the task that the machine solves are hidden in increasingly complex models.
...
More complex models such as boosted trees and deep neural networks and ensembles of models often win machine learning competitions. 
...
![A learner learns a model from labeled training data. The model is used to make predictions.](./images/learner.jpg)
...
The opposite of a black box is sometimes referred to as **White Box**, and is called [interpretable model](#simple) in this book.
...
The **Prediction** is the machine learning model "guess" for the target value based on the given features.
