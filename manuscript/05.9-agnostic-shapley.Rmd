...
- Required: Number of iterations M, instance of interest x, feature index j, data matrix X, and machine learning model f
  - For all m = 1,...,M:
    ...
    - Order instance x: $x_o=(x_{(1)},\ldots,x_{(p)})$
    - Order instance z: $z_o=(z_{(1)},\ldots,z_{(p)})$
    - Find the position $i$ of the j-th feature in $x_o$
    - Construct two new instances
        - With the j-th feature: $x_{+j}=(x_{(1)},\ldots,x_{(i-1)},x_{(i)},z_{(i+1)},\ldots,z_{(p)})$
        - Without the j-th feature: $x_{-j}=(x_{(1)},\ldots,x_{(i-1)},z_{(i)},z_{(i+1)},\ldots,z_{(p)})$
