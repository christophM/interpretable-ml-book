...
We use the Shapley value to analyze the predictions of a random forest model predicting [penguin species](#penguins):
...
```{r shapley-penguins-plot, fig.cap = sprintf("Shapley values for a penguin in the penguins dataset. With a prediction of %.2f, this penguin's probability of being Adelie is %.2f above the average prediction of %.2f. The short bill length contributed most to the probability of being an Adelie penguin. The sum of contributions yields the difference between actual and average prediction (%.2f).", actual.prediction,diff.prediction, avg.prediction, avg.prediction)}
