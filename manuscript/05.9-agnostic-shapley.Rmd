...
```{r shapley-instance, fig.cap = "The predicted price for our apartment is 300,000€. It is a 50 square meter apartment on the second floor. There is a park nearby and cats are forbidden. Our goal is to explain how each of these features values contributed towards the predicted price of 300k€.", out.width=500}
...
Since we do not have the $\beta$'s from a linear equation in other model types, we need a different solution.
...
The Shapley value of a feature value $x_{ij}$ is it is contribution to the payed outcome, weighted and summed over all possible feature value combinations:
