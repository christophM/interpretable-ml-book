...
Each x~j~ is a feature value, with j = 1,...,p.
The $\beta_j$ are the weights corresponding to the feature values x~j~.
...
where $E(\beta_jX_{j})$ is the mean effect estimate for feature x~j~.
...
**Symmetry**  
The contributions of two features x~j~ and x~k~ should be the same if they contribute equally to all possible coalitions.
If 
