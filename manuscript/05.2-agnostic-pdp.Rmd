...
```{r pdp-bike, fig.cap = 'Partial dependence plots for the bike count prediction model and different weather measurements (Temperature, Humidity, Windspeed). The biggest differences can be seen in the temperature: On average, the hotter the more bikes are rented, until 20C degrees, where it stays the same also for hotter temperatures and drops a bit again towards 30C degrees. The marks on the x-axis indicate the distribution of the feature in the data.'}
data(bike)
library("mlr")
library("iml")
library("ggplot2")
...
Interestingly, the predicted bike counts do not drop between 25 and 35 km/h windspeed, but there is just not so much training data, so we cannot be confident about the effect.
...
```{r pdp-bike-cat, fig.cap = 'Partial dependence plots for the bike count prediction model and the season. Unexpectedly, all seasons show the same effect, only for "spring" the model predicts less bike rentals.'}
...
It is complicated when features are correlated, see also disadvantages.
...
That is not the fault of PDPs, but of the 2-dimensional representation (paper or screen) and also our inability to imagine more than 3 dimensions.
- Some PD plots do not include the **feature distribution**. 
...
For the partial dependence of one of the features, let us say height, we assume that the other features (weight) are not correlated with height, which is obviously a wrong assumption. 
