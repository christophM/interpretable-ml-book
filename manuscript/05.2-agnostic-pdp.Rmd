...
This time we fit a random forest to classify penguin species based on bodily measurements.
We compute and visualize the partial dependence of the probability for species Adelie on bill length and body mass for the random forest:

```{r pdp-penguin, fig.cap = 'The higher the bill length, the less likely the penguin is an Adelie penguin, and the more likely it is a Chinstrap. A higher bill length also slightly increases the probability of Gentoo. The effect of body mass is less stark: For the Adelie probability we see no effect of body mass (always around 50%). The probability of Chinstrap slightly decreases with a higher weight, while for Gentoo it decreases.', dev.args = list(pointsize = 5.5), fig.height = 4}
...
p2 = pdp$plot() +
  scale_x_continuous("Body mass in gram") +
...
```{r pdp-penguins-2d, fig.cap = 'PDP for P(Adelie) and the interaction between bill length and body mass. The black dots show data points for Adelie penguins, and the grey dots for Chinstrap and Gentoo. For bill length, the plot shows what we already knew: A sharp drop in P(Adelie) the larger the bill length after 40mm. Body mass shows an interesting interaction with bill length: For penguins with bill length > 50, we can see slight differences in P(Adelie) between lighter and heavier penguins, even though there are zero Adelie penguins with that bill length.'}
