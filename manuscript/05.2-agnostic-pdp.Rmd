...
However, ICE plots do not indicate **in which cases** the effect is positive or negative.
Suppose that whether the effect is positive or negative depends on another feature; if the other feature is above a certain threshold, the effect is positive, otherwise negative.
A potential solution to this challenge is provided by so-called regional feature effects plots [^repid], [^gadget], [^vine].
Regional PDP plots will group the data conditioning on this other feature and compute a separate PDP for each partition.
This way, both partitions will show clear effects --- positive in the first and negative in the second --- and minimized heterogeneity.
...
Another package that implements both global and regional PDPs is [`effector`](https://github.com/givasile/effector).
...
[^repid]: Herbinger, J., Bischl, B., & Casalicchio, G. (2022, May). REPID: regional effect plots with implicit interaction detection. In International conference on artificial intelligence and statistics (pp. 10209-10233). PMLR.

[^gadget]: Herbinger, J., Wright, M. N., Nagler, T., Bischl, B., & Casalicchio, G. (2024). Decomposing global feature effects based on feature interactions. Journal of Machine Learning Research, 25(381), 1-65.

[^vine]: Britton, M. (2019). Vine: Visualizing statistical interactions in black box models. arXiv preprint arXiv:1904.00561.