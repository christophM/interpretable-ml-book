...
One assumption is that the features in $x_C$ are uncorrelated with the features in $x_S$. 
If this assumption is violated, the averages, which are computed for the partial dependence plot, incorporate data points that are very unlikely or event impossible (see disadvantages).
...
p2 = pdp$set.feature("hum")$plot() + my_theme()+ scale_x_continuous('Humidity', limits = c(0, NA)) + scale_y_continuous('', limits = c(0, 5500))
p3 = pdp$set.feature("windspeed")$plot() + my_theme()+ scale_x_continuous('Windspeed', limits = c(0, NA)) + scale_y_continuous('', limits = c(0, 5500))
...
p1 = pdp$plot() + my_theme() + 
  ...
  scale_y_continuous('Predicted cancer probability', limits = c(0, 0.5))
p2 = pdp$set.feature("Hormonal.Contraceptives..years.")$plot() + my_theme()+ 
  scale_x_continuous(limits = c(0, NA)) + 
...
- If the feature for which you computed the PDP is uncorrelated with the other model features and if you have an indicator which part of the grid has how much weight, then the PDPs are perfectly representing how the feature influences the target on average.
...
That's not the fault of PDPs, but of the primarily 2-dimensional representation (paper or screen) and also our inability to imagine more than 3 dimensions.
...
A solution to this problem are ALEPlots, that only average over close data points. [^ALE]
- **Heterogenous effects might be hidden** because the PDP only shows the average over the observations. 
Assume that for feature $x_1$ half your data points have a positive assocation with the outcome - the greater $x_1$ the greater $\hat{y}$ - and the other half has negative assocation - the smaller $x_1$ the greater $\hat{y}$.
