...
For layers or the whole network you can now start plotting diagrams that shows how many units there are for detecting different concepts.
You can also analyse how many of the units detect a distinct concept at all (based on the IoU cutoff).

TODO: Include example from paper.
...
- IoU is usually smaller than 0.20, which I find rather low.
...
- With network dissection, we can also detect concepts that were not in the classification task, as long as we have other datasets that contain that have labeled segments with the concepts.
- Network dissection also gives us a way to automatically link units to concepts, which is very convenient.
