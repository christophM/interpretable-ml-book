...
Network Dissection by Bau & Zhou et al. (2017) [^dissect] is an approach to quantify the interpretability of a unit of a convolutional neural network through segmentation and linking highly activated areas to human concepts (objects, parts, textures, colors, ...).
...
Disentangled means that individual units detect specific real world concepts.
...
The opposite of a disentangled representation would be a completely entangled distribution of concepts among the units.
To understand whether or nor a dog snout is on an image, it is not enough to look at a single unit, but we would have to look at all units to understand whether a dog snout is on the image (excluding the class output neurons here).

Disentangled features would imply that the network is highly interpretable.
Let us assume that we train a network that learns completely disentangled features and we label all the channels by the concepts they learn.
This would open possibilities to truly understand the decision making progress.
For example, we want to check how the network makes predictions for classifying wolves vs. huskeys.
We identify the unit that is associated with huskey.
When this unit depends on the "dog snout"-, "fluffy fur"- and "snow"-units from the previous, we know that it will missclassify a huskey with a snow background as wolf.
We could easily detect problematic non-causal correlations.
To explain individual predictions we could automatically list all units that were highly actived and inspect the associated concepts.
Spoiler-alert: The current convolutional networks are not perfectly disentangled.

But to find out just how interpretable the networks are, and especially individual units, we will look closer at how the Network Dissection method works.
...
The basic idea is: 
Use a dataset that is labeled with many different concept down to the pixel level (i.e. segmentation) and measure how much highly activated areas of a channel match with each of those concepts.
Assign the best matching concept to each channel.

Three steps: 

1. Get images with human-labeled visual concepts, from stripes to skyscrapers.
1. Measure the convolutional channel's activations for those images.
1. Quantify the intersection of activations and labeled concepts.


**Step 1: Broden dataset** 

To gather data that is labeled pixel-wise with concepts of different abstraction levels (for stripes over dog snouts to street scences) is a very crucial step and difficult step.
The authors managed to generate such a datast by combining multiple datasets.
They called this new collection of concepts datasets 'Broden', which stands for broadly and densely labeled data.
The Broden dataset is segmented to the pixel level mostly (for some datasets the whole image is labeled).
All in all, Broden contains 60.000 images with over 1000 visual concepts in different abstraction levels: 468 scences, 585 objects, 234 parts, 32 materials, 47 textures and 11 colors.
The follwing figures shows some sample images from the Broden dataset.

```{r fig.cap = "Examples images from the Broden dataset. Figure originally from Bau & Zhou et. al (2017)."}
knitr::include_graphics("images/broden.png")
```

Given the Broden dataset and a trained convolutional neural network (e.g. an Inception V3 network trained on ImageNet), the goal is to map each convolutional channel to a concept.
For that, we need to upscale each channel, so that it matches the original image and compare the overlap of high activation and concepts in Broden:
...
In the first part of the algorithm, we create the masks of the top actived areas, per channel and per image.
At this point, we don't yet need the concept labels.

1. For each image x in the Broden dataset and for each convolutional channel k:
  1. Forward propagate image x to the target layer that contains channel k.
  1. Extract the activation map of convolutional channel k: $A_k(x)$
  1. Compute distribution of pixel activations $\alpha_k$ over image
  1. Determine the 0.005-quantile level $T_k$ of activations $\alpha_k$. This means 0.5% of all activations of channel k for image x are larger than $T_k$.
  1. Scale up the (possibly) lower-resolution activation map $A_k(x)$ to the resolution of image x. We call the result $S_k(x)$.
...
For each of the images, we now know the maximally activated area per channel.
We have $K$ layers per image.
We want to find the concept with the highest overlap with the most activated area.

We can compute the alignment between activation mask k and concept mask c with the Intersection over Union (IoU) score:

$$IoU_{k,c}=\frac{\sum|M_k(x)\bigcap{}L_c(x)|}{\sum|M_k(x)\bigcup{}L_c(x)|}$$

where $|\cdot|$ is the cardinality of a set.
Intersection over union compares the alignment between the two areas.
The alignment is the size of the overlapping area between activated area and the concept mask, summed over all images.
This sum is divided by the size of the area of the union, also summed over all images.
...
The following Figure visualizes intersection and union of activated area and concept mask.
The dataset-wide IoU score is not the sum of IoU scores per image.
It is the sum over all intersections over the sum of all unions.
...
The authors trained different network architectures (AlexNet, VGG, GoogleNet, ResNet) from scratch on different datasets (ImageNet, Places205, Places365).
ImageNet contains 1.6 million images from 1000 classes.
The dataset is focused on objects.
Places205 and Places365 contain images of scenes, with 2.4 million and 1.6 million images and 205 amd 365 different scenes.
They additionally trained AlexNet on self-supervised training tasks, like predicting context, video frame order or tracking, detecting object-centric alignment, colorizing images, predicting crosschannel and predicting ambient sound from frames.
...
The rotation decreased interpretability, i.e., the number of channels aligned with a concept decreased.
...
- The networks detect lower-level concepts at lower layers and higher-level concepts at higher layers.
  We already saw this in the [Feature Visualizations](#feature-visualization).
- Different architectures have different levels interpretability.
  Interpretability also differs across tasks.
...
- Increasing the number of channels in a layer increase the number of interpetable units.


For more images of detected concepts, visit the [projects website](http://netdissect.csail.mit.edu/dissect/resnet-152-torch-places365/).
Note how many units there are for detecting airplanes.
Also other concepts are matched by many units.
For all detections, IoU is rather low, below 0.2.

Not covered here, but the authors also used Network Dissection for Generative Adversarial Networks (GANs).
You can find Network Dissection for GANs on [this website](https://gandissect.csail.mit.edu/).
...
Network dissection expands those insights and makes interpretability of network units measureable.
- Network dissection gives us a way to **automatically link units to concepts**, which is very convenient.
...
- **Illusion of Interpretability?**
 ...
 The neural network remains to incomprehensible: The neurons interact in a complex way, positive and negative activations are unrelated, multiple neurons might learn very similar features and for many of the features we do not have equivalent human concept.
  ...
  Network Dissection showed that architectures like ResNet or Inception have units that react to certain concepts, but the IoU is no that great and usually multiple units respond to a given concept, so they don't learn disentangled features.
  All the units are entangled and we can't interpret them in isolation.
...
- IoU is usually smaller than 0.20, which I find rather **low unit-concept alignment**.
  If you have an object, e.g. a dog which is labeled as dog and the network is highly activated for the dog and an area around the dog and the area, including the dog, is five times as big as the dog, the IoU is 0.2. 
