...
The channels of a convolutional neural network learns new features, as we saw in the chapter on [Feature Visualizations](#feature-visualization).
...
The hypothesis is:
Units of a neural network (like convolutional channel) learn disentangled concepts.

**The Question of Disentangled Features**

Do (convolutional) neural networks learn disentangled features?
Disentangled features mean that individual units detect specific real world concepts.
Convolutional channel 394 might detect sky scrapers, channel 121 dog snouts, channel 12 stripes at 30 degree angle ...
The opposite of disentangled network is a completely entangled network.
...
When can check whether this unit depends on the "dog snout", "fluffy fur" and "snow"-units from the previous layer.
...
Spoiler-alert: convolutional networks are not perfectly disentangled.
We will look more closely at Network Dissection to find out how interpretable neural networks are.
...
The alignment is visualized in the following image:

```{r fig.cap = "For a given input image and a trained network (fixed weights), we forward propagate the image up to the target layer, upscale the activations to match the original image size and compare the maximum activations with the ground truth pixel-wise segmentation.", out.width=800}
...
The following figures show sample images from the Broden dataset.
...
- For each image x in the Broden dataset and for each convolutional channel k:
    1. Forward propagate image x to the target layer containing channel k.
    1. Extract the pixel activations of convolutional channel k: $A_k(x)$
    1. Calculate distribution of pixel activations $\alpha_k$ over image x
    1. Determine the 0.005-quantile level $T_k$ of activations $\alpha_k$. This means 0.5% of all activations of channel k for image x are greater than $T_k$.
    1. Scale the (possibly) lower-resolution activation map $A_k(x)$ to the resolution of image x. We call the result $S_k(x)$.
    1. Binarize the activation map: A pixel is either on or off, depending on whether it exceeds the activation threshold $T_k$. The new mask is $M_k(x)=S_k(x)\leq{}T_k(x)$.
...
This threshold is chosen by Bau & Zhou et. al.

The following figure visualizes intersection and union of activation mask and concept mask for a single image:
...
```{r, fig.cap = "Activation mask for inception_4e channel 750 which detects dogs with $IoU=0.203$.", out.width=800}
