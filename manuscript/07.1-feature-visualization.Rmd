...
```{r pos-neg, fig.cap="Positive (left) and negative (right) activation of Inception V1 neuron 484 from layer mixed4d pre relu. While the neuron is maximally activated by wheels, something which seems to have eyes yields a negative activation. [Code available in colab notebook](https://colab.research.google.com/github/tensorflow/lucid/blob/master/notebooks/feature-visualization/negative\\_neurons.ipynb)"}
...
```{r activation-optim, fig.cap="Iterative optimization from random image to maximizing activation. Olah, et al. 2017 (CC-BY 4.0), [distill.pub](https://distill.pub/2017/feature-visualization/)."}
...
```{r, fig.cap = "Activation mask for inception\\_4e channel 750 which detects dogs with $IoU=0.203$. Figure originally from the [project website](http://netdissect.csail.mit.edu/)"}
...
[^distill-fv]: Chris Olah, et al., "Feature Visualization", Distill, 2017.

[^distill-blocks]: Chris Olah, et al., "The Building Blocks of Interpretability", Distill, 2018.
