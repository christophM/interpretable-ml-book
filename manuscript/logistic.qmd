...
$$\frac{odds_{x_j+1}}{odds_{x_j}}=\frac{exp\left(\beta_{0}+\beta_{1}x_{1}+\ldots+\beta_{j}(x_{j}+1)+\ldots+\beta_{p}x_{p}\right)}{exp\left(\beta_{0}+\beta_{1}x_{1}+\ldots+\beta_{j}x_{j}+\ldots+\beta_{p}x_{p}\right)}$$
...
$$\frac{odds_{x_j+1}}{odds_{x_j}}=exp\left(\beta_{j}(x_{j}+1)-\beta_{j}x_{j}\right)=exp\left(\beta_j\right)$$
...
cap = "Logistic regression results for predicting penguin P(female): Weight, standard errors and odds ratios."
kableExtra::kbl(coef.table[, c('Weight', 'Std. Error', 'Odds ratio')], digits=2, caption=cap, booktabs = TRUE) %>% kable_styling(position = "center")
...
# 
Interpretation of a categorical feature ("chonkiness"):
