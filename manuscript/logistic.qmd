...
We use logistic regression to predict [whether a penguin is female](#penguins) for Chinstrap[^not-all-penguins] penguins based on penguin characteristics.
...
kableExtra::kbl(coef.table[, c('Weight', 'Std. Error', 'Odds ratio')], digits=2, caption='The results of fitting a logistic regression model to the penguin dataset.', booktabs = TRUE) %>% kable_styling(position = "center")

```

Interpretation of a numerical feature ("bill_length_mm"):
An increase in a penguin's bill length increases the odds of being a female vs a male penguin by a factor of `r coef.table['bill_length_mm', 'Odds ratio']`, when all other features remain the same.

Interpretation of a categorical feature ("chonkiness"):
For the mid chonky penguins, the odds of being female versus male are increased by a factor of `r coef.table['chonkinessRegular_Penguin', 'Odds ratio']`, compared to lightweight penguins, given all other features remain the same.
For the chonkiest penguins, the odds of being female versus male are decreased (factor of `r coef.table['chonkinessAbsolute_Unit', 'Odds ratio']`), compared to lightweight penguins, given all other features remain the same.
Again, given all other features remain the same.

The intercept is a very large value.
This is because it is to be interpreted as odds of being female when all numerical features are set to zero and all categorical features to the reference level.
