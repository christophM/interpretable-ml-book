...
Imagine using an algorithm to learn decision rules for predicting the value of a house (`low`, `ok` or `high`).
...
Suppose 100 of 1,000 houses are big and in a good location, then the support of the rule is 10%.
...
For example:
Let's say of the 100 houses, where the rule `size=big AND location=good THEN value=high` applies, 85 have `value=high`, 14 have `value=ok` and 1 has `value=low`, then the accuracy of the rule is 85%.
...
The prediction of the default rule is often the most frequent class of the data points that are not covered by other rules.
...
The OneR algorithm [@holte1993very] is one of the simplest rule induction algorithms.
...
A more fitting name would be OneFeatureRule.
...
Let's look at an example of how the best feature is chosen by OneR.
@tbl-oner-freq-table1 shows an artificial dataset about houses with information about their value, location, size, and whether pets are allowed.
We are interested in learning a simple model to predict the value of a house.

```{r}
#| label: tbl-oner-freq-table1
#| tbl-cap: Artificial dataset for house value.
value = factor(c("high", "high", "high", "ok", "ok", "ok", "ok", "low", "low", "low"), levels = c("low", "ok", "high"))
...
value.f = factor(paste("value=", value, sep = ""), levels = c("value=low", "value=ok", "value=high"))
kableExtra::kbl(df,  booktabs = TRUE, ) 
```

OneR constructs rules based on the cross-tables between each feature and the outcome.
@tbl-cross-tables shows the three cross tables.

```{r}
#| label: tbl-cross-tables 
#| tbl-cap: Cross tables between each feature and the target.
t1 = table(paste0("size=", df[,"size"]), value.f)
t2 = table(paste0("pets=", df[,"pets"]), value.f)
t3 = table(paste0("location=", df[,"location"]), value.f)
kbl(do.call("rbind", list(t1, t2, t3)), booktabs=TRUE)
```
...
The most frequent value for houses in bad locations is `low`, and when we use `low` as a prediction, we make two mistakes, because two houses have an `ok` value.
The predicted value of houses in good locations is `high`, and again we make two mistakes, because two houses have an `ok` value.
The error we make by using the location feature is $\frac{4}{10}$, for the size feature it is $\frac{3}{10}$, and for the pet feature it is $\frac{4}{10}$.
...
If not properly validated, OneR may overfit on features with many possibly levels.
...
We use the [Palmer penguins data](#penguins) to test the OneR algorithm.
All continuous input features were discretized into 5 quantiles.
The rules created by the OneR learning algorithms are shown in @tbl-oner-penguins.

```{r}
#| label: tbl-oner-penguins
#| tbl-cap: "Rule learned by OneR for penguin classification."
library("OneR")
rule = OneR::OneR(sex ~ ., data = penguins_train)
...
kableExtra::kbl(rule.to.table(rule), row.names = FALSE, booktabs = TRUE)
```

The `bill_depth_mm` feature was chosen by OneR as the best predictive feature.
To get a better feel for how this one rule performs, let's look into the cross table between the (discretized) `bill_depth_mm` feature and the penguin species, see @tbl-oner-penguin-confusion.

```{r}
#| label: tbl-oner-penguin-confusion
#| tbl-cap: Confusion matrix for penguin sex classification rule found by OneR.
# make sure to use the same bins
bin_edges <- levels(bin(penguins_train$bill_depth_mm))
clean_edges = gsub("]", "", bin_edges)
clean_edges = gsub("\\(", "", clean_edges)
edges <- unique(as.numeric(unlist(strsplit(clean_edges, ","))))
breaks = sort(edges)
breaks[length(breaks)] = breaks[length(breaks)] + 0.01

tt <- penguins_test %>%
  mutate(bill_depth_bin = cut(bill_depth_mm, breaks = breaks, include.lowest = FALSE, right=FALSE)) %>%
  group_by(bill_depth_bin, sex) %>%
  summarise(count = n()) %>%
  group_by(bill_depth_bin) %>%
  mutate(percentage = count / sum(count) * 100) %>%
  filter(!is.na(bill_depth_bin)) %>%
  ungroup() %>%
  mutate(display = sprintf("%.1f%% (%d)", percentage, count)) %>%
  pivot_wider(id_cols = bill_depth_bin, 
              names_from = sex, 
              values_from = display, 
              values_fill = "0% (0)")

# Rename the first column
colnames(tt)[1] <- "bill_depth_mm"
# Create the table with kable and kable_styling
kableExtra::kbl(tt, booktabs = TRUE, align = "c") %>%
  kable_styling(position = "center")
```

We can see that this feature is particularly useful for classifying penguins with deep bills.
Penguins with bills with a depth of over 1.98 cm are most likely to be male.
Very short bills make it likely that we are dealing with a female penguin.
Caveat: This model ignores the species, or rather bundles them all together.
To fix this, we could train one model per species.

OneR doesn't inherently support regression tasks.
...
@tbl-oner-bike shows the selected feature after fitting the OneR model:
The selected feature is the previous bike rental count.

```{r}
#| label: tbl-oner-bike
#| tbl-cap: OneR classification rule for the bike data.
bike2 = bike
bike2$cnt =  cut(bike2$cnt, breaks = quantile(bike$cnt), dig.lab = 10, include.lowest = TRUE)
rule = OneR::OneR(cnt ~ ., data = bike2)
kableExtra::kbl(rule.to.table(rule), row.names = FALSE, booktabs = TRUE) %>% kable_styling(position = "center")
```
...
Many rule-learning algorithms are part of the sequential covering family.
This chapter introduces the main recipe and uses RIPPER, a variant of the sequential covering algorithm, for the examples.

The idea is simple: First, find a good rule that applies to some of the data points.
Remove all data points that are covered by the rule.
...
```{r}
#| label: covering-algo
#| fig.cap: Steps of the covering algorithm.
#| fig-asp: 0.9
set.seed(42)
n = 100
...
p3 = ggplot(dat.reduced) + 
  ...
  scale_color_viridis(guide = "none", discrete = TRUE, option = "D", end = 0.9) +
  scale_shape_discrete(guide = "none") + 
  ggtitle("Step 2: Remove instances")
...
Maybe: If `location=good`, then `value=ok`.
...
The OneR algorithm wouldn't work here since it would always cover the whole feature space.
...
@fig-learn-one-rule illustrates the beam search in a tree:

![Rule learned by searching a path through a decision tree. We end up with: If `location=good` and `size=big`, then `value=high`.](./images/learn-one-rule.jpg){#fig-learn-one-rule}
...
RIPPER (Repeated Incremental Pruning to Produce Error Reduction) [@cohen1995fast] is a variant of the Sequential Covering algorithm.
...
extract.rules.jrip = function (rule) {
  ...
  # Remove the initial description lines and the summary at the end
  rules = rules[-c(1, 2, length(rules))]
  # Clean up the confidence numbers from each rule (e.g., "(69.0/2.0)")
  rules = gsub("\\([0-9]+\\.[0-9]+/[0-9]+\\.[0-9]+\\)", "", rules)
  # Convert to a data frame for better formatting in the output
  rules_df = data.frame(rules = rules, stringsAsFactors = FALSE)
  # Display the rules in a formatted table
  if (nrow(rules_df) == 0) {
    return(NULL)
  } else {
    kableExtra::kbl(rules_df, booktabs = TRUE) %>%
      column_spec(1, width = single_col_width)
  }
}

```

First, we extract the rules for the [penguin sex classification task](#penguins).
The rules plus the default rule are printed in @tbl-jrip-penguins.
The interpretation is simple:
To predict a new instance, start at the top of the list and check whether a rule applies.
If the conditions apply, predict the species on the right-hand side of the rule.
If it doesn't apply, go to the next rule until you have a classification.
The default rule ensures that there is always a prediction.

```{r}
#| label: tbl-jrip-penguins
#| tbl-cap: "JRip rules learned for the penguin classification task."
# give shorter names for more narrow tables


penguins2 = penguins
penguins2$sex <- ifelse(penguins2$sex == "male", "M", 
                       ifelse(penguins2$sex == "female", "F", NA))
penguins2$sex = factor(penguins2$sex)
rule = JRip(sex ~ ., data = penguins2)
...
RIPPER has a specific way of resolving conflicts between rules: it applies them sequentially.
This is just one of many general strategies, ranging from rules with the most-specific if-clause to using the rule with the highest precision.
...
For example, the interval \([4548, 5956]\) covers predicted bike counts between 4548 and 5956.
@tbl-jrip-bike shows the decision list of learned rules.
Interpretation is again: If the conditions apply, we predict the interval on the right-hand side for the number of bikes.
I don't like the decision rules for this example.
The nature of the `cnt_2d_bfr` feature is to adjust for a general trend of bike rentals.
`cnt` and `cnt_2d_bfr` have a linear relationships which decision rules have a hard time to reflect.

```{r}
#| label: tbl-jrip-bike
#| tbl-cap: JRip rules for the bike task.
...
A specific approach using this recipe is called Bayesian Rule Lists [@letham2015interpretable] or BRL for short.
BRL uses Bayesian statistics to learn decision lists from frequent patterns which are pre-mined with the FP-tree algorithm [@borgelt2005implementation].
...
This does not mean that the houses are removed from the data; it just means that `size=big` is not returned as a frequent pattern.
...
Only in the case of frequent patterns do we have to check patterns of higher order.
...
Other approaches suggest including the outcome of interest in the frequent pattern mining process and also executing the second part of the Apriori algorithm that builds IF-THEN rules.
...
The posterior probability distribution of lists makes it possible to say how likely a decision list is, given assumptions of shortness and how well the list fits the data.
...
1) Generate an initial decision list, which is randomly drawn from the prior distribution.
...
(That's Bayesian statistics.)
If you are unfamiliar with Bayesian statistics, do not get too caught up in the following explanations.
It's important to know that the Bayesian approach is a way to combine existing knowledge or requirements (so-called priori distributions) while also fitting to the data.
...
quantifies how probable a decision list is, given the observed data and the prior assumptions.
...
Estimations in Bayesian statistics are always a bit tricky because we usually cannot directly calculate the correct answer, but we have to draw candidates, evaluate them, and update our posterior estimates using the Markov chain Monte Carlo method.
For decision lists, this is even trickier because we have to draw from the distribution of decision lists.
...
At each step, the algorithm evaluates the posterior probability of the decision list (mixture of accuracy and shortness).
...
The examples use a faster variant of BRL called Scalable Bayesian Rule Lists (SBRL) [@yang2017scalable].
We use the SBRL algorithm to predict the [species of the penguins](#penguins).
I had to discretize all input features for the SBRL algorithm to work.
To do this, I binned the continuous features based on the frequency of the values by quantiles.
We get the rules displayed in @tbl-sbrl-penguins-show.

<!-- Set eval = TRUE to recompute the rules -->
```{r}
#| label: sbrl-penguins-prepare
#| results: hide
#| eval: true
library("sbrl")
library("arules")
...
options(scipen = 999)
penguins2 = as.data.frame(lapply(penguins, function(x) {
...
get.sbrl.rules = function(x) {
    res = lapply(1:nrow(x$rs), function(i) {
        if (i == 1) 
            sprintf("If %s then p = %.3f\n", 
                x$rulenames[x$rs$V1[i]],  x$rs$V2[i])
        else if (i == nrow(x$rs)) 
            sprintf("else  (default rule)  then p = %.3f\n", 
                x$rs$V2[nrow(x$rs)])
        else sprintf("else if %s then p = %.3f\n", 
            x$rulenames[x$rs$V1[i]], x$rs$V2[i])
    })
    data.frame(rules = unlist(res))
}

penguins2$label = penguins2$sex 
penguins2$sex = NULL
rules = sbrl(penguins2, pos_sign = "female", neg_sign = "male", rule_maxlen = 2)
rn = rules$rulenames
rl = get.sbrl.rules(rules)
saveRDS(list(rulenames = rn, rules = rl), file = "../data/cached-sbrl-penguins.RDS")
```


```{r}
#| label: tbl-sbrl-penguins-show
#| tbl-cap: "Rules from SBRL model."
rules = readRDS("../data/cached-sbrl-penguins.RDS")
kableExtra::kbl(rules$rules, booktabs = TRUE)
```
...
@tbl-sbrl-penguins-premined shows a sample of ten patterns.

```{r}
#| label: tbl-sbrl-penguins-premined
#| tbl-cap: Premined rules for the penguin classification task.
set.seed(1)
conditions = sample(rules$rulenames, size = 10)
conditions = gsub("\\{|\\}", "", conditions)
conditions = gsub(",", ", ", conditions)
tab = kableExtra::kbl(conditions, col.names = "pre-mined conditions",  booktabs = TRUE) %>% kable_styling(position = "center")
tab
```
...
Fewer and shorter rules make for better model interpretation.
But there is a trade-off with complexity and therefore with predictive performance.
...
What's the splitting criterion: Fixed interval lengths, quantiles, or something else?
...
Decision rules are **bad at describing linear relationships** between features and output.
That's a problem they share with decision trees.
...
OneR is also implemented in the [Weka machine learning library](https://www.eecs.yorku.ca/tdb/_doc.php/userg/sw/weka/doc/weka/classifiers/rules/package-summary.html) and, as such, is available in Java, R, and Python.
RIPPER is also implemented in Weka.
For the examples, I used the R implementation of JRIP in the [RWeka package](https://cran.r-project.org/web/packages/RWeka/index.html).
SBRL is available as an [R package](https://cran.r-project.org/web/packages/sbrl/index.html) (which I used for the examples), in [Python](https://github.com/datascienceinc/Skater), or as a [C implementation](https://github.com/Hongyuy/sbrlmod).
Additionally, I recommend the [imodels package](https://github.com/csinva/imodels), which implements rule-based models such as Bayesian rule lists, CORELS, OneR, greedy rule lists, and more in a Python package with a unified scikit-learn interface.
...
I recommend the book "Foundations of Rule Learning" by @furnkranz2012foundations.
It's an extensive work on learning rules, for those who want to dive deeper into the topic.
It provides a holistic framework for thinking about learning rules and presents many rule learning algorithms.
I also recommend checking out the [Weka rule learners](http://weka.sourceforge.net/doc.dev/weka/classifiers/rules/package-summary.html), which implement RIPPER, M5Rules, OneR, PART, and many more.
