...
@tbl-oner-freq-table1 shows an artificial dataset about houses with information about its value, location, size and whether pets are allowed.
...
cap = "Artificial dataset for house value."
kableExtra::kbl(df,  booktabs = TRUE, caption=cap) %>% kable_styling(position = "center")
```

OneR creates the cross-tables between each feature and the outcome.
@fig-cross-tables shows the three cross tables.

::: {#fig-cross-tables layout-ncol=2}



```{r}
kableExtra::kbl(table(paste0("size=", df[,"size"]), value.f),  booktabs = TRUE) %>% kable_styling(position = "center")
```


```{r}
kableExtra::kbl(table(paste0("pets=", df[,"pets"]), value.f),  booktabs = TRUE) %>% kable_styling(position = "center")
```

```{r}
kableExtra::kbl(table(paste0("location=", df[,"location"]), value.f), booktabs = TRUE) %>% kable_styling(position = "center")
```
...
cap = "Rule learned by OneR for penguin classification."
kableExtra::kbl(rule.to.table(rule), row.names = FALSE, caption = cap, booktabs = TRUE) %>% kable_styling(position = "center")
```
...
cap = "Confusion matrix for penguin sex classification rule found by OneR."
# Create the table with kable and kable_styling
kableExtra::kbl(tt, booktabs = TRUE, align = "c", caption=cap) %>%
...
extract.rules.jrip = function (rule, caption="") {
  ...
  if (nrow(rules_df) == 0) {
    ...
    kableExtra::kbl(rules_df, booktabs = TRUE, caption=caption) %>%
...
cap = "JRip rules learned for the penguin classification task."
extract.rules.jrip(rule, caption=cap)
```
...
cap = "JRip rules for the bike task."
extract.rules.jrip(rule, caption=cap)
```
...
cap = "Rules from SBRL model."
kableExtra::kbl(rules$rules, booktabs = TRUE, caption = cap) %>% kable_styling(position = "center")
```
...
#if (is.pdf) tab = tab %>%  column_spec(1, "10cm")
tab
```
...
cap = "SBRL rules for the bike task."
tab = kableExtra::kbl(rules, booktabs = TRUE, caption=cap) %>% kable_styling(position = "center")
