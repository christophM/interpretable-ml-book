...
For example:
Let's say of the 100 houses, where the rule `size=big AND location=good THEN value=high` applies, 85 have `value=high`, 14 have `value=medium` and 1 has `value=low`, then the accuracy of the rule is 85%.
...
kableExtra::kbl(df,  booktabs = TRUE) %>% kable_styling(position = "center")
```

OneR creates the cross-tables between each feature and the outcome:
```{r}
#| label: OneR-freq-table2
...
Always validate your chosen model with a separate validation set to ensure that it performs well on unseen data, even if it's a simple model "one-rule" model.
...
#| fig-cap: Confusion matrix for penguin sex classification rule found by OneR.
...
Here we get these rules plus the default rule.

```{r}
#| label: jrip-penguins
#| include: true
rule = JRip(sex~ ., data = penguins)
...
Interpretation: If the conditions apply, we predict the interval on the right-hand side for the number of bikes.
...
Since there are 3 possible species in the Palmers' penguin data, but SBRL needs binary labels, I simplified the task to classify Adelie versus other (Gentoo/Chinstrap).
I also had to discretize all input features for the SBRL algorithm to work.
...
get.sbrl.rules = function(x) {
    res = lapply(1:nrow(x$rs), function(i) {
        if (i == 1) 
            sprintf("If      %s (rule[%d]) then positive probability = %.3f\n", 
                x$rulenames[x$rs$V1[i]], x$rs$V1[i], x$rs$V2[i])
        else if (i == nrow(x$rs)) 
            sprintf("else  (default rule)  then positive probability = %.3f\n", 
                x$rs$V2[nrow(x$rs)])
        else sprintf("else if %s (rule[%d]) then positive probability = %.3f\n", 
            x$rulenames[x$rs$V1[i]], x$rs$V1[i], x$rs$V2[i])
    })
...
```{r}
#| label: sbrl-penguins-show
rules = readRDS("../data/cached-sbrl-penguins.RDS")
tab = kableExtra::kbl(rules$rules, booktabs = TRUE) %>% kable_styling(position = "center")
#if (is.pdf) tab = tab %>%  column_spec(1, "10cm")
tab
```
...
```{r}
#| label: sbrl-bike-show
rules = readRDS("../data/cached-sbrl-bike.RDS")
tab = kableExtra::kbl(rules, booktabs = TRUE) %>% kable_styling(position = "center")
