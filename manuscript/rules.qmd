...
@fig-cross-tables shows the three cross tables.

::: {#fig-cross-tables layout-nrow=3}



```{r}
kableExtra::kbl(table(paste0("size=", df[,"size"]), value.f),  booktabs = TRUE) 
```


```{r}
kableExtra::kbl(table(paste0("pets=", df[,"pets"]), value.f),  booktabs = TRUE)
```

```{r}
kableExtra::kbl(table(paste0("location=", df[,"location"]), value.f), booktabs = TRUE) 
```



Cross tables between each feature and the target.
...
The bill depth feature was chosen by OneR as the best predictive feature.
To get a better feel for how this one rule performs, let's look into the cross table between the (discretized) bill depth feature and the penguin species, see @tbl-oner-penguin-confusion.
...
I have arbitrarily created a classification task by creating a label that is 1 if the number of bikes exceeds 4,000 on a day, else 0.
...
The rule list in @tbl-sbrl-bike-show was learned by SBRL.

<!-- Set eval = TRUE to recompute the rules -->
```{r}
#| label: sbrl-bike-prepare
#| results: "hide"
#| eval: false 
library("sbrl")
library("arules")
...
```{r}
#| label: tbl-sbrl-bike-show
#| tbl-cap: SBRL rules for the bike task.
rules = readRDS("../data/cached-sbrl-bike.RDS")
tab = kableExtra::kbl(rules, booktabs = TRUE)
tab %>% column_spec(1, width = single_col_width)
```
...
The first rule does not apply, since it only applies to days in 2011.
The second rule applies, because the day is in 2012, and 17 degrees lies in the interval `[7.35,19.9)`.
Our prediction for the probability that more than 4000 bikes are rented is 88%.
...
I recommend the book "Foundations of Rule Learning" by Fuernkranz et al. [@furnkranz2012foundations].
