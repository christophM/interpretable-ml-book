...
We use the Shapley value to analyze the predictions of a random forest model predicting [penguin species](#penguins), more specifically the probability of Adelie:
...
ntree = 30
penguins.x = penguins[names(penguins) != 'species']

model <- caret::train(penguins.x,
               penguins$species,
               method = 'rf', ntree=ntree, maximise = FALSE)
predictor = Predictor$new(model, class = "Adelie", data = penguins.x, type = "prob")

instance_indices = 10
x.interest = penguins.x[instance_indices,]

avg.prediction = mean(predict(model, type = 'prob')[,'Adelie'])
actual.prediction = predict(model, newdata = x.interest, type = 'prob')['Adelie']
diff.prediction = actual.prediction - avg.prediction
```


```{r shapley-penguins-plot, fig.cap = sprintf("Shapley values for penguin classification. This penguin's predicted probability for P(Adelie)=%.2f is %.2f above the average probability for Adelie of %.2f. The short bill length contributed most to the probability of being Adelie. The sum of the contributions yields the difference between actual and average prediction. (%.2f).", actual.prediction,diff.prediction, avg.prediction, avg.prediction)}
