...
> Given an instance $x$ to be explained, a rule or an anchor $A$ is to be found, such that it applies to $x$, while the same class as for $x$ gets predicted for a fraction of at least $\tau$ of $x$’s neighbors where the same $A$ is applicable.
A rule’s precision results from evaluating neighbors or perturbations (following $D_x (z|A)$) using the provided machine learning model (denoted by the indicator function $1_{\hat{f}(x) = \hat{f}(z)}$).
...
It would require evaluating $1_{\hat{f}(x) = \hat{f}(z)}$ for all $z \in \mathcal{D}_x(\cdot|A)$, which is not possible in continuous or large input spaces.
...
](./images/anchors1-1.png){#fig-anchors1}
