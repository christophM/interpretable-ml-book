...
$$ \mathbb{E}_{\mathcal{D}_\mathbf{x}(z|A)}[1_{\hat{f}(\mathbf{x})=\hat{f}(z)}] \geq \tau; A(\mathbf{x})=1 $$
...
A rule’s precision results from evaluating neighbors or perturbations (following $\mathcal{D}_\mathbf{x} (z|A)$) using the provided machine learning model (denoted by the indicator function $1_{\hat{f}(\mathbf{x}) = \hat{f}(z)}$).
