...
Ribeiro, Singh, and Guestrin proposed the algorithm in 2018[@ribeiro2018anchors] -- the same researchers who introduced the [LIME](#lime) algorithm.
...
The difference between LIME and Anchors is visualized in @fig-lime-vs-anchors.
...
![LIME vs. Anchors -- A Toy Visualization. Inspired by figure from Ribeiro, Singh, and Guestrin (2018).](./images/anchors-visualization.png){#fig-lime-vs-anchors width=75%}
...
Now we would like to know *why* the model predicts for one specific person (see @tbl-anchors-explain) that they survive.
...
$$ \mathbb{E}_{\mathcal{D}_x(z|A)}[1_{\hat{f}(\mathbf{x})=\hat{f}(z)}] \geq \tau; A(\mathbf{x})=1 $$
...
A rule’s precision results from evaluating neighbors or perturbations (following $\mathcal{D}_x (z|A)$) using the provided machine learning model (denoted by the indicator function $1_{\hat{f}(\mathbf{x}) = \hat{f}(z)}$).
...
It would require evaluating $1_{\hat{f}(\mathbf{x}) = \hat{f}(\mathbf{z})}$ for all $\mathbf{z} \in \mathcal{D}_x(\cdot|A)$, which is not possible in continuous or large input spaces.
...
$$\mathbb{P}(prec(A) \geq \tau) \geq 1 - \delta \quad \textrm{with} \quad prec(A) = \mathbb{E}_{\mathcal{D}_x(\mathbf{z}|A)}[1_{\hat{f}(\mathbf{x}) = \hat{f}(\mathbf{z})}]$$
...
$$cov(A) = \mathbb{E}_{\mathcal{D}_{(\mathbf{z})}}[A(\mathbf{z})]$$
...
This is why, at the core of this component, there is a pure-exploration Multi-Armed Bandit (*MAB*; *KL-LUCB*[@kaufmann2013information], to be precise).
...
Each time it is pulled, respective neighbors get evaluated, and we thereby obtain more information about the candidate rule's payoff (precision in anchor's case).
...
It's to classify whether the number of rented bicycles lies above or below the trend line.
We also use additional features like the number of days since 2011 as time trend, the month, and the weekday.
...
![Explaining instances near decision boundaries leads to specific rules comprising a higher number of feature predicates and lower coverage. Also, the empty rule, i.e. the base feature, gets less important. This can be interpreted as a signal for a decision boundary, as the instance is located in a volatile neighborhood.](images/anchors2-1.png){#fig-anchors2}
