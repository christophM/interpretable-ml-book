...
In our second example, we predict the classify [penguins](#penguins) with a random forest.
Again we train a decision tree with the original dataset, but with the prediction of the random forest as outcome, instead of the real classes (Adelie, Chinstrap, Gentoo) from the data.

```{r surrogate-penguins, message = FALSE, warning = FALSE, echo = FALSE, fig.cap = "The terminal nodes of a surrogate tree that approximates the predictions of a random forest trained on the penguins dataset. The counts in the nodes show the frequency of the black box models classifications in the nodes."}
library(palmerpenguins)
penguins.task = makeClassifTask(data = penguins, target = "species")
mod.penguins = mlr::train(mlr::makeLearner(cl = 'classif.randomForest', predict.type = "prob"), penguins.task)

pred.penguins = Predictor$new(mod.penguins, data = penguins, type = "prob")
tree.penguins = TreeSurrogate$new(pred.penguins, maxdepth = 2) 
plot(tree.penguins) + 
	theme(strip.text.x = element_text(size = 8))
#pred.tree.penguins = predict(tree.penguins, penguins)["species"]
#pred.penguins= getPredictionProbabilities(predict(mod.penguins, penguins.task))
```

The surrogate model has an R-squared (variance explained) of `r round(tree.penguins$r.squared[1], 2)`, which means it does approximate the random forest well.
Might even be close enough to ditch the random forest alltogether.
