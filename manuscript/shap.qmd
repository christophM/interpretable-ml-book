...
KernelSHAP estimates for an instance $\mathbf{x}$ the contributions of each feature value to the prediction.
...
- Compute the weight for each $\mathbf{z}_k'$ with the SHAP kernel.
...
$$\hat{f}(h_{\mathbf{x}}(\mathbf{z}')) = \mathbb{E}_{X_{-j}}[\hat{f}(\mathbf{x})]$$
...
TreeSHAP defines the value function using the conditional expectation $\mathbb{E}_{X_j|X_{-j}}[\hat{f}(X_j, \mathbf{x}_{-j})])$ instead of the marginal expectation.
...
TreeSHAP uses the conditional expectation $\mathbb{E}_{X_j|X_{-j}}[\hat{f}(X_j, \mathbf{x}_{-j})]$ to estimate effects.
...
For example, when the first split in a tree is on feature $X_3$, then all the subsets that contain feature $X_3$ will go to one node (the one where $\mathbf{x}$ goes).
