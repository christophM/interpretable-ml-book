...
In 2010 (and 2012), Shapley values were suggested for the first time to explain machine learning model predictions.
However, they didn't become so popular.
In 2017, @lundberg2017unified proposed SHAP, which was basically a new way to estimate Shapley values for interpreting machine learning predictions, along with a theory connecting Shapley values with [LIME](#lime) and a bit of theory on Shapley values.

Cyncally, you could say that SHAP is just a rebranding of Shapley values (which is true), but that would miss the fact that SHAP also marks a change in popularity and usage of Shapley values, and introduced lots of new ways of estimating Shapley values and to aggregate them in new types of plots.

So this chapter could totally be a subchapter of the [Shapley value chapter](#shapley), but you can also see it as Shapley values 2.0 for interpreting machine learning models.
...
where $g$ is the explanation model, $\mathbf{z}' \in \{0,1\}^M$ is the coalition vector, $M$ is the maximum coalition size and $\phi_j \in \mathbb{R}$ is the feature attribution for feature $j$, the Shapley values.
...
This section is about 3 ways to estimate Shapley values for explaining predictions: KernelSHAP, Permutation, and TreeSHAP.

## KernelSHAP

<!-- The general Idea of linear model -->
The situation with KernelSHAP is a bit confusing: It has been the entire motivation for SHAP and linking it with LIME and how SHAP was introduced in @lundberg2017unified.
Also all blog posts and so on about SHAP talk about it.
But KernelSHAP is extremely slow compared to TreeSHAP and the Permutation Method, and that's why the main shap Python package no longer uses it.
Even if depreciated, KernelSHAP gives a way to understand Shapley values and also shows how Shapley values and [LIME](#lime) are connected. 

The KernelSHAP estimation has five steps:

- Sample coalitions $\mathbf{z}_k' \in \{0,1\}^M, \quad k \in \{1,\ldots,K\}$ (1 = feature present in coalition, 0 = feature absent).
...
The function $h_\mathbf{x}$ maps 1's to the corresponding value from the instance x that we want to explain.
...
But it's necessary to sample from the marginal distribution.
...
- Interventional: This computes the classic Shapley values.
- Tree-path dependent: The tree-path dependent version redefines the value function $\mathbb{E}_{X_j|X_{-j}}[\hat{f}(X_j, \mathbf{x}_{-j})])$.
...
## Tree-path dependent may give non-zero Shapley values to unused features
...
The estimation for both interventional and tree-path dependent estimation is rather complex, so I'll just give you the rough idea (meaning I haven't fully understood the full version myself).
...
Anyays, to estimate the Shapley values, the algorithm recursively goes through the tree.
Let's say you have a data point $\mathbf{x}$ to explain and, for simplicity, the background dataset contains just one data point $\mathbf{z}$:
...
However, for many of these coalitions, adding a feature from $\mathbf{x}$ wouldn't change the prediction, since the tree only has a limited amount of distinct preditions due to the decision structure.
For example, a binary tree of depth 5 has a maximum of 32 possible predictions.
So instead of iterating through all coalitions, the Intervential TreeSHAP estimator explores the tree paths to only work with the coalitions that would actually change the predictions.
...
Path-dependent TreeSHAP works in a similiar fashion, making use of the tree structure.
The basic idea is to push all subsets of a coalitions S doen the tree simultaneously.
And in each split, we keep track of the number of instances for the subsets.

### Permutation Explainer

The most efficient truly model-agnostic estimator is the Permutation method.
The idea is to sample cleverly from the coalitions by creating permutations of the features.

Let's look at an example (taken from the book "Interpreting Machine Learning Models with SHAP") with four feature values: $x_{\text{park}}, x_{\text{cat}}, x_{\text{area}}$, and $x_{\text{floor}}$.
...
The permutation changes on feature at a time.
This reduces the number of model calls since the second first of a marginal contribution (a model prediction) is also need to compute the next marginal contribution.
...
Each forward + backward permutation give us two marginal contribution per feature.
...
Good thing is we can simply sample the permutations and still do this simple averaging.
...
The permutation method additionanlly ensures that the efficiency axiom is always satisfied, meaning when you add up the SHAP values, they equal prediction minus average prediction.
...
![](./images/cheatsheet-shap.png){width=60% fig-align="center"}
...
img1 <- png::readPNG("images/shap-explain-1.png")
img2 <- png::readPNG("images/shap-explain-2.png")
...
![SHAP feature importance measured as the mean absolute Shapley values.](./images/shap-importance.png){#fig-shap-importance}
...
![SHAP summary plot. ](images/shap-importance-extended.png){#fig-shap-summary-plot}
...
![SHAP dependence plot for body mass. The x-axis shows the feature, the y-axis the SHAP values. Each dot represents a data point.](images/shap-dependence.png){#fig-shap-dependence}
...
![Stacked SHAP explanations clustered by explanation similarity. Each position on the x-axis is an instance of the data. Red SHAP values increase the prediction, blue values decrease it.](./images/shap-clustering.png){#fig-shap-clustering}
