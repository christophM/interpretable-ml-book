...
SHAP (SHapley Additive exPlanations) by Lundberg and Lee (2017) [@lundberg2017unified] is a method to explain individual predictions.
...
Lundberg et al. (2018) [@lundberg2019consistent] proposed TreeSHAP, a variant of SHAP for tree-based machine learning models such as decision trees, random forests and gradient boosted trees.
...
The problem with the conditional expectation is that features that have no influence on the prediction function f can get a TreeSHAP estimate different from zero as shown by Sundararajan et al. (2019) [@sundararajan2020many] and Janzing et al. (2019) [@janzing2020feature].
...
The `shap` Python package comes with a lot of different visualizations, which can be confusing. That's why I summarized them in this [SHAP Plots Cheatsheet](https://christophmolnar.gumroad.com/l/shap-plots-for-tabular-data), along with how to interpret the plots.
...
The following figure shows SHAP explanation force plots for two penguins from the palmer penguins dataset:

```{r, fig.cap = "SHAP values to explain the predicted probabilities for Adelie species for two penguins. The baseline -- the average predicted probability -- is 0.47. The first penguin has a high probability of being an Adelie penguin due to its small bill length. The second penguin is unlikely to be Adelie due to its large bill length and flipper length."}
...
The following figure shows the SHAP feature importance for the random forest trained before for classifying penguins.

```{r fig.cap="SHAP feature importance measured as the mean absolute Shapley values. The bill length was the most important feature, changing the predicted probability for Adelie 33 pecentage points (0.33 on x-axis)."}
...
#| fig-cap: "SHAP summary plot. A short bill length contributes positively to P(Adelie). The summary plot also shows that bill length has the widest range of effects for the different penguins."
...
The following figure shows the SHAP feature dependence for the bill length:

```{r fig.cap="SHAP dependence plot for bill length. The longer the bills the more likely the penguin is an Adelie penguin."}
...
```{r fig.cap = "SHAP feature dependence plot with interaction visualization. Here we see the dependence plot of bill length in interaction with the body mass. Especially for longer bills, the contribution of the bill length to P(Adelie) differs based on body mass."}
...
```{r}
#| label: shap-clustering
#| fig-cap: "Stacked SHAP explanations clustered by explanation similarity. Each position on the x-axis is an instance of the data. Red SHAP values increase the prediction, blue values decrease it."
