...
$$\hat{f}(X) = \phi_0+\sum_{j=1}^M\phi_j X_j' = \mathbb{E}[\hat{f}(X)] + \sum_{j=1}^M\phi_j$$
...
![Function $h_x$ maps a coalition to a valid instance. For present features (1), $h_x$ maps to the feature values of x. For absent features (0), $h_x$ maps to the values of a randomly sampled data instance.](./images/shap-simplified-features.jpg){#fig-shap-simplified-feature width=90%}
...
![Function $h_x$ maps coalitions of superpixels (sp) to images. Superpixels are groups of pixels. For present features (1), $h_x$ returns the corresponding part of the original image. For absent features (0), $h_x$ greys out the corresponding area. ](./images/shap-superpixel.jpg){#fig-shap-images}
...
The bill length was the most important feature, changing the predicted probability for Adelie 33 percentage points (0.33 on x-axis).
...
![SHAP dependence plot for body mass. The x-axis shows the feature, the x-axis the SHAP values. Each dot represents a data point.](images/shap-dependence.png){#fig-shap-dependence}
