...
A few years later, @lundberg2017unified proposed SHAP, which was basically a new way to estimate Shapley values for interpreting machine learning predictions, along with a theory connecting Shapley values with [LIME](#lime), and a bit of additional theory on Shapley values.
...
Even if deprecated, KernelSHAP helps to understand Shapley values and shows how Shapley values and [LIME](#lime) are connected.
...
Let's look at an example (taken from the book [Interpreting Machine Learning Models with SHAP](https://christophmolnar.com/books/shap/)) with four feature values: $x_{\text{park}}, x_{\text{cat}}, x_{\text{area}}$, and $x_{\text{floor}}$. For simplicity, I'll shorten $x^{(i)}_{\text{park}}$ as $x_{\text{park}}$.
