...
SHAP computes Shapley values from coalitional game theory, the same as we discussed in @shapley.
...
Even if no longer the default, KernelSHAP helps to understand Shapley values and shows how Shapley values and [LIME](#lime) are connected and some implementation still use it.
...
A higher body mass contributes negatively to P(female). The summary plot also shows that body mass has the widest range of effects for the different penguins.
...
This implementation works  models in the [scikit-learn](https://scikit-learn.org/stable/) machine learning library for Python.
