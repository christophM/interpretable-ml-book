...
Permutation feature importance (PFI) measures the increase in the prediction error of the model after we permute the feature's values, which breaks the relationship between the feature and the true outcome.
...
Use test data for PFI to avoid overly optimistic results, especially with overfitting models.
PFI on training data can falsely highlight irrelevant features as important due to model overfitting.

To understand what the model actually used, consider alternatives like [SHAP importance](#shap) or [PDP importance](#pdp), which don't rely on error measures.
...
The most important feature was `r imp.dat$feature[best]`, the least important was `r imp.dat$feature[worst]`.

```{r}
#| label: fig-importance-bike
#| fig.cap: "The feature importance for each of the features in predicting bike counts with a support vector machine. The dot shows the average importance across multiple permutations, and the line the 5% and 95% quantile range."
plot(importance) +
   scale_y_discrete("") +
...
I trained a 3 logistic regression models to predict [penguin sex](#penguins), using 2/3 of the data for training and 1/3 for computing the importance.
...
The importance of each of the features for predicting penguin sex with a the logistic regression models.
The most important feature was `r most_imp`.
Permuting `r most_imp` resulted in an increase in classification error by a factor of `r max(imp.dat$importance)`.
But wait, how can `species` be an important feature as well, because I actually trained 3 models separately.
The deal is that I treat the 3 models as one black box model here.
To this overall function, `species` appears as just another feature, and internally this feature splits the data and then we have the three logistic regression models.
That's why we will look next into conditional feature importance.
...
Meaning the data are permuted by species, so that, for example, it couldn't happen that the body mass of a heavy Gentoo was assigned to a more lightweight Adelie.
While grouping reduces the correlation problem, it isn't solved, as we can see in @fig-cor-by-species.
For example, it can still happen that a Gentoo penguin with a body mass of 4000 grams gets "assigned" a flipper length of 230mm, which produces an unrealistic data point.
...
Note that for the three logistic regression models its natural to see it as three prediction models for which we can compute separate PFIs.
However, since PFI is model-agnostic, we could do the same for a random forest which simply uses species as a feature.
Or we could split into subgroups by any other feature, in theory even using variables that weren't used by the model.

```{r}
#| label: fig-importance-penguins-by-species 
#| fig-cap: PFI by species for each logistic regression. The results are quite similar to overal PFI and don't differ much by species.
