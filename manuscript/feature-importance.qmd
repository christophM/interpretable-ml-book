...
Shuffling produces unrealistic or at least unlikely data points.
And the PFI algorithm just uses these unrealistic data to get model prediction, to compute the loss, and then to compute the importance.
The problem is the *marginal* version of PFI ignores dependencies.
But there is also a *conditional* version of PFI.
...
Two strongly dependent features will have a low to zero conditional PFI even when they are used by the model.
...
```{r}
# eval: false
features = setdiff(names(penguins), "species")

lapply(features, function(fname){
  # TODO: train rpart predicting fname from the other features (except species) using penguins_train, and maxdepth of two
  # TODO: create new penguins dataset based on `penguins_test` with column "subgroup" which is the predicted leaf node ID 
  # TODO: Implement groupby (leaf node ID) and within each group, permute the feature (fname) and compute both original logloss on the subgroup before and ater permutation 
  #  
  # return version of penguins data with:
})
