...
If the absent feature values would be sampled conditional distribution, then the results would not be Shapley values any longer.
The results would violate the Shapley axiom of Dummy, which says that a feature that does not contribute to the outcome should have a Shapley value of zero.
...
While TreeSHAP solves the problem of extrapoloating to unlikely data points, it introduces a new problem.
