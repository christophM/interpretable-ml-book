...
Instead of the Shapley sampling strategy, SHAP uses weighted linear regression inspired by LIME (sampling is still involved though)
...
- Compute weights for each z_k' with the SHAP kernel
- Fit weighted linear model
- Return the $\phi_k$, the coefficients from the linear models (Shapley values)

Let us talk about the individual steps.
First, sampling of feature coalitions.
A coalition is represented by a vector of 0's and 1's, where a 1 for feature x_j' means that feature value x_j is part of the coalition ("present"), a 0 means it is "absent".
We create a random coalition by repeated coin flips until we have a chain of 0's and 1's.
For example, the vector of (1,0,1,0) means that we have a coalition of first and third feature.
The K sampled coalitions becomes the dataset for the regression model.
The target for the regression model is the prediction for a coalition.
("Hold on!," you say, "The model was not trained on these binary coalition data and can't give predictions for them.".)
To get from coalition of feature values to a valid data instance, we need a function $x=h_x(x')$ where $h_x:\{0,1\}^M\rightarrow\mathbb{R}^p$.
The h_x(x') function maps the '1'-entries to the corresponding value from the instance x, that we want to explain.
Makes sense since a '1' means that this feature value is present in the coalition.
And it maps '0'-entries to the values of another instance that we sample from the data.
This means that we equate "feature value is absent" with "feature value is replaced by random feature value from data".
For tabular data, the mapping between original and simplified feature space is visualized in the following figure:

```{r shap-simplified-feature, fig.cap = "Function h() maps a data instance to a binary vector which indicates which features are equal to features from x the instance to be explained. The inverse function maps the binary vector back to a valid instance. Absent features (with a 0 in simplified feature space) are replaced by sampled values.", out.width=800}
...
We have to define h_x in such a way that it samples the absent features conditional on features in set S
As explained above, h_x samples from the marginal distribution.
Sampling from the marginal distribution means ignoring the dependence structure between present and absent features.
KernelSHAP therefore suffers from the same problem that all permutation-based interpretation methods suffer from:
The estimation involves unlikely instances and can become unreliable.
...
<!-- Kernel -->
The procedure so far is very similar to both the Shapley sampling approach and LIME..
The big difference to LIME comes now:
It is how we weight the instances.
LIME weights the instances by how close they are to the original instance.
The more 0's there are in the coalition vector, the smaller the weight in LIME.
SHAP weights the sampled instances by the weight the coalition would get in the Shapley value estimation.
Coalitions that are close to the original instance (i.e. many 1's) and coalitions that are very different from the original instance (i.e. many 0's) get the largest weights.
The intuition behind this is:
We learn the most about individual features when we can study its effect in -- more or less -- isolation.
When a feature is the only one in a coalition, we can learn about its isolated main effect on the prediction.
When a feature is the only one missing from a coalition, we can learn about the interaction effects with all other features, since the difference between the coalition with all features and the one missing is the main effect of the feature plus all interactions.
But when a feature is added to a very specific coalition, we learn little since there are many coalitions possible with the same number of features.
To achieve this Shapley-conform weighting, the authors propose the SHAP kernel:

$$\pi_{x'}(z')=\frac{(M-1)}{(M{}choose|z'|)|z'|(M-|z'|)}$$

Here, M is the number of coalition features, $|z'|$ the number of present features in instance z'.
...
<!-- Linear Model -->
SHAP reformulates this problem as a weighted linear regression problem on the simplified feature space.
