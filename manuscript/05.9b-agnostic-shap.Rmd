...
Shap is an additive feature attribution method which fits following model:
...
The $\phi$ result in the Shapley Values.
...
$$\phi_{i,j}=\sum_{S\subeteq\\\{i,j\}}\frac{|S|!(M-|S|-2)!}{2(M-1)!}\delta_{ij}(S)$$
...
```{r, fig.cap="Stacked SHAP feature attributions clustered by explanation similarity. Each position on the x-axis is an instance from the data. Red SHAP values increase the prediction, blue ones decrease it."}
```
