...
But this is the perfect model-agnostic methods to start the book with, since it teaches the basic principles of model-agnostic interpretation.
...
- ICE plots is a CP plot containing all CP curves for an entire dataset.
- A partial dependence plot is the average of all CP curves of one dataset.

![](./images/cp-ice-pdp.jpg){#fig-cp-ice-pdp}
...
1. Create an equidistant value grid: $z_1, \ldots, z_k$, where typically $z_1 = \min_{i\in \{1,\ldots,n\}}(x_j^{(i)})$ and $z_k = \max_{i\in \{1,\ldots,n\}}(x_j^{(i)})$.
...
1. Visualize the CP curves:
     - Plot line for data point $\left\{\mathbf{x}^{(i)}_{x_j := z_l}, \hat{f}(\mathbf{x}^{(i)}_{x_j := z_l})\right\}_{l=1}^k$
...
1. For each category $z_l \in \{z_1, \ldots, z_k\}$:
   1. Create new data point $\mathbf{x}^{(i)}_{x_j := z_l}$
   1. Get prediction $\hat{f}(\mathbf{x}^{(i)}_{x_j := z_l})$
1. Create bar plot or dot plot with categories on x-axis and prediction on y-axis.

But that sounded more complex then necessary.
...
For our first example, we look at the random forest predicting probability of a penguin being female.
We look at the first penguin in the test dataset, a `r data_point["sex"]` `r data_point["species"]` penguin with a bill length of `r data_point["bill_length_mm"]` millimeters.
@fig-ceteris-penguins shows how changing the bill length changes the prediction.

```{r}
#| label: fig-ceteris-penguins
#| fig-cap: The line shows the predicted value for a particular penguin when changing the bill length. Increasing the bill length slightly increases P(female) for this penguin. The penguin's actual bill length is marked with a dot. The prediction function is very wiggly.
...
plot(cp_rf,  selected_variables="bill_length_mm") +
  xlab("Bill length in mm") +
  scale_y_continuous("P(female)", limits = c(0,1)) +
  ggtitle("")
```

Since it's a binary classification task, visualizing the P(male) is redundant -- it would just be the inverted P(female) plot.
But would we have more than two classes, we could plot the ceteris paribus curves for all the classes into one plot.
...
Looking only at pairwise dependencies may be insufficient.
...
#| fig-cap: Manipulating a penguins bill depth, can create an unrealistic data point but when only looking at pairwise dependencies, we will overlook it. 
...
But this time we visualize it for all of the numerical features:
...
@fig-whatif-bike shows that changes in the number of rented bikes 2 days before would change the prediction the most.
Also a higher temperature would have been better for more bike rentals.
Would the prediction be for a non-working day, the SVM would predict fewer bike rentals.
...
We can compute CP curves for different model to better understand how they treat features differently.
In @fig-cp-bike-models we compare the ceteris paribus plots for a decision tree, a random forest, a linear regression model and an SVM. 
...
The tree shows one jump at around 8 degrees C. 
The random forest looks a bit like an interpolation between the tree curve and the SVM curve. 
...
- Compare features.
- Compare models from different ML algorithms or with different hyperparameter settings.
- Compare class probabilities.
- Compare different data points (but this already goes into the direction of ICE curves, another IML method).
...
They are building blocks for other interpretation methods, but you can also get creative in combining these lines across models, classes, hyperparameter settings, and features to create nuanced insight into the model predictions.
...
When feature are correlated, not all parts of the curve are likely or might even be completely unrealistic.
...
This is a problem with all ML model interpretation, but the risk of wrongful causal interpretation may be higher with CP plots since there is a lower barrier to showing the plots to non-experts. 
...
You can also create CP plots with any tool that can produce ICE plots, like ICEBox and iml, by simply providing as "dataset" that only contains the one data point you are interested in. 
However, the ceterisParibus package is better suited because it makes it simpler to compare ceteris paribus curves.

The ceterisParibus method is also available in .


[^hate-latin]: I hate Latin, but for lack of a better name and ceteris paribus already in use ("what-if" plots are too general).  In school they promised me that Latin helps with learning other Latin-based languages, like Italian. But you know what even helps better when your goal is to learn Italian? That's right, you learn skip Latin and learn Italian. They also said Latin is a "mathematical" language that, a probably even bigger lie.
