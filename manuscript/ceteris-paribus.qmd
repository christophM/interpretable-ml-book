...
Since it's a binary classification task, visualizing $\mathbb{P}(\text{male})$ is redundant -- it would just be the inverted $\mathbb{P}(\text{female})$ plot. But if we had more than two classes, we could plot the ceteris paribus curves for all the classes into one plot.
```{r}
