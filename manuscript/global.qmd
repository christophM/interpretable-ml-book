...
If the fit were perfect, we could throw away the support vector machine and use the tree instead.
 The distributions in the nodes show that the surrogate tree predicts a higher number of rented bikes when temperature is above 13 degrees Celsius and when the day was later in the 2 year period (cut point at 435 days).
...
#| warning: false
#| echo: false
#| fig-cap: The terminal nodes of a surrogate tree that approximates the predictions of a support vector machine trained on the bike rental dataset.pred.bike = Predictor$new(bike_svm, data = bike_train)
