...
We use the logistic regression model to predict [whether a penguin is male](#penguins) based on penguin characteristics.
...
penguins_binary <- penguins %>%
    drop_na() %>%
    mutate(
        is_adelie = ifelse(species == "Adelie", 1, 0),
	is_gentoo = ifelse(species == "Gentoo", 1, 0),
	is_chinstrap = ifelse(species == "Chinstrap", 1, 0),
        island = factor(island),
        # Define chonkiness - let's say top 25% of body mass are chonky
        is_chonky = ifelse(body_mass_g > quantile(body_mass_g, 0.75), 1, 0),
        # Also add a categorical chonk_level for fun
        chonk_level = case_when(
            body_mass_g <= quantile(body_mass_g, 0.25) ~ "Smol_Penguin",
            body_mass_g <= quantile(body_mass_g, 0.75) ~ "Regular_Penguin",
            TRUE ~ "Absolute_Unit"
        )
    )
# Convert chonk_level to factor and set reference level
penguins_binary$chonk_level <- factor(penguins_binary$chonk_level, levels = c("Smol_Penguin", "Regular_Penguin", "Absolute_Unit"))
penguins_binary$chonk_level <- relevel(penguins_binary$chonk_level, ref = "Smol_Penguin")

levels(penguins_binary$chonk_level) <- c("Smol_Penguin", "Regular_Penguin", "Absolute_Unit")
mod <- glm(sex ~ bill_length_mm + chonk_level, data = penguins_binary, family = binomial)
...
kableExtra::kbl(coef.table[, c('Weight', 'Odds ratio', 'Std. Error')], digits=2, caption='The results of fitting a logistic regression model on the penguins dataset. Shown are the features used in the model, their estimated weights and corresponding odds ratios, and the standard errors of the estimated weights.', booktabs = TRUE) %>% kable_styling(position = "center")

```

Maybe a quick note is in order.
The "chonk_level" is a binarization of the body mass feature: Penguins in the top quantile of body mass (75% to 100%) chonky=TRUE, the other are chonky=FALSE.
I typically don't recommend binarization of continuous features, since it means missing out on information.
In this case, I used binarization to illustrate the interpretation of a categorical feature with logistic regression.

Interpretation of a numerical feature ("bill_length_mm"):
An increase in a penguins bill length (increases) the odds of being a male penguin by a factor of `r sprintf('%.2f', coef.table['bill_length_mm', 'Odds ratio'])`, when all other features remain the same.

Interpretation of a categorical feature ("chonk_level"):
For penguins, the odds of being a male versus female are by a factor of `r sprintf('%.2f', coef.table['chonk_levelRegular_Penguin', 'Odds ratio'])`, compared to less chonky penguins (`Smol_Penguin`), given all other features remain the same.
And the odds of being male is even `r sprintf('%.2f', coef.table['chonk_levelRegular_Penguin', 'Odds ratio'])` higher for the absolute units of penguins (25% heaviest) compared to the small penguins.
