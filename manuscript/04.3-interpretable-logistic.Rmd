...
Logistic regression can be seen as an extension of the linear regression model for classification problems.
...
$$log\left(\frac{P(y^{(i)}=1)}{1-P(y^{(i)}=1)}\right)=log\left(\frac{P(y^{(i)}=1)}{P(y^{(i)}=0)}\right)=\beta_{0}+\beta_{1}x^{(i)}_{1}+\ldots+\beta_{p}x^{(i)}_{p}$$
...
$$\frac{P(y^{(i)}=1)}{(1-P(y^{(i)}=1))}=odds^{(i)}=exp\left(\beta_{0}+\beta_{1}x^{(i)}_{1}+\ldots+\beta_{p}x^{(i)}_{p}\right)$$
...
$$\frac{odds^{(i)}_{x_j+1}}{odds^{(i)}=\frac{exp\left(\beta_{0}+\beta_{1}x^{(i)}_{1}+\ldots+\beta_{j}(x^{(i)}_{j}+1)+\ldots+\beta_{p}x^{(i)}_{p}\right)}{exp\left(\beta_{0}+\beta_{1}x^{(i)}_{1}+\ldots+\beta_{j}x^{(i)}_{j}+\ldots+\beta_{p}x^{(i)}_{p}\right)}$$
...
$$\frac{odds^{(i)}_{x_j+1}}{odds^{(i)}=exp\left(\beta_{j}(x^{(i)}_{j}+1)-\beta_{j}x^{(i)}_{j}\right)=exp\left(\beta_j\right)$$
...
If you have a weight (=odds ratio) of 0.7, then increasing the respective feature by one unit multiplies the odds by exp(0.7) (approximately 2) and the odds change to 4.
