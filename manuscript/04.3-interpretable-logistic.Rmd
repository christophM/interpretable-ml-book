...
Logistic regression is the equivalent of the linear regression model for classification problems.
...
Why is that?
In case of two classes, you could label one of the classes with 0 and the other with 1 and use a linear model.
Technically it works and most linear model programs will spit out weights for you.
But there are a few problems with this approach:

- A linear model doesn't output probabilities, but it treats the classes as numbers (0 and 1) and fits the best hyperplane (for a single feature, it's a line) that minimizes the distances between the points and the hyperplane.
So it simply interpolates between the points, and you can't interpret it as probabilities.
- Also, a linear model extrapolates and gives you values below zero and above one.
...
The higher the value of a feature with a positive weight, the more it contributes to the prediction of a class with a higher number.
Even if classes that happen to get a similar number are not closer than other classes.


```{r linear-class-threshold, fig.cap="A linear model is trained on artificial data to classify a tumor as malignant (1) or benign (0), depending on tumor size. Each point is a tumor, the x-axis shows the size of the tumor, the y-axis the malignancy, points are slightly jittered to reduce over-plotting. The lines show the prediction of the linear model. For the data on the left, we can use 0.5 as classification threshold. After introducing a few more malignant tumor cases, the regression line shifts and a threshold of 0.5 no longer separates the classes."}
library("ggplot2")
...
In the linear regression model, we have modelled the relationship between the outcome and the features with a linear equation:
...
```{r logistic-class-threshold, fig.cap="The logistic regression model finds the correct decision boundary between malignant and benign depending on tumor size. The blue line is the logistic function shifted and squeezed to fit the data."}
...
To do this, we can first apply the exp() function on both sides of the equation:
...
and removing many terms gives us:

$$\frac{odds_{i,x_j+1}}{odds_i}=exp\left(\beta_{j}(x_{i,j}+1)-\beta_{j}x_{i,j}\right)=exp\left(\beta_j\right)$$

In the end, we have something as simple as $\exp(\beta_j)$.
A change of $x_j$ by one unit changes the odds ratio (multiplicative) by a factor of $\exp(\beta_j)$.
We could also interpret it this way:
A change in $x_j$ by one unit changes the log odds ratio by $\beta_j$ units.
...
For example, if you have odds of 2, it means that the probability for $y_i=1$ is twice as high as $y_i=0$.
If you have a $\beta_j$ (=odds ratio) of 0.7, then increasing the respective $x_j$ by one unit multiplies the odds by $\exp(0.7)\approx2$ and the odds change to 4.
But usually you don't deal with the odds and interpret the $\beta$'s only as the odds ratios.
...
You only need L-1 columns for a categorical feature with L categories, otherwise it is over-parameterised.
The L-th category is then the reference category.
You can also use any other encoding that can also be used in linear regression.
...
That's because the weight for that feature would not converge, but the optimal weight would be infinite.
This is really a bit unfortunate, because such a feature is really useful. 
But you don't need machine learning if you have a simple rule that separates both classes.
