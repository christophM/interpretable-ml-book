...
Using our look-at-the-formula tactic, we would conclude that therr is an interaction between features $x_1$ and $x_2$, but not individual feature effects.
...
Viewing the linear regression model through the lens of functional decomposition, we see that the model itself represents a functional decomposition of the true function that maps from features to target, but under strong assumptions that the effects arelinear effects and there are no interactions.
...
Viewed through the functional decomposition lense, we can see that the permutation "destroys" the effect of all components in which the feature was involved.
