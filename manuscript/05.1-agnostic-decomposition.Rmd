...
The estimation is done on a grid of points in the feature space, and stated as a minimization problem that can be solved using regression techniques.
...
ALE plots (Apley and Zhu 2021[^ale]) also provide a functional decomposition, meaning that adding up all ALE plots from intercept, 1D ALE plots, 2D ALE plots and so on, you end up with the prediction function.
ALE differ from the (generalized) functional ANOVA, as the components are not orthogonal, but, as the authors call it, pseudo-orthogonal.
To understand pseudo-orthogonality, we have to define the operator $H_S$ that takes a function $\hat{f}$ and maps it to its ALE plot for feature subset $S$.
For example, the operator $H_{1,2}$ takes as input a machine learning model and produces the 2D ALE plot for features 1 and 2: $H_{1,2}(\hat{f}) = f_{ALE,12}$.
If we apply the same operator twice, we end up with the same ALE plot.
...
This is possible, because the 2D ALE component itself is again a function.
The result is again $f_{ALE,12}$, meaning we can apply the same operator multiples times and always end up with the same ALE plot.
...
The answer is zero.
If we first use ALE operator $H_S$ on a function, and then apply $H_U$ on the result (with $S \neq U$) then the result is zero.
In other words, the ALE plot of an ALE plot is zero, unless you apply the same ALE plot twice.
Or in yet other words, the ALE plot for feature set S, has zero other ALE plots in it.
Or in mathematical other words, the ALE operator maps to orthogonal subspaces of an inner product space.

As Apley and Zhu (2021) state, pseudo-orthogonality can be more desirable than hierarchical orthogonality, because it does not entangle marginal effects of the features.
Furthermore, ALE does not require an estimate of the joint distribution, the components can be estimated in a hierarchical fashion, meaning that to compute the ALE effect for features 1 and 2, the only requirements are computations of ALE components of 1 and 2 individually, and nothing else.
...
As a consequence, adding up all PDPs for all subsets will not return the original function, and therefore not be a valid decomposition.
But could we adapt the PDP, by maybe removing all lower effects?
Yes, we could, but we would en up with something similar to the functional ANOVA.
However, instead of integrating over a uniform distribution, the PDP integrates over the marginal distribution of $X_{-S}$, estimated using Monte Carlo sampling.

### Advantages

I see functional decomposition as a core concept of machine learning interpretability.
...
Functional decomposition also enables a better understanding of other methods.
For example, [permutation feature importance](#feature-importance) breaks the association between a feature and the target.
Viewed through functional decomposition, we can see that the permutation "destroys" the effect of all components in which the feature was involved .
This involves the main effect of the feature, but also all interactions with other features.
As another example, Shapley values decompose a prediction into additive effects of each feature.
...
Shapley values provide a fair attribution of effects to the individual features, meaning that also all interactions are attributed fairly to the features, and are divided up among the "main" effects.

Looking at functional decomposition as a practical technique, there are many advantages in using ALE plots.
...
The concept of functional decomposition quickly reaches its limits early for high-dimensional components beyond interactions of two features.
This exponential explosion of number of features both limits the practicability, since we cannot easily visualize higher-order interactions, but also the computational time is insane, if we were to compute all interactions.

Each functional decomposition method has drawbacks.
