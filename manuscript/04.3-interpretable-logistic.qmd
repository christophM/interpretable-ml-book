...
```{r}
#| label: logistic-class-threshold
...
We use logistic regression to predict [whether a penguin is male](#penguins) based on penguin characteristics.
The following table shows the estimated weights, associated odds ratios, and standard errors of the estimates.

```{r}
#| label: logistic-example
...
kableExtra::kbl(coef.table[, c('Weight', 'Odds ratio', 'Std. Error')], digits=2, caption='The results of fitting a logistic regression model to the penguin dataset.', booktabs = TRUE) %>% kable_styling(position = "center")
...
For regular penguins, the odds of being male versus female are increased by a factor of `r sprintf('%.2f', coef.table['chonk_levelRegular_Penguin', 'Odds ratio'])`, compared to lightweight penguins ("Smol_Penguin"), given all other features remain the same.
And for the absolute units among the penguins, the odds of being male is `r sprintf('%.2f', coef.table['chonk_levelRegular_Penguin', 'Odds ratio'])` compared to the small penguins.
