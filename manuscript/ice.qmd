...
A more formal definition: In ICE plots, for each instance in $\{(x_{S}^{(i)},\mathbf{x}_{C}^{(i)})\}_{i=1}^{n}$, the curve $\hat{f}_{S}^{(i)}$ is plotted against $x^{(i)}_{S}$, while $x^{(i)}_{C}$ remains fixed.
...
$$\hat{f}(\mathbf{x}) = \hat{f}(\mathbf{x}_S, \mathbf{x}_C) = g(\mathbf{x}_S) + h(\mathbf{x}_C), \quad\text{with}\quad\frac{\partial \hat{f}(\mathbf{x})}{\partial x\mathbf{x}_S} = g'(\mathbf{x}_S)$$
...
In addition to displaying the individual curves for the derivative of the prediction function with respect to the feature in $S$, showing the standard deviation of the derivative helps to highlight regions in feature $S$ with heterogeneity in the estimated derivatives.
