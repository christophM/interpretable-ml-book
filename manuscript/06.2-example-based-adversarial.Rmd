...
```{r adversarial-toaster, fig.cap = "A sticker, when added to a scene, makes a VGG16 classifier trained on ImageNet categorize an image of the scence as toaster. Work by Brown et. al (2017)."}
knitr::include_graphics("images/adversarial-toaster.png")
```

A big difference to previously presented methods for adversarial examples is that the constraint is lifted that the adversarial image has to be very close to the original image.
Instead the methods creates an image by replacing a part of the image completely with the patch. 
...
This next method is literally adding another dimension to the to the toaster:
...
The difficult thing that the authors achieved is that they found a way to create an adversarial example in 3D for a 2D classifier, that is adversarial over all possible transformations (all the ways of turning the turtle, zooming in etc.)
