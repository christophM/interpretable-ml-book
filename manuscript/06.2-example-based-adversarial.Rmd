...
[Counterfactual examples](#counterfactual) are not only good for understanding the decision, but also for fooling machine learning models.
Instances that are designed to fool a model into giving a wrong prediction are also called adversarial examples.
Adversarial examples must be so similar to the original instance, so that a human couldn't tell the difference, but the prediction changes to a wrong value.
...
The differences are the focus on fooling instead of interpreting and using image data instead of tabular data.

Why do we even care about adversarial examples?
Are they not only some curiose by-products of machine learning models, which have no real isssue?
The answer is a clear No.
With machine learning being used in many products, adversarial examples make any system vulnerable to bad people trying to exploit it.
Here are some fictious scenarios:
A self-driving car crashes because it doesn't recognize a stop sign. 
Someone had sticked an image over the sign, which looks like a stop sign to humans, but was design to look like a no-parking-here sign to machine learning models.
A spam detector missclassifyies spam as valid mail.
The spam mail was designed to resemble a normal mail, but of course trying to scam the recipient at the same time.
...
There are many adversarial examples in image recognition, but also in text.
One of the frist adversarial examples hit spam filtering models in the early 2000s.
The examples in this section focus on image classifiers using deep neural networks, because they have lots of research going on and are easy to fool.

### Examples

There are many ways of creating these adversarial examples (with [counterfactuals](#counterfactual) being one of them).
Some require access to the gradients of the model (which makes those methods model-specific) and some only work with input features and predictions (which makes those methods model-agnostic).
All have in common, that they minimize the distance of adversarial example and instance to be manipulated, while moving the prediction to the desired class.
...
A possible source for deep neural networks being vulnerable to adversarial examples is that data points that are close in input space can be very different in the deep space (think of the embedding of an image or text).
...
<!-- Example 1-->
The first example shows adversarial examples produced by Szegedy et. al (2013)[^szegedy].
This line of research uses box-constrained L-BFGS optimization to find adversarial examples.
...
```{r adversarial-toaster, fig.cap = "A sticker when put into an image makes a VGG16 classifier trained on ImageNet categorize everything as toaster. Work by Brown et. al (2017)."}
