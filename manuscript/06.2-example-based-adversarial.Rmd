...
An adversarial example is an instance with small, intentional feature perturbations to cause a machine learning model to make false prediction.
I recommend reading the chapter about [counterfactual explanations](#counterfactual) first, as the concepts are very similar.
Adversarial examples are counterfactual examples in which the aim is not to interpret a system but to deceive it.
...
A self-driving car crashes into another car because it doesn't recognize the stop sign.
...
Some methods require access to the gradients of the model, which of course only works with gradient based models such as neural networks, other methods only require access to the prediction function, which makes theses methods model-agnostic.
The methods in this section focus on image classifiers with deep neural networks, as  a lot of research is done and the visualization of adversarial images is very revealing.
...
```{r adversarial-panda, fig.cap = "Goodfellow et. al (2014) make a panda look like a gibbon for a neural network. By adding small perturbations (middle image) to the original panda pixels (left image), the authors create an adversarial example that is classified as a gibbon (right image) but looks like a panda to humans."}
...
where $\bigtriangledown_x{}J$ is the gradient of the loss function of the model with respect to the original input pixel vector x, y is the true label vector for x and $\theta$ is the model parameters.
...
Goodfellow et. al (2014) suggested protecting neural networks by adding adversarial examples to the training data to learn robust prediction models.
...
```{r adversarial-1pixel, fig.cap = "By intentionally changing a single pixel (marked with red circles) a neural network trained on ImageNet is deceived to predict the wrong class (in blue letters) instead of the original class (in black letters). Work by Su et. al (2017)."}
...
A population of individuals called candidate solutions recombines generation by generation through until a solution is found.
...
```{r adversarial-toaster, fig.cap = "A sticker that makes a VGG16 classifier trained on ImageNet categorize an image of a banana as a toaster. Work by Brown et. al (2017)."}
...
```{r adversarial-turtle, fig.cap = "A 3D-printed turtle that is recognized as a rifle by TensorFlow’s standard pre-trained InceptionV3 classifier. Work by Athalye et. al (2018)"}
knitr::include_graphics("images/adversarial-turtle.png")
```

The authors have found a way to create an adversarial example in 3D for a 2D classifier that is adversarial over tansformations, such as all possibilities to rotate the turtle, zoom in and so on.
...
Instead of minimizing the distance between the adversarial example and the original image, EOT minimizes the expected distance between the two, given a selected distribution of possible transformations.
...
Howerver, these 3D-printed, robust adversarial examples can be inserted into any real  scene and deceive a computer to wrongly classify an object.
...
This type of zero-knowledge attack is called black box attack.

How it works:

1. Start with a few images that don't have to come from the training data, but should come from the same domain (e.g. if the classifier to be attacked is a digit classifier, use images of digits).
...
1. Create new synthetic images using a heuristic that examines for the current set of images in which direction to manipulate the pixels to make the model output vary more.
...
The authors tested this approach by attacking neural image classifiers trained on various cloud machine learning services.
...
The authors were able to find adversarial examples for various providers, with up to 84.24% of the adversarial examples being misclassified.

The method even works if the black box model to be deceived is a not a neural network.
...
As machine learning is integrated into more and more system, suach as autonomous vehicles or medical devices, they are also becoming entry points for attacks.
Even if the predictions of a machine learning model are on a dataset are 100% correct, there will still be adversarial examples to deceive it.
...
Assuming you donn't know that these spammers exist and the only abuse of the email service is sending pirated copies of music, then the defense would be different (e.g. scanning the attachments for copyrighted material instead of analysing the text for spam indicators).

Being proactive means to actively testing and identifying weakpoints of the system.
...
Using interpretability methods to understand which features are important and how features affect the prediction is also proactive step in understanding the weaknesses of a machine learning model.
As the developer of a model, do you trust your model in this dangerous world without ever having looked beyond the predictive power on a test dataset?
...
Just think of the many different types spam that are constantly evolving.
New method of attacks against machine learning models are invented and new defensive measures are proposed  against these new attacks.
