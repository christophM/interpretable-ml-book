...
[Counterfactual examples](#counterfactual) are not only good for understanding the decision, but also to fool machine learning models.
The field of fooling machines by slightly manipulating an instance is called adversarial examples.
Adversarial examples are also called evasion attacks  that happen during test or application time.
The broader research area is also known as adversarial machine learning, which also includes attacks during training time not covered here.
The goal is to search a datapoint x', that is very near to x so that a human couldn't tell it is different or should be of a different class.
But the model thinks it is some other class than x.


*Keywords: adversarial examples, counterfactuals, evasion attacks, machine learning security*
...
<!-- examples of adversarial in the wild-->
Evasion is meant in the sense that the attacker tries to evade or fool the system.
Here are some scenarios:
Self-driving car not recognizing stop sign.
A surveillance camera not recognizing a weapon.
A spam detector missclassifying spam as valid mail.
...
This section focuses on image classifiers with deep learning, because they have lots of research going on and are easy to fool.
...
They use very similar technique to finding counterfactuals:
Minimize the following:

$$c\cdot|r|+loss(x+r,l)$$

Where x is the image, r are the changes to the pixels to create the adversarial image, l is the desired outcome class and parameter c for balancing distance of images and distance between predictions.
There are additional constraints on r, so that the pixel values stay between 0 and 1. 
The propose to solve this with a box-constrained L-BFGS.

This is the same the counterfactuals: The first term is the distance between the original instance and the adversarial example, the second term is the difference in adversarial prediction and user-defined prediction.
