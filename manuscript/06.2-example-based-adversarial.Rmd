...
Aren't tehy only curious by-products of machine learning models without practical relevance?
...
But it turns out that you can re-use adversarial examples for foolling networks with a different architecture trained on the same task.
...
1. Create new, synthetic images by using a heuristic that explores in which feature direction the model outputs varie most around the current set of images.
...
The classifier then gives predictions for uploaded images, but the model itself can't be acessed or downloaded.
For different providers, the authors managed to find adversarial examples, with up to 84.24% missclassified by one one of the services. 
...
Machine learning deals with known unkowns: classifying unknown data points from  a known distribution.
...
Being reactive, the opposite of proactive, means waiting until the system was attacked and only aftewards understanding the problem and installing some defenses.
...
Another suggested method is to use multiple classifiers instead of only one and leting them vote (ensemble), but this has no guarantee to work, since they might all suffer from similar adversarial examples.
...
With this chapter, I hope to sensibilize you of the problem of adversarial examples and that only by proactively studying the machine learning models we are able to discover and fix vulnerabilities.
