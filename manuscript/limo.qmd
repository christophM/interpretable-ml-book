...
[^statistician-view]: This chapter view linear regression through a statistician lens: You can definitely use linear regression without making any assumption whatsoever, but then you also don't get all the perks like confidence intervals and certain types of interpretations.
...
We will not discuss in detail how the optimal weights can be found, but if you are interested, you can read chapter 3.2 of the book "The Elements of Statistical Learning" (Friedman, Hastie and Tibshirani 2009)[@hastie2009elements] or one of the other online resources on linear regression models.
...
You only need them to get certain more out of the linear model like confidence intervals.
...
For each feature, the @tbl-load-linear-model shows the estimated weight, the standard error of the estimate, and the absolute value of the t-statistic.
...
cap = "Coefficients, standard errors (SE), and absolute value of the t-statistic for linear model predicting bike rentals."
kableExtra::kbl(lm_summary_print[,c('Estimate', 'Std. Error', 't value')], digits = 1, col.names = c('Weight', 'SE', "|t|"), caption = cap, booktabs = TRUE, center = TRUE) %>% kable_styling(position = "center")
```
...
Don't compare coefficients of features with different scales, because the magnitude of a coefficient is affected by the units of the feature:
...
kableExtra::kbl(df, col.names = c("Feature", "Value"), row.names = FALSE, booktabs = TRUE, caption = sprintf("Feature values for instance %i", i)) %>% kable_styling(position = "center")
```
...
With increasing penalty of the weights, fewer and fewer features receive a non-zero weight estimate. These curves are also called regularization paths. 
...
cap = "Weight estimates when using Lasso and setting the number of features to two."
kableExtra::kbl(extract.glmnet.effects(l.mod$beta, max(which(n.features == 2))), col.names = "Weight", caption=cap, digits = 2, booktabs = TRUE) %>% kable_styling(position = "center")
```
...
```{r}
#| label: tbl-lasso_effects2
cap = "Weight estimates when using Lasso and setting the number of features to five."
extract.glmnet.effects(l.mod$beta, max(which(n.features == 5))) %>%
  kableExtra::kbl(col.names = "Weight", digits = 2, caption=cap) %>%
