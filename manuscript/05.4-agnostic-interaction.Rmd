...
When features interact with each other in a prediction model, then the influence of the features on the prediction is not additive but more complex.
...
a constant part, one for the first feature, one for the second feature and one for the interaction effect between the two features.  
The interaction between two features is the change in the prediction function that occurs by varying the features, after having accounted for the individual feature effects.
It's the effect that comes on top of the individual feature effects. 
For example: a model predicts the value of a house, using house size (big or small) and location (good or bad) as features. 
This means that there are four possible types of houses with a distinct prediction each:

| Location | Size  | Predictions |
|---------:|------:|------------:|
| good     | big   | 300,000     | 
| good     | small | 200,000     |
| bad      | big   | 250,000     | 
...
We could decompose the model prediction into the following parts: 
A constant term (150,000), an effect for the size feature (+100,000 if big, +0 if small) and effect for the location (+50,000 if good, +0 if bad). 
This decomposition would completely explain the model predictions.
There is no interaction effect, because the model prediction is a sum of the single effects for size and location. 
Making a small house big always adds 100,000 to the value, no matter the location. 
Also the value difference between a good and a bad location is 50,000, independent of the size.
...
We could decompose the table into the following parts: 
A constant term (100,000), an effect for the size feature (+100,000 if big, +0 if small) and an effect for the location (+50,000 if good, +0 if bad). 
For this table, we need an extra term for the interaction: +100,000 if the house is big and in a good location.
This is an interaction between the size and the location, because in this case, the difference in value between a big and a small house depends on the location. 
...
In theory, arbitrary interactions between any number of features can be measured, but those two cases represent the most useful applications.
...
where $PD_{jk}(x_j,x_k)$ is the partial dependence function for both features and $PD_j(x_j)$ and $PD_k(x_k)$ the partial dependence functions for the single features.
...
This decomposition expresses the partial dependence (or full prediction) function without interaction (between features $x_j$ and $x_k$ or $x_j$ and $x_{-j}$). 
...
In their paper, Friedman and Popescu also propose a test for interaction H-statistic being significantly different from zero:
...
The following plot shows the results of the feature interaction analyis:
...
We analyse the interactions between features in a random forest that is trained to predict [cervical cancer risk](#cervical).
...
ia = Interaction$new(pred.cervical, grid.size = 70) 
plot(ia)
```

After looking into the feature interactions of each feature with all other features, we can pick one of the features and specifically dive deeper into all the 2-way interaction between the chosen feature with the other features:
...
ia = Interaction$new(pred.cervical, grid.size = 70, feature = "Num.of.pregnancies") 
...
This means when we sample points, then the estimates will also vary from run to runand the results **become unstable**.
...
We would need to create a formal test, but this test is not (yet) available in a model-agnostic version. 
...
- The H-statistic tells us how strong the interactions is but, it doesn't tell us how the interaction is shaped.
...
- There are other implementations, which focus on a specific model:
  - The R package [pre](https://cran.r-project.org/web/packages/pre/index.html) implements the [RuleFit algorithm](#rulefit) plus interaction statistic.
  - The R package [gbm](https://github.com/gbm-developers/gbm3) implements gradient boosted models  plus interaction statistic.
...
When the variance is high, then the features interact with each other, if it is zero, there is no interaction.
