...
<!-- Other names -->
You'll see later in this chapter what is going on in this particular image.
...
$$\frac{\delta f}{\delta \mathbf{x}_l} = \frac{\delta f}{\delta \mathbf{x}_{l+1}} \cdot \mathbf{I}(\mathbf{x}_l > 0)$$

Here, $\mathbf{I}$ is the element-wise indicator function, which is zero where the activation at the lower layer was negative, and one where it's positive or zero.
...
where $R_n$ and $R_{n+1}$ are the layer reconstructions, and $\mathbb{I}$ is the indicator function.
...
Figure @fig-smoothgrad shows the pixel attributions that aim to explain the classification.
