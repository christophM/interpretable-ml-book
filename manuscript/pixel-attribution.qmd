...
![A saliency map in which pixels are colored by their contribution to the classification.](./images/vanilla.png){#fig-vanilla}
...
The idea of Vanilla Gradient, introduced by Simonyan et al. (2013) [@simonyan2014deep] as one of the first pixel attribution approaches, is quite simple if you already know backpropagation.
...
Using my incredible ASCII art skill, the ReLU function looks like this: _/ and is defined as $\mathbf{x}_{l+1}(x)=\max(0,\mathbf{x}_l)$ from layer $\mathbf{x}_{l}$ to layer $\mathbf{x}_{l+1}$.
...
Vanilla Gradient has a saturation problem, as explained in Avanti et al. (2017) [@shrikumar2017learning].
...
DeconvNet by Zeiler and Fergus (2014) [@zeiler2014visualizing] is almost identical to Vanilla Gradient.
...
The idea of SmoothGrad by Smilkov et al. (2017) [@smilkov2017smoothgrad] is to make gradient-based explanations less noisy by adding noise and averaging over these artificially noisy gradients.
...
The network under examination is VGG-16 (Simonyan et al. 2014 [@simonyan2015very]), which was trained on ImageNet and can therefore distinguish more than 20,000 classes.
...
![Images of a dog classified as greyhound, a ramen soup classified as soup bowl, and an octopus classified as eel.](./images/original-images-classification.png){#fig-classifications}
...
![Pixel attributions or saliency maps for the Vanilla Gradient method, SmoothGrad and Grad-CAM.](./images/smoothgrad.png){#fig-smoothgrad}
...
Ghorbani et al. (2019) [@ghorbani2019interpretation] showed that introducing small (adversarial) perturbations to an image, which still lead to the same prediction, can lead to very different pixels being highlighted as explanations.

Kindermans et al. (2019) [@kindermans2019unreliability] also showed that these pixel attribution methods **can be highly unreliable**.
...
However, the sanity checks paper itself has found some criticism from Tomsett et al. (2020) [@tomsett2020sanity] with a paper called "Sanity checks for caliency metrics" (of course).
