...
The biggest strength and also the biggest weakness of the [linear regression model](#linear) is that the outcome to be predicted is modeled as a weighted sum of the features.
...
The linear regression model assumes that the outcome of the i-th instance $y_i$ can be expressed by a weighted sum of its p features $x_{ij}$ with an individual error $\epsilon_i$ followig a Gaussian distribution.
...
```{r three-lm-problems, fig.cap = "Three assumptions of the linear model (left half): Gaussian distribution of the outcome given the features, additivity (= no interactions) and linear relationship. Reality usually doesn't adhere to those assumptions (right half): Outcomes might have non-Gaussian distributions, features might interact and the relationship might be nonlinear."}
...
Also if you want to predict probabilities with a linear model, you can get probabilities that are negative or greater than 1.
...
**Example**: The influence of the temperature on my desire to ride a bike could be linear between 0 and 25 degrees Celsius, which means that an increase from 0 to 1 degree causes the same increase in cycling desire as an increase from 20 to 21. 
But at a certain point it levels off and could even decrease the my motivation to cycle - I don't like to bike when it's too hot.
...
The outcome can also be a category (cancer vs. healthy), a count (number of children), the time to the occurence of an event (time to failure of a machine) or a very skewed outcome with unusually many high values (household income).
The binary classification case can be handled by modifying the linear regression model with the logistic function as described in the [logistic regression chapter](#logistic).
...
In the following formula, y is replaced with $E_Y(y|X)$. 
The observed y in the data and the expected outcome can be different things.
...
The link function g, the weighted sum (sometimes called linear predictor) ($\beta{}X$), and a probability distribution from the exponential family that defines $E_Y$.

The exponential family is a set of distributions that can be written with the same (parameterized) formula that inclues an exponent, the mean and variance of the distribution and some other parameters.
...
Each distribution has a canonical link function that can be derived mathematically from the distribution.
...
Along with number of cups, you record your current stress level on a scale of 1 to 10, how well you slept the night before on a scale of 1 to 10 and whether you worked that day.
...
The number of coffees was then drawn from a Poisson distribution, modelling the intensity $\lambda$ (which is also the expected value of the Poisson distribution) as a function of the features sleep, stress and work.
...
Let's look at the distribution target variable, the number of coffees on a given day:
...
On `r tab[1,2]` of the `r n` days you have no coffee at all and on the most extreme day you have `r tab[nrow(tab),1]`.
...
In this case, the data was even generated from the Poisson distribution, so the Poisson GLM is the perfect choice.
...
Increasing the stress by one point multiplies the expected number of coffees by the factor `r round(cc["stress", "exp.beta"], 2)`. 
Increasing the sleep by one point multiplies the expected number of coffees by the factor `r round(cc["sleep", "exp.beta"], 2)`.
The predicted number of coffees is on average `r round(cc["weekendYES", "exp.beta"], 2)` times the number of coffees on a non-working day compared to a working day.
...
To predict the [number of bicycles](#bike-data) rented, there may be an interaction between temperature and the fact whether it's a working day or not.
Perhaps, when people have to work, the temperature doesn't influence the number of rented bikes much, becaus people will ride the rented bike to work no matter what happens.
...
How can we get the linear model include interactions?
...
In the working day and temperature example, we would add a new feature that is zero if it is non-working day, otherwise the value of the temperature feature.
...
The following table shows what the data looks like if we don't specify any interactions to model.
...
If we want the linear model to include the interaction between temperature and the workday feature, we must give it the following matrix with the additional column for the interaction:
...
The new column "workingdayYes" captures the interaction between the features workingday and temperature.
This new feature is zero for an instance if the workingday feature is at the reference category ("No"), otherwise it assumes the value of the instances temperature feature.
...
For each category of both features (except the reference categories), we create a new feature column that is 1 if both features have a certain category, otherwise 0.
...
One of them can be found in the [RuleFit chapter](#rulefit), which first mines interaction terms and then estimates a linear regression model including interactions.

**Example**

Let's return to the [bike rental prediction task](#bike-data) which we have already modeled with a linear model in the [linear model chapter](#linear).
...
The answer is no, even if the weights suggest it to the untrained eye.
We can't interpret the "workingdayWORKING DAY:temp" interaction weight directly, since the interpretation would be:
...
Suppose we have a working day, and want to know what would happen if the temperature were 1 degree warmer today.
Then we need to add both the weights for "temp" and "workindayWORKING DAY:temp" to determine how much the estimate increases.
...
Linearity means no matter what value an instance has in a particular feature, increasing the value by one unit always has the same effect on the predicted outcome.
...
Intuitively one expects that increasing the temperature from 10 to 11 degrees Celsius has a positive effect and from 40 to 41 a negative effect, which is also the case, as you will see from many examples in the book.
The increase the temperature has a linear, positive effect on the number of rental bikes, but at some point it flattens out and even has a negative effect at high temperatures.
...
Before I go into the details of each method, let's start with an example that illustrates all three methods.
...
The following figure shows the estimated slope with: 1) the standard linear model, 2) a linear model with a transformed temperature feature (logarithm), 3) a linear model with temperature treated as categorical feature and 4) using regression splines (GAM).

```{r nonlinear-effects, fig.cap = "Predicting the number of rented bicycles using only the temperature feature. The use a linear model (top left) doesn't capture the flattening of the temperature effect at high temperatures suggested by the data and overestimates the number of bikes at low temperatures. One solution is to transform the feature with e.g. the logarithm (top right), categorize it (bottom left), which is usually a bad decision or use Generalized Additive Models that can automatically fit a smooth curve for temperature (bottom right). GAMs are often the best solution."}
...
When you use a GLM with a link function that is not the identity function, then the interpretation gets more complicated, because you have to incorporate both transformations into the interpretation.

**Feature categorization**

Another, perhaps naive, solution to achieve a nonlinear effect is to discretize the feature; turn it into a categorical feature.
For example, you could cut the temperature feature into 20 intervals with the levels [-10, -5), [-5, 0), ... and so on, you get it.
...
I would only use discretiziation if there is a very strong case for it.
...
Linear effects are also covered by the framework, because for features to be handled linearly, you can limit their $f_j(x_j)$ only to take the form of $x_j\beta_j$.

The big question is how to learn nonlinear functions of some features.
The answer is called "splines" or "spline functions".
Splines are functions that can be added to approximate arbitrary functions.
A bit like stacking Lego bricks to build something more complex.
There are a confusing number of different ways to define these spline functions.
If you are interested in learning more about all the different ways to define splines, I wish you good luck on your journey.
I'm not going to go into detail here, I'm just going to build an intuition.
...
For example, to model the temperature with splines, we remove the temperature feature from the data and replace it with, say, 10 columns, each representing a spline function.
...
This effectively reduces the flexibility of the splines and reduces the danger of overfitting. 
...
```{r splines, fig.cap = "To smoothly model the temperature effect, we use 9 spline functions. Each temperature value is mapped to (here) 9 spline values. If an instance has a temperature of 30 C, then the value for the first spline feature is -1, for the second 0.5 ... and for the 9th it's 2. This feature matrix is provided to the GAM fitting function."}
...
- The opacity of many machine learning models comes from 1) a lack of sparseness, which means that many features are used 2), features that are treated in a nonlineary, which means you need more than a single weight to describe the effect 3) the integration of interactions between the features.
...
As promised, here is a list of modeling problems you might have with your linear model, along with the name of a solution for this problem that you can copy and paste into your favorite search engine.
...
For example, when predicting the value of a house, the errors are usually higher in expensive houses, which violates the homoscedasticity of the linear model.
...
If the outcome has two categories use a [logistic regression model](#logistic), that model sthe probability for the categories.  
...
Also, if you have the problem that the count value of 0 is very frequent:  
