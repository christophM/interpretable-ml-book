...
The linear regression model assumes that the outcome y of the i-th instance can be expressed by a weighted sum of its p features with an individual error $\epsilon^{(i)}$ that follows a Gaussian distribution.
...
$$g(E_Y(y^{(i)}|x^{(i)}))=\beta_0+\beta_1{}x^{(i)}_{1}+\ldots{}\beta_p{}x^{(i)}_{p}$$
...
$$x^{(i)}\beta=ln\left(\frac{E_Y(y^{(i)}|x^{(i)})}{1-E_Y(y^{(i)}|x^{(i)})}\right)=ln\left(\frac{P(y^{(i)}=1|x^{(i)})}{1-P(y^{(i)}=1|x^{(i)})}\right)$$

And if we solve this equation to have P(y^{(i)}=1) on one side, we get the logistic regression formula:

$$P(y^{(i)}=1)=\frac{1}{1+exp(-x^{(i)}\beta)}$$
...
$$g(E_Y(y^{(i)}|x^{(i)}))=\beta_0+f_1(x^{(i)}_{1})+f_2(x^{(i)}_{2})+\ldots+f_p(x^{(i)}_{p})$$

The formula is similar to the GLM formula with the difference that the linear term $\beta_j{}x^{(i)}_{j}$ is replaced by a more flexible function $f_j(x^{(i)}_{j})$.
...
Linear effects are also covered by the framework, because for features to be handled linearly, you can limit their $f_j(x^{(i)}_{j})$ only to take the form of $x^{(i)}_{j}\beta_j$.
