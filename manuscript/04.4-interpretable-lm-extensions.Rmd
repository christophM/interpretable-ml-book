...
The categorical feature is coded, with 0 for the reference category and 1 for the other.

TODO: CONTINUE HERE


When we add the interaction between temperature and the working day feature, the model will receive the following matrix with the additional column for the interaction:

```{r data-frame-lm}
mod = lm(1:4 ~ workingday * temp, data = x)
knitr::kable(data.frame(model.matrix(mod)))[,-1]
```

The first column is the working day feature, which is now only the indicator for the feature value Yes, since No is assumed as the reference category.
