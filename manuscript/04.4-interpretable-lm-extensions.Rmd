...
**Problem**: The target outcome y is not Gaussian distributed given the features. 
...
There are many other scenarios in which you will reach the boundaries of the linear regression model, but I don't have the time nor the motivation to cover it all here. because this would quickly become a book within a book.
But it's very useful to have all the stuff in one place, so at the [end of the chapter](#more-lm-extension) is a list of problem statements + the name of a solution that you can copy-paste into search.

### Different types of targets: GLMs {#glm}
...
Some outcomes are categories (cancer vs. healthy), a count (number of children), the time until an event happens (time until a machine breaks down) or a very skewed variable with many high values (household incomes).
As we have seen in the [logistic regression chapter](#logistic), the binary categorization case can be handled when we modify the linear regression model with the logistic function, while the weighted sum of the inputs remains at the core.
This concept of keeping the linear weighted sum at the core, but transforming the sum and modeling the mean of a distribution different from the Gaussian (the Bernoulli distribution in the logistic regression case) is called **Generalized Linear Models (GLMs)**.
GLMs are a flexible class of models, which (partially) keeps the interpretability of the linear model.

Mathematically the GLM links the weighted sum of the features to the mean of the assumed distribution through a so called link function g, which can be chosen flexibly depending on the type of outcome. 
...
### Adding interactions {#lm-interact}
...
### Non-linear effects: GAMs and more. {#gam}


*The world is not linear.*
Linearity means no matter the value an instance has in a particular feature, increasing the value by one unit always has the same effect on predicted outcome.
Is it reasonable to assume that increasing the temperature by one degree has the same effect at 10 degrees Celsius and at 40 C for the number of rented bikes? 
As you will see throughout the book, the intuition is correct:
...
If you feed it the data, it will dutifully find the best plane (= least squares).
You have a chance to model non-linear relationships with one of the following techniques:
...
Before I explain each of them, we will start with an example right away.
We take the [bike rental dataset](#bike-data) and fit a linear model with only the temperature feature to predict the number of rented bikes.
The following figure shows the estimated slope with: 1) the default linear model, 2) a linear model with a transformed features (logarithm of temperature), 3) a linear model with temperature treated as categorical feature or 4) using regression splines (GAM).

```{r non-linear-effects, fig.cap = "Predicting the number of rented bikes given the temperature. Using a linear model (top left) doesn't capture the flattening of the temperature effect as suggested by the point cloud and overestimates the number of bikes at low temperatures. One solution is to transform the feature with e.g. the logarithm (top right), to categorize it (bottom left) which is usually a bad decision or to use Generalized Additive Models, which can automatically fit a smooth curve for temperature (bottom right), often the best solution."}
...
Another solution: We discretize the feature, turn it into a categorical feature.
So the temperature would become a categorical feature with the categories [-10, -5), [-5, 0), ... and so on, you get it.
Then the linear model would estimate a step function, because each level gets its own estimate.
The problem with this approach is that you need a lot more data, are more likely to overfit and have the additional problem to choose a meaningful way to discretize the feature (equidistant intervals? quantiles? how many intervals?).
Also, you loose information, the same way as capturing an analogue audio signal as a discrete digital signal always comes with a loss.
