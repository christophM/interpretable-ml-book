...
Features:
- Stress level between 1 and 100
...
n = 100
df = data.frame(x1  = runif(n = n, min = 1, max = 100), 
  x2 = runif(n = n, min = 1, max = 2), 
  x3 = sample(c("a", "b"), size = n, replace = TRUE))
lambda = log( pmax(df$x1/100 + df$x2  - 1 * (df$x3 == "a"), 1))
...
mod.pois = glm(y ~ ., data = df, x = TRUE, family = poisson())
...
kable(summary(mod.pois)$coefficients)
```

TODO: Interpretation, combined with the link function.

TODO: Add general statement how to interpret results in glm, depending on the link function.
...
TODO: Research options to add interactions automatically
TODO: Link to RuleFit for automatically detecting interactions

TODO: Talk about different types of interaction (multiplicative, nested, etc.)
TODO: Explain how the interpretation becomes a lot more difficult

TODO: example showing how complicated the interpretation is
      first model with the two features
      second model with the two features with interaction
      show that substantially different
      
TODO: link AME as a solution
...
TODO: Plot with single feature, where a log transformation makes sense
Example with log-transformed feature
TODO: Plot with some arbitrary function


Advanced and more flexible: GAMs.

TODO: Figure explaining splines, reusing plot with arbitrary function
      Plot 1: function
      Plot 2: splines 
      Plot 3: arrows showing how splines are weighted
      plot 4: fitted curve
TODO: Text for explaining splines

TODO:  Explain how to interpret splines

*Examples*

TODO: Example with manually transformed features. e.g. bike sharing with temperature as sqrt or so, whatever makes sense.
TODO: Example with bike sharing data with temperature with splines
