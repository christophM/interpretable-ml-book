...
The outcome given the features might have a non-Gaussian distribution, the features might interact and the relationship between the features and the outcome might be non-linear.
...
When you want to predict probabilities with a linear model, you might also get probabilities that are negative or larger than 1.
...
The GLM framework allows to choose the link function independently from the distribution.
...
The assumed distribution together with the link function dictate how tho interpret the estimated feature weights.
...
The additional interaction effect is negative (`r round(lm_summary_print['workingdayWORKING DAY:temp','Estimate'], 1)`) and significantly different from zero, as indicated by the 95% confidence interval, which does not include the zero.
...
GAMs loosen the restriction that the relationship must be a simple weighted sum, and instead assume that it's a sum of arbitrary function of each feature.
Mathematically, the relationship looks like this then:

$$\beta_0+f_1(x_1)+f_2(x_2)+\ldots+f_m(x_m)=g(E_Y(y|x))$$

The formula is similar to the GLM formula with the difference that the linear term $\beta_j{}x_J$ is exchanged with a more flexible function $f_j(x_j)$.
The core of a GAM is still a sum of feature effects, but you have the option to allow more complex, non-linear relationships between some features and the output.
Linear effects are also covered by the framework, because for feature that you want to be handled linearly you can restrict their $f_j(x_j)$ to only take on the form $x_j\beta_j$.

The big question is how to learn non-linear functions of some features.
The answer for GAMs is called splines.
...
If you are interested in learning more about all those different ways to define splines, I wish you good luck on your journey.
I will not go into details here, but only build up some intuition.
What helped me personally the most for understanding splines was to simply visualize the individual spline functions and to look into how the data matrix is modified.
For example, to model the temperature with splines, we remove the temperature feature from the data, and replace it with, say, 10 columns, where each represents a spline function.
The value for each instance for those new spline features depends on their temperature value.
Together with all linear effects, the GAM also estimates then those spline weights.
GAMs additionally introduce some penalty term for the weights to keep them close to zero. 
This effectively reduces the flexibility of the splines and reduces the danger of overfitting. 
A smoothness parameter that is usually used to control the flexibility of the curved is then tuned via cross-validations.
Penalty aside, non-linear modeling with splines is fancy feature engineering.

In the example where we predict the number of bikes using only temperature with splines in a GAM, the model feature matrix looks like this:

```{r splines-df}
kable(head(model.matrix(mod.gam)), digits = 2)
```

Each row still represents an individual instance from the data (a day).
...
```{r splines, fig.cap = "To model the temperature effect smoothly we use 10 spline functions. Each temperature value gets mapped to (here) 9 spline values. If an instance has a temperature of 30 C, then the value for the first spline feature is -1, for the second 0.5 ... and for the 9th it's 2. We feed the GAM with those  spline features."}
...
And the actual curve, which is the sum of the spline function times the weights looks like this:
...
The interpretation of smooth effects requires to visually examine the fitted curve.
...
For example, at 0 degrees C, the predicted number of bikes is 3000 lower than the average prediction.
...
- All these extensions of the linear model are a bit of a universe of their own. 
For whatever problems you face with linear models, **you will probably find an extension that fixes it**.
- Most of the methods have been used for decades, for example GAMs are almost 30 years old. 
The linear model and its extensions have been established in many research field as the status quo for modeling data. 
Because of that, many researchers and practitioners from industry are very **experienced** with linear models and the methods are **accepted as status quo for modeling** in many communities.
- The linear model with its extensions comes with (statistical) batteries included.
...
You get confidence intervals for the weights, significance tests, prediction intervals and much more. 
- Statistical software has usually really good interfaces (for example R) to fit GLMs, GAMs and more special linear models.
- The opaqueness of many machine learning models comes from 1) a lack of sparseness meaning many features are used 2), features being treated in a non-linear fashion (meaning you need more than a single weight to describe the effect) 3) and the methods capturing interactions between the features.
Assuming that linear models are highly interpretable but often underfit reality, the extensions described in this chapter provide a good way to **smoothly transition towards more flexible models**, while keeping some of the interpretability.


### Disadvantages

- As advantage I stated that linear models are living in their own universe.
The sheer **number of ways how you can extend the simple linear model is overwhelming**, not only for beginners.
Actually it's multiple parallel universes, because many communities of researchers and practitioners have their own naming for methods that more or less do the same, which can be very confusing.
- Most of the modifications of the linear model make the model **less interpretable**.
Any link function (in a GLM) that is not the identity function complicate the interpretation;
Interactions complicate the interpretation of the weights, because the weight of the main effect of a feature only caries meaning of an unconditional effect when the effect of the interaction is zero, which is never.;
Non-linear feature effects are either less intuitive (like the log transformation) or cannot be summarized with single number any longer (e.g. spline functions).
- GLMs, GAMs and so on **rely on assumptions** regarding the data generating process.
If those are violated, the interpretation of the weights is not valid any longer.
...
As promised, here is a list of modeling problems you might have with your linear model, along with the name of a solution for that problem, which you can copy-paste into your favorite search engine.
...
For example when predicting the value of a house, the errors are usually higher for expensive houses, which violates the homoscedasticity assumption of the linear model.
Search for: *robust regression*.  
- I have outliers that severely influence my model.  
Search for: *robust regression*.
- I want to predict the time until an event happens.  
Time-to-event data usually comes with censored measurements, meaning for some instances there wasn't enough time to observe the event.
For example, a company wants to predict the failure of their ice cream machines, but has only data for two years.
...
If the outcome has two categories use a [logistic regression model](#logistic), which will model the probability for the categories.  
...
- My outcome to predict is an ordered categories.
For example a grade in school.  
...
- I am not sure which features to include into the model to do proper causal inference.  
