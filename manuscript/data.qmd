...
I trained all regression models using a simple holdout strategy: $\frac{2}{3}$ of the data for training and $\frac{1}{3}$ for testing.
...
@fig-bike-correlations shows that the only larger correlation is between count 2 days before, and the temperature.
...
Normalized mutual information scales MI from $[0, \infty[$ to $[0,1]$:
...
![The three penguin species in the data: Chinstrap, Gentoo, and Adelie. Artwork by \@allison_horst.](./images/lter_penguins.jpg){#fig-penguins width=80%}
...
- Length of the bill (the beak), measured in millimeters (`bill_length_mm`). See also @fig-bill-measurements.
...
![Bill measurements. Artwork by \@allison_horst.](./images/culmen_depth.jpg){#fig-bill-measurements width=70%}
...
For the data examples, I trained the following models, using a simple split into training $\frac{2}{3}$ and holdout test data $\frac{1}{3}$.
...
Combining all penguin data results in a negative correlation between bill depth and body mass.
