...
Mutual information between two random variables $X_j$ and $X_k$ is given by:
...
To use (normalized) mutual information with numerical features, we discretize the observed values of the feature $\mathbf{x}_j$ into bins.  
...
But also other features are correlated, like flipper length and bill length or flipper length and bill length.
