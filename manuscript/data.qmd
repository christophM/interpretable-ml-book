...
That's why we also don't rely on NMI alone, but visualize the raw data and analyze correlation.
...
The mutual information between two categorical features $X_j$ and $X_k$ is given by:

$$
MI(X_j, X_k) = \sum_{c \in X_j} \sum_{d \in X_k} \mathbb{P}(X_j=c, X_k=d) \log \left( \frac{\mathbb{P}(X_j=c, X_k=d)}{\mathbb{P}(X_j=c) \mathbb{P}(X_k=d)} \right),
$$

where $\mathbb{P}(X_j=c, X_k=d)$ is the joint probability that feature $X_j$ takes on category $c$ and $X_k$ category $d$.
$\mathbb{P}(X_j=c)$ is the probability that feature $X_j$ takes on category $c$, and the same for $\mathbb{P}(X_k=d)$.

Normalized mutual information brings MI from $[0, \infty[$ to $[0,1]$:

$$NMI(X_j, X_k) = \frac{2 \cdot MI(X_j, X_k)}{H(X_j) + H(X_k)}$$

where $H(X_j) = -\sum_{c \in X_j} \mathbb{P}(X_j=c) \log(\mathbb{P}(X_j=c)).$
...
That's why we'll use male / female classification  based on body measurements (as did the paper).
...
Again, we look at scatter plots and normalized mutual information:

```{r}
#| label: fig-penguins-nmi
#| fig.cap: Pairwise normalized mutual information 
# give shorter col names for this plot
...
ggplot(data = penguins, aes(x = bill_depth_mm, y = body_mass_g, color = species)) +
  ...
  geom_smooth(method = "lm", se = FALSE, aes(group = 1), color = "black") +  # Regression line for all data
