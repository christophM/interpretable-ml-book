...
This chapter presents MMD-critic by Kim et al. (2016) [@kim2016examples], an approach that combines prototypes and criticisms in a single framework.
...
@fig-mmd illustrates the MMD2 measure.
...
$$\mathrm{witness}(\mathbf{x})=\frac{1}{n}\sum_{i=1}^{n}k(\mathbf{x}, \mathbf{x}_i)-\frac{1}{m}\sum_{j=1}^{m}k(\mathbf{x}, \mathbf{z}_j)$$
...
A negative witness function at point $\mathbf{x}$ means that the prototype distribution overestimates the data distribution (for example, if we select a prototype but there are only few data points nearby);
...
![Prototypes for a handwritten digits dataset.](./images/handwritten-prototypes.png){#fig-prototypes-and-criticisms2}
...
One solution could be a scree plot showing the number of prototypes on the x-axis and the MMD2 measure on the y-axis.
...
The simplest alternative to finding prototypes is [k-medoids](https://en.wikipedia.org/wiki/K-medoids) by Kaufman et al. (1987).[@rdusseeun1987clustering]
