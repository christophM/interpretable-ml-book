...
IF the condition is true THEN make prediction.
Decision rules are probably the best interpretable prediction models.
Their IF-THEN structure semantically resembles natural language and the way we think, provided that the condition is built from intelligible features and the length of the condition (number of $feature=value$ pairs combined with an AND) and there are not too many rules.
In programming it's very natural to write IF-THEN rules.
New in machine learning is that these types of rules are learned through an algorithm.
...
If a house is bigger than 100 square meters and has a garden the value is high.
More formally:
...
- Both are connected with an 'AND', so both must be true for the rule to apply.
...
An exception is the default rule, which has no explicit IF-part applies when no other rule applies, but more about this later.

How do we assess whether a particular decision rule makes sense?
The usefulness of a decision rule is usually summarized in two numbers: Accuracy and support.
...
The prediction (IF-part) is not important for the calculation of support.
...
For example: 
Let's say of the 100 houses, where the rule with condition $size=big$ applies, 85 have $value=high$, 14 $value=medium$ and 1 $value=low$, then the accuracy of the rule is 85%.
...
1. Rules can overlap: 
What if I want to predict the value of a new house and two or more rules apply that imply contradictory predictions?
1. No rule applies:
What if I want to predict the value for a new house and none of the rules apply?
...
If the first rule applies to an instance, we use the prediction of the first rule. 
...
A **decision set** is a set of rules without an order (as opposed to a decision list). 
...
To make a prediction, either a single rule applies and we use the attached prediction, or if several rules apply, we resolve the conflict with e.g. majority vote to make the prediction.
...
The algorithms are chosen to cover a wide range of general ideas for learning rules, so all three are very different approaches.

1. **OneR** learns rules from a single feature.
OneR is characterised by its simplicity, interpretability and its use as a benchmark.
1. **Sequential covering** is a general procedure used by many rule learning algorithms and as such important as basic knowledge.
1. **Bayesian Rule Lists** combines re-mined frequent feature values into a decision list using Bayesian statistics. 
Using pre-mined patterns is a common approach used by many rule learning algorithms.
I contrast to the other examples, a statistical approach is used.
...
It selects a single feature that produces the best decision rules.

Despite the name OneR, the algorithm generates more than one rule: 
...
1. For each feature, do:
    ...
    - For each feature level: Create a rule which predicts the most frequent class for each level of the feature (can be easily read from the cross table).
    - Calculate the total error of the rules for that feature.
1. Select the feature with the smallest total error.

OneR always covers all instances of the dataset, since it uses all levels of the selectd feature.
Missing values can be either treated as an additional level or be imputed beforehand.
...
The following table shows an artificial dataset about houses that contains information about location, size and whether pets are allowed. 
...
the most common class for instances with this feature level is the prediction, the THEN-part of the rule.
...
The location feature has possible values, $bad$ and $good$. 
The most frequent value for houses in bad locations is $low$ when we use $low$ as a prediction, we make two mistakes, because two houses have a $medium$ value.
...
OneR prefers features with many possible levels, because those features can moverfit the target more easily.
...
Another issue are ties, i.e. when two features result in the same total error.
...
Since $Cancer$ is rare, for each rule the majority and therefore the prediction is always $Healthy$. 
It does not make sense to use the OneR algorithm in this case.
...
The output would be more informative if OneR returned the percentage of instances with the majority class as the prediction rather than just the label of the majority class.
...
We use this trick to predict the number of [bike rentals](#bike) with OneR by cutting the number of bike rentals into four quartiles.
...
The month feature has (surprise!) 12 feature levels, which is more than for most other features.
So there is a danger of overfitting. 
On the more optimistic side: the month feature can handle the seasonal trend (e.g. low rental numbers in winter) and the predictions seem sensible.

Now we move from the simple OneR algorithm to a more complex procedure using rules with more complex conditions consisting ofseveral features: Sequential Covering.

### Sequential Covering

Sequential covering is a general procedure that repeatedly applies an algorithm that learns a single rule to create a decision list (or set) that covers the entire dataset rule by rule.
Many rule-learning algorithms are variants of the sequential covering algorithm.
This chapter introduces the main recipe and uses a variant for the examples, the RIPPER algorithm.

The idea is simple:
First, find a good rule that applies to some of the data points.
Remove all data points which are covered by the rule, that is, each data point to which the conditions apply, regardless of whether the points are classified correctly or not. 
...
This approach of rule-learning, removing covered data and repeating is called 'separate-and-conquer'.
...
For multi-class settings, the approach musts be modified.
...
Then we remove all big houses in a good neighbourhood from the dataset.
With the remaining data we learn the next rule: 
Maybe: 'If $location=good$, then $value=medium$' (Note that this rule is learned on data without big houses in good neighbourhoods, leaving only medium and small houses in good neighbourhoods remain).
...
1. Pre-mine frequent patterns from the data (unsupervised) that can be used as conditions (IF-part) for the decision rules.
...
A frequent pattern is the frequent (co-)occurrence of feature levels.
...
where $x_j=A$ is the feature level, $n$ the number of data points in the dataset and $I$ the indicator function that returns $1$ if the feature $x_j$ of the instance $i$ has level $A$ otherwise $0$.
In a dataset of house values, if 20% of houses have no balcony (and 80% have one or more), then the support for the pattern $balcony=0$ would be 20%.
Support can also be measured for combinations of feature levels, for example the support for $\text{balcony=0}\land\text{pets=allowed}$.
...
For this application, we are only interested in the frequent patterns that are generated in the first part.

In the first step, the Apriori algorithm starts with all feature levels that have a support greater than the minimum support defined by the user. 
...
This does not mean that the houses are removed from the data, it just means that $\text{size=big}$ is not be considered to find frequent patterns.
...
But before we go into the details of BRL, I would like to show you another way for rule-learning based on pre-mined patterns. 
...
BRL addresses this goal by defining a distribution of decision lists with prior distributions for the length of conditions (preferably shorter rules) and the number of rules (preferably shorter list). 

The posteriori probability distribution of lists (kind of meta, right?), makes it possible to say how likelya decision list is, given assumptions of shortness and how well the list fits the data.
...
2) Iteratively modify the list by adding, switching orremoving rules, ensuring that the lists originate the distribution of lists and 
...
where $d$ is a decision list, $x$ are the features, $y$ is the target, $A$ the set of pre-mined conditions, $\lambda$ the prior expected length of decision list, $\eta$ the prior expected number of conditions in a rule, $\alpha$, the prior pseudo-count for the positive and negative classes (best fixed at $(1,1)$).
...
The next step is to generate many new lists starting from this initials sample to obtain many samples from the posterior distribution of decision lists.
From these samples we can select the decision list with the highest posterior probability.

The new decision lists are sampled by starting from the initial list and then randomly either 1) moving a rule to a different position in the list or 2) adding a rule to the current decision list from the pre-mined conditions or 3) removing a rule from the decision list. 
...
This procedure provides us with many samples from the distribution of decision lists (a list of decision lists). 
...
Each continuous features was divided into three intervals by frequency: 
The first interval contains instances with the smallest third of the respective feature values; 
The second interval contains the instances with the feature values that lie between the 33% and 66% quantile; 
The third interval contains the third of the data with the highest feature values.
...
The following table displays the pool of conditions from which the SBRL algorithm could select by building a decision list.
The maximum number of feature values in a condition I allowed as a user was two.
Here is a selection of ten patterns:
...
Decision tree often also suffer from replicated sub-trees, that is, if the splits following a left and a right child node have the same structure.
...
- Decision rules are **robust** against monotonous transformations of the input features, because the only the threshold in the conditions changes.
They are also robust against outliers, since it only depends on whether a condition applies or not.
- IF-THEN rules usually generate very sparse models, which means that not many features are included.
...
- The research and literature for IF-THEN rules is focuses on classification and  almost **completely neglects regression**.
...
Decision trees and rules can only produce step-like prediction functions, where the prediction sharply changes when a feature changes.
This is related with the issue that the inputs have to be categorical (in decision trees, they are implicitly categorized by splitting them).
...
- SBRL is available as [R package](https://cran.r-project.org/web/packages/sbrl/index.html) (which I used in for the examples), in [Python](https://github.com/datascienceinc/Skater) or as [C implementation](https://github.com/Hongyuy/sbrlmod).

I won't even try to make an exhaustive list of alternatives for learning decision rule sets and lists, but will point to some summarizing work. 
...
It provides a holistic framework for thinking about learning rules and features many rule learning algorithms.
