...
An example for a decision list made from some pre-mined rules.
Priori distribution for rule length, number of conditions and so on encourage the rules to be sparse.
Setting: Multi-class classification. 
First: Antecedents (= conditions) are mined.
The Bayesian Rule List uses the FB-Growth algorithm. 
...
The labels have a distribution: 
...
- Pre-mine rules
- sample a decision list from the rules
- sample default rule parameter $\theta_0$
- for all rules: sample cardinality of antecedent, sample antecedent of that cardinality from pre-mined, sample $\theta_j$
- For each observation, find the antecedent that first applies. sample y from priori.
