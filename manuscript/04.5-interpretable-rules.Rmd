...
- while the the accuracy is bigger than a threshold:
  - Add rule $r$ to $rlist$
  - Remove the data that is covered by rule $r$
  - use learn-one-rule on the reamining data
- Return set of rules (sometimes called cover), ordered by accuracy

Alternatives: 
Use other performance measure than accuracy, like entropy or information gain. 
Do some pruning of the rules.
...
bike2$cnt =  cut(bike$cnt, breaks = quantile(bike$cnt))
...
[^m5rules]: M. Hall, G. Holmes, and E. Frank (1999). Generating rule sets from model trees. Proceedings of the Twelfth Australian Joint Conference on Artificial Intelligence, Sydney, Australia, pages 1–12. Springer-Verlag. http://citeseer.ist.psu.edu/holmes99generating.html
