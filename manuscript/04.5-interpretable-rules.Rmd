...
## Building a decision list from pre-mined rules

In this section I will show you another approach to learn a decision list, which follows this general recipe:

1. Pre-mine lots of rules from the data (potentially unsupervised).
1. Construct a decision list from a selection of the pre-mined rules.

One specific approach that uses this general recipe is called (Scalable) Bayesian Rule Lists (Letham et. al, 2015)[^brl], which are decision lists that use methods from Bayesian statistics to learn decision lists from rules pre-mined with the FP-tree algorithm (Borgelt 2005)[^fp-tree]

But let's start slowly:

### Pre-mining of rules

When we dive into the theory of association rule or frequent pattern mining, it is useful to keep the idea of a recommender system in your mind.
For example many shopping websites show "People that bought this bycicle also bought this helmet" or movie recommder systems say something like "People who watched 'Princess Mononoke' and 'My Neighbor Totoro' also watched 'Spirited Away'". 
These are association rules, which are a special kind of IF-THEN rule ("IF someone watched 'Princess Mononoke' THEN also 'Spirited Away'").
Association rule mining or frequent pattern mining is the process of extracting lots of those rules from a dataset. 
These algorithms are unsupervised machine learning algorithms meaning they don't need any label.
An association rule is a decision rule when on the THEN-part the prediction is for the target outcome of interest.
In our context, a pattern is the same as a feature value (e.g. size=medium), which is the combination of a feature (e.g. house size) and some level of that feature (e.g. medium).
In the context of baskets or movies, the features are all binary, a user has watched a movie or not, so it's a bit easier there.

Probably the most popular pattern mining algorithm is the Apriori algorithm. 
The Apriori algorithm works with to measurements to create assocations rules:

Support, which is the frequency of a condition (feature level) in the dataset:
...
For example we if 20% of the houses have no balcony (and 80% one or more), then the support for $x_{balcony}=0$ would be 20%.
The support can also be measured for combinations of feature levels, for example the support for $x_{balcony}=0\land{}x_{pets}="allowed"$.

The other measure is the accuracy of a rule (usually called the 'confidence', but in our context of rule learning it makes sense to say accuracy). 
The accuracy is the probability that the prediction of a rule (THEN-part) is true, given that the conditions apply:

$$Accuracy((x_j=A)\rightarrow(y=B))=\frac{Support(x_j=A\land{}y=B)}{Support(x_j=A)}$$


The Apriori algorithm gets as input the dataset with categorical features (where the target is also treated like a categorical feature), the minimal support as decided by the user and the minimal confidence as decided by the user.
The algorithm works in two steps:
1) Find frequent combination of feature levels 2) construct assocation rules from the frequent combinations.

For the first step, the Apriori algorithm starts with all feature levels that have a support larger than the minimal support set by the user. 
...
Makes sense: By removing a condition from a pattern, the reduced pattern can  only cover more or the same number of data points (support), but never less. 
...
In the second step we create association rules from the frequent item sets that have at least the minimal support and the minimal accuracy. 
Assocation rules have one item on the right hand side (THEN-part) and arbitrary number of conditions.
For example from the frequent pattern 'size=medium AND location=bad AND value=medium' we would create 3 rule candidates and check them for support and accuracy:
$\text{size=medium}\land\text{location=bad}\rightarrow\text{value=medium}$, 
$\text{size=medium}\land\text{value=medium}}\rightarrow\text{location=bad}$ and 
$\text{value=medium}\land\text{location=bad}\rightarrow\text{size=medium}$ 
But since we are interested in only predicting the value we would only consider the first rule.
This second part produces lots of association rules.
When we use those in the context of a supervised machine learning problem like classification, where we know the label, we are not interested in all the rules that are created.
We can still use those rules by keeping only the ones that have the target outcome on the right hand side (the THEN-part).
Once we have mined those rules, filtered for those that have the outcome of interest in the THEN-part, we already have a decision set. 
We would only need some mechanism how to average predictions in the case of overlapping rules
...
So there are more elaborate approaches:

## (Scalable) Bayesian Rule Lists

We want to reduce the number of the pre-mined rules, prioritize short rules over long ones and get have a decision list (=with order) and not a decision set.
The Bayesian Rule List (BRL) approach solves this by introducing a likelihood function with some prior distributions for the length of rules (preferring shorter ones) and the number of rules (also preferring less rules over more). 
...
The examples are using a faster variant of BRL called Scalable Bayesian Rule Lists by Yang et. al (2016) [^sbrl].
...
rules = sbrl(bike2, pos_sign = TRUE, neg_sign = FALSE)
rules
```

## Advantages of IF-THEN rules
...
## Disadvantages of IF-THEN rules
...
There are lots of ways to bin a continuous feature:
How many splits do you have? 
Do you use fixed intervals? 
Do you use quantiles?
This is a non-trivial issue that is often neglected and one of the option simply applied (like I did in my examples).
