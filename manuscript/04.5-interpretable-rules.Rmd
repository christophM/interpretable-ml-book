...
At each step, the algorithm evaluates the posteriori probability of the decision list (mix of how acurate it is and how much it satisfies the priori distributions).
The Metropolis Hastings algorithm ensures that we sample more heavily decision lists thathava a high posterior probability.
...
Each continuous features was divided into three intervals by frequenc: 
...
Decision tree also often suffer from replicated subtrees, i.e. when the splits that follow a left and right child node have the same structure.
...
- OneR is implemented in the an [R package `OneR`](https://cran.r-project.org/web/packages/OneR/), which was used for the examples in this book. 
...
- RIPPER is also implemeted in Weka. For the examples, I used the R implementation of JRIP in the [RWeka package](https://cran.r-project.org/web/packages/RWeka/index.html). 
- SBRL is vailable as [R package](https://cran.r-project.org/web/packages/sbrl/index.html) (which I used in for the examples), in [Python](https://github.com/datascienceinc/Skater) or as [C implementation](https://github.com/Hongyuy/sbrlmod).
