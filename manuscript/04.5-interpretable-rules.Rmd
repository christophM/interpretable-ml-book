...
Their IF-THEN structure semantically resembles natural language and the way we think, assuming the condition is constructed from intelligible features and the length of the condition (number of $feature=value$ pairs connected with an AND) and assuming there are not too many rules.
...
There is usually a trade-off between accuracy and support:
...
For example, the size feature with the levels $small$, $medium$ and $big$ produces three rules.
...
```
For example: 
We have a task and dataset for predicting the values of houses from size, location and if pets are allowed.
...
- Build a decision tree (with CART or some other tree learning algorithm).
- Start at the root node and recursively choose the purest node (e.g. with the lowest misclassification rate).
...
Since RIPPER only works for classification, the bike counts have to be turned into a categorical outcome. 
...
As a pre-processing step for the BRL algorithm, we use the features (at this step we don't need the target outcome) and extract frequently occurring patterns from them.
...
The posteriori probability distribution of lists (kind of meta, right?), enables to tell how likely  a decision list is given assumptions of shortness and how well the list fits the data.
...
It's important to know that the Bayesian approach is a way to combine pre-existing knowledge or requirements (called priori distributions) while also fitting the data.
...
It multiplicatively incorporates a truncated Poisson distribution (parameter $\lambda$) for the number of rules in the list, and a truncated Poisson distribution (parameter $\eta$) for the number of feature values in the conditions of the rules.
...
From these samples we can pick the decision list with the highest posterior probability.o

The new decision lists are sampled by starting from the initial list and then randomly either 1) moving a rule to a different position in the list or 2) adding a rule to the current decision list from the pre-mined conditions 3) Removing a rule from the decision list. 
Which of the rules is switched, added or deleted is chosen at random.
At each step, the algorithm evaluates the posteriori probability of the decision list (mix of how accurate it is and how much it satisfies the priori distributions).
The Metropolis Hastings algorithm ensures that we sample more heavily decision lists that have a high posterior probability.
This procedure gives us lots of samples from the distribution of decision lists (a list of decision lists). 
We end up with the posterior distribution of decision lists.
The BRL algorithm selects the decision list of the samples with the highest posterior probability.


That's it with the theory, no let's see the BRL method in action.
The examples are using a faster variant of BRL called Scalable Bayesian Rule Lists (SBRL) by Yang et. al (2016) [^sbrl].
First we apply the SBRL algorithm to predict the [risk for cervical cancer](#cervical).
...
the first interval contains instances with the lowest third of the respective feature values; 
...
Notice that we get meaningful rules, since the prediction on the THEN-part is not the class outcome, but the predicted probability for cancer.
...
The maximum number of feature values in a condition allowed by me as a user was two.
...
I arbitrarily created a classification task by creating a label that is 1 if the number of bike rentals exceeds 4000 bikes on a day, else 0.
...
They are probably the most interpretable among the interpretable models.
This statement applies only if the number of rules is small, the conditions of the rules are short (maximum 3 I would say) and if the rules are organized in a decision list or a non-overlapping decision set.
- Decision rules can be **as expressive as decision trees, while being more compact**. 
Decision tree also often suffer from replicated sub-trees, i.e. when the splits that follow a left and right child node have the same structure.
...
- The research and literature for IF-THEN rules is focused on classification and is almost **completely neglecting regression**.
...
In general, approaches are more attractive when they can be used for both regression and classification.
...
Dealing with binning of continuous features is a non-trivial issue that is often neglected and people just use the next best method (like I did in the examples).
...
This is related with the issue that the inputs have to be categorical (in decision trees they are implicitly made categorical by splitting them).
...
I won't even attempt to create an exhaustive list of alternatives for learning decision rule sets/lists, but rather point towards some summarizing work. 
...
- IF-THEN rules can be used in linear models, which is described in this book in the chapter about the [RuleFit algorithm](#rulefit).
