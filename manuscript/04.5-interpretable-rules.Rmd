...
- OneR. There is an [R package `OneR`](https://cran.r-project.org/web/packages/OneR/). 
- RIPPER: In Weka, available for Java, R, Python and so on. I used the R implementation of JRIP RWeka package. TODO: Cite
- SBRL: Only classification and only categorial features. Uses Bayesian statistics for learning the rules and a clever bit-wise implementation. Builds a one-sided tree or decision list. Available as R package and in Python or the [pure C implementation](https://github.com/Hongyuy/sbrlmod).


## Alternatives

I want even attempt to create an exhaustive list of alternatives, but rather point towards some more recent research and summarizing work. 
