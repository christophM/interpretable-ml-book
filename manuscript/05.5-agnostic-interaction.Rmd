...
a constant term, a term for the first feature, a term for the second feature and a term for the interaction effect between the two features.  
...
Let's now look at an example with an interaction:
...
This is an interaction between size and location, because in this case the difference in predicted value between a big and a small house depends on the location. 
...
In theory, arbitrary interactions between any number of features can be measured, but these two cases are the most interesting interaction cases.


If two features $x_j$ and $x_k$ do not interact, we can decompose the [partial dependence function](#pdp) as follows (assuming the partial dependence functions are centered at zero):
...
where $PD_{-j}(x_{-j})$ is the partial dependence function that depends on all features except $x_j$.

This decomposition expresses the partial dependence (or full prediction) function without interactions (between features j and k, respectively j and all other features). 
...
To generate the interaction statistic under the null hypothesis, you must be able to adjust the model so that it has no interaction between feature $x_j$ and $x_k$ or all others.
This is not possible for all types of models.
Therefore this test is model-specific and not model-agnostic and as such is not covered here.

The interaction strength statistic can also be applied in a classification setting if the prediction is the probability for a class.
...
```{r interaction-bike, fig.cap = 'The interaction strength for each feature with all other features for a support vector machine predicting bicycle rentals. Overall, the interaction effects between the features are very weak (below 1 percent of variance explained by each feature).'}
data(bike)
library("mlr")
library("iml")
library("ggplot2")
...
In this next example, we calculate the interaction statistic for a classification problem, dealing with the partial dependence of the predicted probability.
...
```{r interaction-cervical-include, fig.cap = 'The interaction strength for each feature with all other features for a random forest predicting the probability of cervical cancer. The number of sexually transmitted diseases diagnosed has the highest interaction effect with all other features, followed by the number of pregnancies.'}
...
```{r interaction2-cervical-age, eval = TRUE, fig.show = "hide"}
# Due to long running time and timeouts on TravisCI, this has to be run locally. 
# And image has to be added to git repo manually.
...
```{r interaction2-cervical-age-include, fig.cap = 'The 2-way interactions between number of pregnancies and each other feature. There is a strong interaction between the number of pregnancies and the age.'}
...
These **estimates also have a certain variance** if we do not use all of the data points. 
...
Concerning the test problem, it is difficult to say when the H-statistic is large enough for us to consider an interaction as strong.

The H-statistic tells us how strong the interactions are, but it does not tell us how the interaction looks like.
...
You can't say in general if it leads to overestimation or underestimation.
...
The R package [pre](https://cran.r-project.org/web/packages/pre/index.html) implements the [RuleFit algorithm](#rulefit) and includes the H-statistic.
...
This approach measures the feature importance (defined as the variance of the 1D partial dependence function) of one feature conditional on different, fixed points of the other feature. 
...
[^Hooker2004]: Hooker, Giles (2004). Discovering Additive Structure in Black Box Functions. Knowledge Discovery and Data Mining, 575–580. http://doi.org/10.1145/1014052.1014122


[^Greenwell2018]: Greenwell, Brandon. M., Boehmke, B. C., & McCarthy, A. J. (2018). A Simple and Effective Model-Based Variable Importance Measure, 1–27. Retrieved from http://arxiv.org/abs/1805.04755
