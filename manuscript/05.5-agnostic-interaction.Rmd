...
We analyze the interactions between features in a random forest trained to predict [penguins species](#penguins), given bodily measurements.

```{r interaction-penguins-prep}
...
```{r interaction-penguins, eval = FALSE, fig.show = "hide"}
...
```{r interaction-penguins-include, fig.cap = 'The interaction strength (H-statistic) for each feature with all other features for a random forest predicting the probability of penguin species. The feature with the overall the strongest interactions is the bill length.'}
...
```{r interaction2-penguins-bill, eval = FALSE, fig.show = "hide"}
# Due to long running time and timeouts on TravisCI, this has to be run locally. 
# And image has to be added to git repo manually.
