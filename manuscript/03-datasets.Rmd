...
For classification, we will be using the Palmer penguin data.
This cute dataset contains measurements from hundreds of penguins from the Palmer Archipelago in the Antarctica.
...
The dataset is loaded via the palmerpenguins R package [^palmerpenguins-r-package].

![The 3 penguin species in the data: Chinstrap, Gentoo, Adelie. Artwork by @allison_horst](images/lter_penguins.png).

Each row contains the following information:

- The species of the penguin (Chinstrap/Gentoo/Adelie) which we will use as output for classification for the examples in this book.
- The weight of the penguin, measured in grams.
- Bill length, measured in millimeters (see also the following Figure).
- Bill depth, measured in millimeters
- Sex of the penguin (male/female)
- The island the penguin came from (Biscoe/Dream/Torgersen). However, we won't be using this as feature since it is too predictive of the species.

![Bill length measurements. Artwork by @allison_horst.](images/culment_depth.png).

11 penguins had missing data.
Since the purpose is a simple demonstration of interpretable machine learning and not an in-depth penguin study, I simply dropped the missing values.
