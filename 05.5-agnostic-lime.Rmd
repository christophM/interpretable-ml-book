...
- Pertube your dataset and  get the black box predictions for these new points.
...
LIME sampling is not done around the instance of interest, but from the training data's mass center, which is problematic.
...
Figure  \@ref(fig:lime-tabular-example-explain-plot-2) shows a variant where the continuous features are turned into categorical features by putting them into bins along the quantiles.



```{r lime-tabular-example-explain-plot-2, fig.cap=sprintf('LIME explanations for two instances of the bike rental classification task. This time continuous features were turned into categorical features by binning them.')}
...
```{r lime-text-variations-html, results='asis', eval=output.type=='html'}
DT::datatable(variations, options = list(DT::JS('{"scrollX": true}')))
```
```{r lime-text-variations-latex, results='asis', eval=output.type=='latex'}
kable(variations)
```
