...
```{r show-dating-data-speed-dating}
source(sprintf('%s/load_speed_dating_data.R', src_dir))

set.seed(42)
speed_dating_sample = speed_dating_both[sample(1:nrow(speed_dating), size=50), ]

DT::datatable(speed_dating_sample, options = list(DT::JS('{"scrollX": true}')))
```

## TubeSpam dataset: Spam classification on YouTube comments {#TubeSpam}
...
DT::datatable(tube_spam[c('CONTENT', 'CLASS')], options = list(DT::JS('{"scrollX": true}')))
```
<br />
<br />
...
X = speed_dating_regression[features_of_interest]
colnames(X) = speed_dating_names_matches[colnames(X)]

y = speed_dating_regression[,target_var_regress]
dat = cbind(X, y)
