...
For example, for \(x_2 = 1\), the effect of \(x_1\) is \(\hat{f}(x_1, x_2 = 1) = x_1\), and when \(x_2 = 10\) the effect is \(\hat{f}(x_1, x_2 = 10) = 10 \cdot x_1\).
...
For \(S=\{2,3\}\), we subtract the main effects of both features \(\hat{f}_2\) and \(\hat{f}_3\), as well as the intercept \(\hat{f}_0\).
...
The result is again \(\hat{f}_{ALE,12}\), meaning we can apply the same operator severeal times and always get the same ALE plot.
...
Recently an extension of MMD-Critic was developed: Protodash. The authors claim advantages that are available on the \href{https://arxiv.org/pdf/1707.01212.pdf}{publication}.
A Protodash implementation is available on \href{https://github.com/Trusted-AI/AIX360}{IBM AIX360} tool.
...
DiCE implements both model-agnostic and a gradient-based methods.
...
\caption{Pixel attributions or saliency maps for the vanilla method, smoothgrad and Grad-Cam.}\label{fig:unnamed-chunk-60}
\end{figure}
