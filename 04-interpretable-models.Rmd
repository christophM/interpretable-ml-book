...
- A **(Machine learning) Model** is the outcome of a machine learning algorithm. This can be a set of weights for a linear model or for a neural network plus the information about the architecture.
...
$$\hat{\boldsymbol{\beta}} = \arg\!\min_{\beta_0, \ldots, \beta_p} \sum_{i=1}^n \left(y_i - \left(\beta_0 + \sum_{j=1}^p \beta_j x_{ij}\right)\right)^{2}$$
...
- Categorical feature with multiple levels: A feature with a fixed amount of possible values. An example is the feature "Flooring type", with possible levels "carpet", "laminate" and "parquet". One solution to deal with many levels is to one-hot-encode them, meaning each level gets it's own binary column. From a categorical feature with $l$ levels, you only need $l-1$ columns, otherwise the coding is   overparameterised. The interpretation for each level is then according to the binary features. Some languages, like R, allow you to code categorical features in different ways, see Chapter \@ref(cat.code).
...
The horizontal lines extend to$\pm 1.58 \text{ IQR} / \sqrt{n}$, with IQR being the inter quartile range ($q_{0.75} - q_{0.25}$).
...
Judging by the attributes that constitute a good explanation as presented in Chapter \@ref(good-explanation), linear models don't create the best explanations.
...
Another aspect of a good explanation is selectivity, which can be achieved in linear models by using less features or by fitting sparse linear models, see Chapter \@ref(sparse-linear).
...
RuleFit [@friedman2008predictive] fits a sparse linear model with automatically created binary decision rules as features.

One big problem with linear models is that they don't account for interaction effects between features.
Wouldn't it be cool to have a model that is as simple as linear models, but can, somehow, also integrate feature interactions?
Let me introduce RuleFit to you!

RuleFit fits a sparse linear model with (optionally) the original features and also a set of new features which are decision rules, extracted from decision trees.
These new features are interaction effects between original features.
RuleFit generates these features automatically.
It uses the trick, that you can create for each path through a tree a decision rule by combining the split decisions to a rule, see for example Figure \@ref(fig:rulefit-split).
```{r rulefit-split, fig.cap="4 rules can be generated from a tree with 3 terminal nodes.", out.width="80%"}
knitr::include_graphics("images/rulefit.jpg")
```
Where are those trees coming from?
These are trees that are trained to predict the outcome of interest, so the splits it finds are meaningful for the task at hand and not arbitrary.
Basically any algorithm that creates trees can be used, like a Random Forest for example.
Each tree from that ensemble is then disassembled into decision rules, which are used in the Lasso linear model.
...
RuleFit also comes with automatic feature importance measurements.
This makes it easy to see, which linear feature or rule was important globally for the predictions.
The feature importance is calculated from the weights of the regression model.

The RuleFit paper (@friedman2008predictive) also introduces partial dependence plots to plot the average change of the prediction by changing one feature.
The partial dependence plot is a model-agnostic method, which can be used with any model, and it has it's own part in the book, see Chapter \@ref(dp).

### Interpretation and Example

The interpretation is the same as in linear models.
The only difference is that some features are coming from decision rules.
So they are binary features, with 1 meaning that the conditions of the rule are met, and 0 otherwise.
...
coefs = left_join(coef(mod), importance(mod, plot=FALSE)$baseimp)
...
```
An example of rules that are generated from the bike dataset (see Chapter \@ref(bike-data)) can be seen in Figure \@ref(fig:rulefit-example).
The most important rule was: `r coefs[1, 'description']`.
When all these conditions are met, the predicted number of bike rentals increases by `r round(coefs[1, 'coefficient'], 1)`.
In total,  `r nrow(coef(mod))` rules were created from the original `r ncol(X)` features.
Quite a lot!
But thanks to Lasso, only `r nrow(coefs[coefs$coefficient != 0,])` of the `r nrow(coef(mod))` got a weight different from zero.

```{r rulefit-example, fig.cap="The table shows the resulting rules and weights after fitting 'RuleFit' on the bike dataset. ", out.width="80%"}
DT::datatable(coefs[c('description', 'coefficient', 'imp', 'sd')], options = list(scrollX=TRUE),
colnames = c('Description', 'Weight', 'Importance', 'Std. Dev'),
...
The feature importance measurement includes the importance from interaction of a feature with another feature.
...
Figure \@ref(fig:rulefit-importance) shows the aggregated feature importance values.
The feature importances for each rule are attributed to each base feature from X.
...
The original RuleFit paper also introduces the partial dependence plot which displays the average prediction.
The partial dependence method is not specific to RuleFit and can in fact be applied to any machine learning model.
That's why it deserves its own space in the book, see Chapter \@ref(dpd).

```{r rulefit-trees, fig.cap="4 rules can be generated from a tree with 3 terminal nodes.", out.width="80%", eval = FALSE}
library('pre')
features_of_interest = c('season','holiday', 'workingday', 'weathersit', 'temp', 'hum', 'windspeed', 'days_since_2011')
X = bike.train[features_of_interest]
...
plot(mod, nterms = 3, plot.dim = c(3,1), linear.terms = FALSE)
```
...
What is slightly different is the interpretation when the feature $x_j$ is a decision rule:
If all conditions of decision rule $x_j$ apply, the predicted outcome changes by $\beta_j$.
And, respectively, for classification: If all conditions of decision rule $x_j$ apply, the odds for event vs. no-event changes by a factor of $\beta_j$.


**The Good**:

- RuleFit brings **automatic interaction** modeling to linear models.
Therefore it solves the problem of linear models with having to manually add interaction terms and, partially, the problem of modeling non-linear relationships.
...
Either the feature applies or not to an instance.
This only holds as long as the number of conditions within a rule is not to big.
1 to 3 conditions seems reasonable to me.
This translates into a maximum depth of 3 for the trees in the tree ensemble.
- Even if there are many rules, they do not apply to each instance, so for one instance only a handful of rules are important which makes interpretation easy.
- RuleFit comes with a bunch of tools, namely an importance measurement, interaction effects calculation and a partial dependence plot.


**The Bad**:

- Sometimes RuleFit creates many rules which are also not sparsed out much by Lasso and you end up with a lot of non-zero weights. This is not nice to interpret.
A promising approach is to force feature effects to be monotonic, meaning that an increase in a feature has to result in an increase of the predicted outcome.
- A more anecdotal drawback: The papers claim good performance of RuleFit - often close to the predictive performance of Random Forests! - yet in the few cases where I personally tried it, the performance was disappointing.
...
When the weather is good and the temperature is above 10 degrees, the temperature is automatically also always bigger then 15, which means if the second rule applies, the first one also applies.
...
The first component produces "rules" from decision trees and the second component fits a linear model with these rules as input (hence the name "RuleFit").
...
The rules are derived by disassembling decision trees:
Every path in a tree can be turned into a decision rule.
The trees are fitted to predict your target outcome.
So the splits and resulting rules are optimized to predict the outcome you are interested in.
...
Any type of ensemble of trees can be used to generate the trees:
$$F(x) = a_0 + \sum_{m=1}^M a_m f_m(X)$$
where $M$ is the size of the ensemble (number of trees). and $f_m(x)$ is one of its members (a single tree).
The $\{f_m(x)\}_1^M$ is the set of base learners (trees).
Bagged ensembles, Random forests, AdaBoost and MART are all algorithms that yield an ensemble of trees and can be used for RuleFit.

From all of the trees of the ensemble, we produce the rules.
See Figure \@ref(fig:rulefit-split).
Each takes rule $r_m$ takes on the form:
$$r_m(x) = \prod_{j \in \text{T}_m} I(x_j \in s_{jm})$$
where $\text{T}_{m}$ is the set of features used in tree $m$, $I(\cdot)$ is the indicator function, which is 1 if the feature $x_j$ is in the specified subset of values $s_{jm}$ for $x_j$ (as specified by the tree splits) and otherwise 0.
For numerical features, $s_{jm}$ is usually a range in the feature values with lower and upper limit $x_{s_{jm}, \text{lower}} < x_j < x_{s_{jm}, upper}$.
This is the general formulation, but in practice only one of the limits is in the rule and the other limit is implicitely the minimum or maximum value of $x_j$ in the data.
For categorical features the subset $s_{jm}$ contains some specific categories of $x_j$.

An example, which might be come out for the bike rental data set:
...
The decision rules weighting will happen in step 2 of fitting RuleFit.
...
Every rule and numerical feature gets a weight.
The numerical features are added because trees suck at representing simple linear relationships between y and x.
Before we put everything into Lasso to get a sparse linear model, we winsorize the original features, so that they are more robust against outliers:
...
A good value is usually $\delta = 0.025$.
In addition, the linear terms have to be normalized so that they have the same prior influence as a typical decision rule:
...
We combine both types of features to generate a new features matrix and estimate a sparse linear model with Lasso, with the following structure:
$$ \hat{f}(x) = \hat{\beta}_0 + \sum_{k=1}^K\hat{\alpha}_k r_k(x) + \sum_{j=1}^p\hat{\beta}_j l_j (x_j)$$
and since it will be estimated with Lasso, the loss function gets the additional constraint that forces some of the weights to get a zero estimate:
$$(\{\hat{\alpha}\}_1^K, \{\hat{\beta}\}_0^n) = argmin_{\{\hat{\alpha}\}_1^K, \{\hat{\beta}\}_0^n} \sum_{i=1}^n L(y_i, f(x)) + \lambda \cdot (\sum_{k=1}^K |\alpha_k| + \sum_{j=1}^p |b_j|)$$
...
For the linear terms, the feature importance is measured with the standardized predictor:
...
where $\hat{\beta{k}}$ is the associated Lasso weight of the decision rule and $s_k$ is the support of the feature in the data, which is the percentage of data points for which the decision rule applies: $s_k = \frac{1}{n}\sum_{i=1}^n r_k(x_i)$

A feature occurs as a linear term and also within possibly many decision rules.
How do we measure the importance of the original feature $x_j$?
...
The importance of the linear predictor $I_l$ can be measured by looking at the
