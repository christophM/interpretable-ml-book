...
It uses the trick, that you can create for each path through a tree a decision rule by combining the split decisions to a rule, see for example Figure \@ref(rulefit-split).
...
TODO: Mention feature importance, interaction effects and partial dependence
...
An example of rules that are generated from the bike dataset (see Chapter \@ref(bike-data)):

```{r rulefit-example, fig.cap="4 rules can be generated from a tree with 3 terminal nodes.", out.width="80%"}
library('pre')
features_of_interest = c('season','holiday', 'workingday', 'weathersit', 'temp', 'hum', 'windspeed', 'days_since_2011')
X = bike.train[features_of_interest]
...
coefs <- coef(mod)
DT::datatable(coefs[coefs$coefficient!=0,], options = list(scrollX=TRUE))
```

TODO: Show table with importance, coefficient, support and rule. Do interpretation.

TODO: Add extended bike sharing example here.

TODO: Add interpretation example

TODO: Add importance measurement.

TODO: Add feature interaction example

TODO: Add partial dependence plots and refert to pdp chapter

TODO: Add cervical cancer example

TODO: Mention number of rules that were generate for each application example. How many with non-zero weights?
