...
The i-th observation's outcome is a weighted sum of it's $p$ features. The $\beta_{j}$ represent the learned feature weights or coefficients. The $\epsilon_{i}$ is the error we are still making, the difference between the predicted and actual outcome.
...
- **Independence**: Each observation is assumed to be independent from the next one. If you have repeated measurements, like multiple records per patient, the data points are not independent from each other and there are special linear model classes to deal with these cases, like mixed effect models or GEEs.
...
effect_plot(mod, dat)
```
The largest contributions are from temperature and the days feature, which capture the trend that the bike rental became more popular over time. The temperature has a high contribution distribution.
The day trend feature has goes from zero to large positive contribution, because the first day in the dataset (1.1.2011) get's a very low contribution, and the estimated weight with this feature is positive (`r sprintf('%.2f', lm_summary_print['days_since_2011', 'Estimate'])`), so the effect gets higher with every day and is highest for the latest day in the dataset (31.12.2012).
...
Let's have a look at the effect realization for the rental bike count of one observation (= one day). Some features contribute unusually much to the predicted bike count: Temperature (`r X[i, 'temp']`) and the trend feature "days_since_2011", because this instance is from late 2011 (value = `r  X[i, 'days_since_2011']`).
...
LASSO stands for "least absolute shrinkage and selection operator" and when added to a linear model, it performs feature selection and regularization of the selected features.
